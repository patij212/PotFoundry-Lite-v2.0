...
def test_obj_has_outward_unit_vertex_normals(tmp_path):
    ...
    # Smooth vertex normals must agree with the (outward) face orientation:
    # for every face, the geometric face normal and the average of its three
    # vertex normals point the same way. This proves the smooth normals are
    # outward everywhere (outer wall outward, inner wall toward the cavity),
    # without having to classify which wall each vertex belongs to.
    f0 = pf[:, 0] - 1
    f1 = pf[:, 1] - 1
    f2 = pf[:, 2] - 1
    face_n = np.cross(verts[f1] - verts[f0], verts[f2] - verts[f0])
    vavg = (pn[f0] + pn[f1] + pn[f2]) / 3.0
    agree = np.einsum("ij,ij->i", face_n, vavg)
    # Allow a tiny fraction of tight-crease faces (rim/drain) to be ambiguous.
    assert np.mean(agree > 0) > 0.98
