...
def test_property_mesh_is_watertight(H, Rt, Rb, t_wall, t_bottom, r_drain, n_theta, n_z):
    ...
    for edge, count in edge_counts.items():
        assert count == 2, f"Edge {edge} appears {count} times, expected 2 (watertight mesh)"
...
def test_property_volume_increases_with_diameter(H, Rt, scale_factor):
    ...
    assert vol2 > vol1, f"Scaled pot volume {vol2:.1f} not larger than original {vol1:.1f}"
