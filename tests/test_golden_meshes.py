...
class TestMeshProperties:
    ...
    def test_mesh_has_consistent_normals(self):
        """Verify the mesh is consistently oriented with outward normals.

        Uses the divergence-theorem signed volume: for a closed, consistently
        wound mesh it is positive iff every face normal points out of the solid.
        (See tests/test_mesh_quality.py for the full per-property breakdown.)
        """
        ...
        # All face normals must be non-zero (no degenerate faces).
        v0 = verts[faces[:, 0]]
        v1 = verts[faces[:, 1]]
        v2 = verts[faces[:, 2]]
        ...
        # And they must all be consistently oriented outward.
        result = validate_mesh(verts, faces)
        assert result.oriented, f"{result.reversed_edges} inconsistently wound faces"
        assert result.outward, f"normals point inward (vol={result.signed_volume:.1f})"
