...
class TestMeshProperties:
    ...
    def test_mesh_has_consistent_normals(self):
        """Verify face normals point consistently outward (closed-solid invariant).

        For a watertight mesh the global signed volume is positive iff every
        triangle is wound CCW seen from outside, i.e. all normals point outward.
        This is what lets Rhino/Grasshopper recognise the mesh as a closed
        solid. See tests/test_mesh_orientation.py for the full matrix.
        """
        style_fn = STYLES["SuperformulaBlossom"][0]
        ...
        # Outward orientation: signed volume must be strictly positive.
        signed_volume = float(
            np.sum(np.einsum("ij,ij->i", v0, np.cross(v1, v2))) / 6.0
        )
        assert signed_volume > 0, (
            f"Mesh normals point inward (signed volume {signed_volume:.1f} <= 0)"
        )
