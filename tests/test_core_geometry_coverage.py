...
class TestCoreGeometryErrorHandling:
    ...
    def test_build_pot_mesh_with_zero_resolution(self):
        ...
        verts, faces, diag = build_pot_mesh(
            ...
            expn=1.0, n_theta=8, n_z=4,  # Very low resolution
    ...
    def test_build_pot_mesh_extreme_wall_thickness(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=100, Rt=60, Rb=40, t_wall=5, t_bottom=5, r_drain=6,
            expn=1.0, n_theta=60, n_z=40,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts={}
        )
        assert verts.shape[0] > 0
    ...
    def test_build_pot_mesh_with_large_drain_hole(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=120, Rt=70, Rb=50, t_wall=3, t_bottom=3, r_drain=15,
            expn=1.0, n_theta=80, n_z=50,
    ...
    def test_build_pot_mesh_inverted_taper(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=100, Rt=40, Rb=60, t_wall=3, t_bottom=3, r_drain=8,
            expn=1.0, n_theta=80, n_z=50,
    ...
    def test_build_pot_mesh_with_very_low_expn(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=120, Rt=80, Rb=40, t_wall=3, t_bottom=3, r_drain=8,
...
class TestCoreGeometryCombinedFeatures:
    ...
    def test_build_pot_with_all_features_combined(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=150, Rt=80, Rb=50, t_wall=3, t_bottom=3, r_drain=10,
            expn=1.3, n_theta=120, n_z=80,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts=opts
        )
        assert verts.shape[0] > 0
    ...
    def test_build_pot_with_negative_bell_and_spin(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=130, Rt=70, Rb=45, t_wall=3, t_bottom=3, r_drain=9,
            expn=1.1, n_theta=100, n_z=70,
            r_outer_fn=STYLES["FourierBloom"][0],
            style_opts=opts
        )
        assert verts.shape[0] > 0
...
class TestCoreGeometryDiagnostics:
    ...
    def test_diagnostics_with_high_clamp_ratio(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=100, Rt=60, Rb=40, t_wall=8, t_bottom=5, r_drain=10,
            expn=1.0, n_theta=80, n_z=50,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts={}
        )
    ...
    def test_diagnostics_diameter_estimates(self):
        ...
        Rt, Rb = 70, 50
        verts, faces, diag = build_pot_mesh(
            H=120, Rt=Rt, Rb=Rb, t_wall=3, t_bottom=3, r_drain=10,
            expn=1.0, n_theta=100, n_z=60,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts={}
        )
...
class TestWriteAsciiStl:
    ...
    def test_write_ascii_stl_creates_file(self):
        ...
        verts, faces, _ = build_pot_mesh(
            H=80, Rt=50, Rb=35, t_wall=3, t_bottom=3, r_drain=8,
            expn=1.0, n_theta=40, n_z=30,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts={}
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.warns(DeprecationWarning, match="write_ascii_stl is deprecated"):
    ...
    def test_write_ascii_stl_correct_face_count(self):
        ...
        verts, faces, _ = build_pot_mesh(
            H=60, Rt=40, Rb=30, t_wall=3, t_bottom=3, r_drain=6,
            expn=1.0, n_theta=30, n_z=20,
            r_outer_fn=STYLES["FourierBloom"][0],
            style_opts={}
        )
...
class TestCoreGeometryBoundaryConditions:
    ...
    def test_build_pot_very_short(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=20,  # Very short
            Rt=80, Rb=75, t_wall=3, t_bottom=3, r_drain=10,
            expn=1.0, n_theta=80, n_z=10,
            r_outer_fn=STYLES["HarmonicRipple"][0],
            style_opts={}
        )
        assert verts.shape[0] > 0
    ...
    def test_build_pot_very_tall(self):
        ...
        verts, faces, diag = build_pot_mesh(
            H=300,  # Very tall
            Rt=50, Rb=40, t_wall=3, t_bottom=3, r_drain=8,
            expn=1.2, n_theta=100, n_z=150,
            r_outer_fn=STYLES["SpiralRidges"][0],
            style_opts={}
        )
        assert verts.shape[0] > 0
...
class TestCoreGeometryAllStyles:
    ...
    def test_all_styles_produce_valid_meshes(self):
        ...
        for style_name, (style_fn, description) in STYLES.items():
            verts, faces, diag = build_pot_mesh(
                H=100, Rt=60, Rb=40, t_wall=3, t_bottom=3, r_drain=8,
                expn=1.1, n_theta=80, n_z=50,
                r_outer_fn=style_fn,
                style_opts={}
            )
            assert verts.shape[0] > 0, f"{style_name} produced no vertices"
            assert faces.shape[0] > 0, f"{style_name} produced no faces"
            assert diag['estimated_top_od_mm'] > 0, f"{style_name} has invalid top diameter"
            assert diag['estimated_bottom_od_mm'] > 0, f"{style_name} has invalid bottom diameter"
...
class TestCoreGeometryNumericalStability:
    ...
    def test_build_pot_with_extreme_taper_ratio(self):
        ...
        verts, faces, diag = build_pot_mesh(
            ...
            Rb=25,   # Narrow bottom
            t_wall=3, t_bottom=3, r_drain=6,
            expn=1.5, n_theta=100, n_z=80,
            r_outer_fn=STYLES["SuperformulaBlossom"][0],
            style_opts={}
        )
        assert verts.shape[0] > 0
