...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    v10 = outer_idx[1:, :][:, j]
    v11 = outer_idx[1:, :][:, jn]
    # Wind CCW seen from outside (+radial) so outer-wall normals face outward.
    ...
    vi10 = inner_idx[1:, :][:, j]
    vi11 = inner_idx[1:, :][:, jn]
    # Inner wall: normals face the cavity (radially inward, away from solid).
    ...
    # Rim cap: normals face up (+Z), away from the solid wall below.
    ...
    # Bottom underside: normals face down (-Z), away from the solid slab above.
