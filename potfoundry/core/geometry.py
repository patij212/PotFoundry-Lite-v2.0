...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    # Winding note (CAD export quality): every face group below is wound so the
    # whole shell is a single *consistently oriented* manifold with normals
    # pointing OUT of the solid (positive signed volume). Outer wall normals
    # therefore point away from the central axis. See tests/test_mesh_orientation.py.
    rows = len(z_outer) - 1
    ...
    vi10 = inner_idx[1:, :][:, j]
    vi11 = inner_idx[1:, :][:, jn]
    # Inner wall faces the cavity, so its normals point toward the axis
    # (opposite winding to the outer wall).
