...
def base_radius(z: float | npt.NDArray[np.float64], H: float, Rb: float | npt.NDArray[np.float64], Rt: float | npt.NDArray[np.float64], expn: float, opts: Dict[str, Any]) -> float | npt.NDArray[np.float64]:
...
def _spin_twist_radians(z: float, H: float, opts: dict) -> float:
    ...
    phase_deg = float(opts.get("spin_phase_deg", opts.get("twist_start_angle_deg", 0.0)))
    curve = max(0.1, float(opts.get("spin_curve_exp", opts.get("twist_ease_exponent", 1.0))))
...
def r_outer_superformula_blossom(theta, z: float, r0: float, H: float, opts: Dict):
    ...
    if strength == 0.0:
        ...
        return float(r0) if th0.shape == () else np.full_like(th0, float(r0), dtype=float)
    ...
    if bool(opts.get("sf_edge_solidify_enable", False)) and isinstance(rf, np.ndarray) and (rf.ndim >= 1) and (rf.size > 1):
        # Parameters
        es_strength = max(0.0, min(1.0, float(opts.get("sf_edge_solidify_strength", 0.7))))
    ...
    if apply_tame:
        ...
        eff = tame_strength if tame_strength > 0.0 else float(opts.get("sf_auto_tame_amount", 0.45))
    ...
    if bool(opts.get("sf_spike_clip_enable", False)) and isinstance(rf, np.ndarray) and (rf.ndim >= 1) and (rf.size > 1):
        arr = np.asarray(rf, dtype=float)
    ...
    if bool(opts.get("sf_spike_mad_enable", False)) and isinstance(rf, np.ndarray) and (rf.ndim >= 1) and (rf.size > 1):
        arr = np.asarray(rf, dtype=float)
        ...
        if bool(opts.get("sf_spike_mad_z_boost_enable", True)):
            ...
            ramp = 0.0 if t_z <= z_start else ((t_z - z_start) / max(1e-6, 1.0 - z_start))
            ramp = ramp ** z_pow
    ...
    if bool(opts.get("sf_peak_snap_enable", False)) and isinstance(rf, np.ndarray) and (rf.ndim >= 1) and (rf.size > 1):
        arr = np.asarray(rf, dtype=float)
    ...
    if edge_sharp > 0.0 and isinstance(rf, np.ndarray) and (rf.ndim >= 1) and (rf.size > 1):
...
def r_outer_harmonic_ripple(theta, z: float, r0: float, H: float, opts: Dict):
    ...
    out = r0 * f
    return float(out) if np.isscalar(theta) else out
def r_outer_lowpoly_facet(theta: npt.ArrayLike | float, z: float, r0: float | npt.NDArray[np.float64], H: float, opts: Dict) -> float | npt.NDArray[np.float64]:
    t = z / H if H > 0 else 0.0
    th = np.asarray(theta, dtype=float)
    ...
    cut_depth_frac = max(0.0, float(opts.get("lp_cut_depth_frac_of_facet", 0.0)))  # 0 disables
    ...
    if tiers >= 1:
        ...
        r0_bot = base_radius(z_bot, H, Rb if Rb > 0 else r0, Rt if Rt > 0 else r0, expn, opts)
        r0_top = base_radius(z_top, H, Rb if Rb > 0 else r0, Rt if Rt > 0 else r0, expn, opts)
        ...
        def _smooth_min(a, b, s):
            ...
            # min(a,b) = -max(-a,-b)
            return -_smooth_max(-np.asarray(a, dtype=float), -np.asarray(b, dtype=float), s)
        ...
        f_dir_current = (1.0 + amp * (tri_s)) if outward_dir else (1.0 - amp * (1.0 - tri_s))
        r_base_local = r0 * f_dir_current
        # Keep an original copy for seam-flattening blends
        r_base_local_orig = r_base_local.copy() if isinstance(r_base_local, np.ndarray) else float(r_base_local)
        ...
        r_base_local_in_orig = r_base_local_in.copy() if isinstance(r_base_local_in, np.ndarray) else float(r_base_local_in)
        ...
        straight_smooth = bool(opts.get("lp_cut_straight_smooth_mode", False)) and enable_straight and has_cut and not uniform_ring
        if (enable_straight or enable_uniform) and has_cut:
            ...
            if bool(opts.get("lp_cut_straight_preserve_facets", False)):
                straight_start = max(straight_start, float(opts.get("lp_cut_straight_preserve_lock_threshold", 0.6)))
                straight_blend_pow = max(straight_blend_pow, float(opts.get("lp_cut_straight_preserve_blend_pow", 2.0)))
            if uniform_ring:
                ...
                if bool(opts.get("lp_uniform_ring_localize", False)):
                    straight_start = max(straight_start, float(opts.get("lp_uniform_ring_lock_threshold", 0.7)))
                    straight_blend_pow = max(straight_blend_pow, float(opts.get("lp_uniform_ring_blend_pow", 2.0)))
            ...
            if not straight_smooth:
                ...
                if cut_bot_deg > 0.0 and (np.any(w_bot > 0.0) if isinstance(w_bot, np.ndarray) else (w_bot > 0.0)):
                    r_base_local = _straight_blend(w_bot, r_base_local_orig, r_uniform_bot_target)
                    r_base_local_in = _straight_blend(w_bot, r_base_local_in_orig, r_uniform_bot_target)
                if cut_top_deg > 0.0 and (np.any(w_top > 0.0) if isinstance(w_top, np.ndarray) else (w_top > 0.0)):
                    r_base_local = _straight_blend(w_top, r_base_local, r_uniform_top_target)
                    r_base_local_in = _straight_blend(w_top, r_base_local_in, r_uniform_top_target)
            ...
            def _apply_plateau_impl(r_vals, weight, depth0, uniform_val, base_guard):
                ...
                if uniform_ring:
                    ...
                    mix_pow = max(1.0, float(opts.get("lp_uniform_ring_blend_pow", 12.0)))
                    ...
                    tentative = np.where(delta >= 0.0, np.minimum(tentative, target_arr), tentative)
                    tentative = np.where(delta < 0.0, np.maximum(tentative, target_arr), tentative)
            ...
            if not uniform_ring and not strict_no_outward and not straight_smooth:
                ...
                lift_gamma = float(opts.get("lp_cut_straight_lift_gamma", 0.7))  # lower => stronger near seam
                ...
                if cut_bot_deg > 0.0 and (np.any(w_bot > 0.0) if isinstance(w_bot, np.ndarray) else (w_bot > 0.0)):
                    r_base_local = _lift_valleys(r_base_local, w_bot, r_uniform_bot_target)
                    r_base_local_in = _lift_valleys(r_base_local_in, w_bot, r_uniform_bot_target)
                if cut_top_deg > 0.0 and (np.any(w_top > 0.0) if isinstance(w_top, np.ndarray) else (w_top > 0.0)):
                    r_base_local = _lift_valleys(r_base_local, w_top, r_uniform_top_target)
                    r_base_local_in = _lift_valleys(r_base_local_in, w_top, r_uniform_top_target)
            ...
            aa_thresh = float(opts.get("lp_cut_straight_aa_thresh", 0.82))  # apply when w >= threshold
            aa_passes = int(max(0, min(3, int(opts.get("lp_cut_straight_aa_passes", 1)))))
        ...
        if dbg_enabled:
            ...
            if np.isscalar(theta):
                ...
                for zc in (z_bot - dz, z_bot + dz, z_top - dz, z_top + dz):
                    r0_mid = base_radius(zc, H, Rb if Rb > 0 else r0, Rt if Rt > 0 else r0, expn, opts)
                    ...
                    if zc <= z_bot:
                        ...
                        f_kc  = _facet_mod_scalar(theta_mid, k)
                        f_kp1 = _facet_mod_scalar(theta_mid, k + 1 if k < (tiers - 1) else k)
                    ...
                    samples.append((float(theta_mid), float(zc), float(r_base_mid), float(Rstart_eff)))
                dbg_sample = samples
            else:
                ...
                for kk in unique_facets[::step]:
                    ...
                    for zc in (z_bot - dz, z_bot + dz, z_top - dz, z_top + dz):
                        r0_mid = base_radius(zc, H, Rb if Rb > 0 else r0, Rt if Rt > 0 else r0, expn, opts)
                        if zc <= z_bot:
                            f_km1 = _facet_mod_scalar(theta_mid, kk - 1 if kk > 0 else kk)
                            f_kc  = _facet_mod_scalar(theta_mid, kk)
                            Rstart_mid = max(r0_mid * f_km1, r0_mid * f_kc)
                        else:
                            f_kc  = _facet_mod_scalar(theta_mid, kk)
                            f_kp1 = _facet_mod_scalar(theta_mid, kk + 1 if kk < (tiers - 1) else kk)
                            Rstart_mid = max(r0_mid * f_kc, r0_mid * f_kp1)
        ...
        r_tmp = r_base
        if has_cut and cut_bot_deg > 0.0 and (np.any(w_bot > 0.0) if isinstance(w_bot, np.ndarray) else (w_bot > 0.0)):
        ...
        if has_cut and cut_top_deg > 0.0 and (np.any(w_top > 0.0) if isinstance(w_top, np.ndarray) else (w_top > 0.0)):
        ...
        if flatten_enabled and (straight_edge or uniform_ring) and has_cut and not straight_smooth:
            if cut_bot_deg > 0.0 and (np.any(w_bot > 0.0) if isinstance(w_bot, np.ndarray) else (w_bot > 0.0)):
                guard_bot = r_base_local_in_orig if cap_to_inward else r_base_local_in_orig
                r_tmp = _apply_plateau(r_tmp, w_bot, depth_bot0, r_uniform_bot_target, guard_bot)
            if cut_top_deg > 0.0 and (np.any(w_top > 0.0) if isinstance(w_top, np.ndarray) else (w_top > 0.0)):
                guard_top = r_base_local_in_orig if cap_to_inward else r_base_local_in_orig
                r_tmp = _apply_plateau(r_tmp, w_top, depth_top0, r_uniform_top_target, guard_top)
        ...
        try:
            ...
            if isinstance(r_tmp, np.ndarray) and bool(opts.get("lp_edge_solidify_enable", False)) and has_cut:
                ...
                passes_es = int(max(1, min(5, int(opts.get("lp_edge_solidify_passes", 2)))) )
                # Build combined proximity weight to either seam plane
                if isinstance(w_bot, np.ndarray):
                    w_any = w_bot.copy()
                else:
                    w_any = np.full_like(r_tmp, float(w_bot) if 'w_bot' in locals() else 0.0)
                if cut_top_deg > 0.0:
                    if isinstance(w_top, np.ndarray):
            ...
            if 'straight_smooth' in locals() and straight_smooth and isinstance(r_tmp, np.ndarray):
                strength = float(opts.get("lp_cut_straight_smooth_strength", 0.65))
                strength = max(0.0, min(1.0, strength))
                spasses = int(max(1, min(4, int(opts.get("lp_cut_straight_smooth_passes", 2)))))
        ...
        if use_outward:
            ...
            if has_cut:
                ...
                if isinstance(r_tmp, np.ndarray):
                    in_seam_band = False
                    if cut_bot_deg > 0.0 and (w_bot_scalar > 0.0 if 'w_bot_scalar' in locals() else (w_bot > 0.0 if 'w_bot' in locals() else False)):
                        in_seam_band = True
                    if cut_top_deg > 0.0 and (w_top_scalar > 0.0 if 'w_top_scalar' in locals() else (w_top > 0.0 if 'w_top' in locals() else False)):
                        in_seam_band = True
                    if in_seam_band:
                        r0_cap = float(r0)
                ...
                rb = _smooth_max(r_tmp, r_req_bot, s_bot) if np.any(w_bot > 0.0) else r_tmp
...
STYLES = {
    ...
    "LowPolyFacet":        (r_outer_lowpoly_facet,       "Faceted polygon look with tiers and beveled edges."),
}
...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
                   expn: float = 1.1, n_theta: int = 64, n_z: int = 32,
                   r_outer_fn: Callable[[np.ndarray | float, float, float, float, dict], np.ndarray | float] | None = None,
                   style_opts: dict | None = None) -> tuple[np.ndarray, np.ndarray, dict]:
    """
    ...
    try:
        ...
        if _has_cuts and H > 0:
            ...
            z_win_raw = float(style_opts.get("lp_cut_z_window_frac", 0.12)) if isinstance(style_opts, dict) else 0.12
            z_win_frac = (z_win_raw * 0.01) if z_win_raw > 1.0 else z_win_raw
            z_win = max(1e-6, z_win_frac * h_tier)
            sampling_boost = int(style_opts.get("lp_seam_sampling_boost", 2)) if isinstance(style_opts, dict) else 2
            ...
            for k in range(1, _tiers):
                ...
                seq = [-offs_edge] + [-v for v in sorted(offs_mid_vals, reverse=True)] + [0.0] + sorted(offs_mid_vals) + [offs_edge]
                for dz in seq:
                    zc = z_seam + dz
                    if (zc > 1e-9) and (zc < H - 1e-9):
                        add_zs.append(zc)
            if add_zs:
                z_outer = np.unique(np.concatenate([z_outer, np.array(add_zs, dtype=float)])).astype(float)
    except Exception:
    ...
    try:
        ...
        if isinstance(sf_style_hint, str) and sf_style_hint.strip():
            ...
            is_blossom = (r_outer_fn == STYLES["SuperformulaBlossom"][0]) or (_fn_name == getattr(STYLES["SuperformulaBlossom"][0], "__name__", ""))
        if is_blossom and isinstance(style_opts, dict) and bool(style_opts.get("sf_edge_flow_reconstruct_enable", False)):
            ...
            try:
                if bool(style_opts.get("sf_edge_flow_debug", False)):
                    ...
                    try:
                        ...
                        payload0 = {"timestamp": time.time(), "event": "entered_edge_flow", "mode": str(style_opts.get('sf_edge_flow_mode'))}
                        # Respect file-write toggle for debug stamp
                        verbose_write_file = bool(style_opts.get('sf_edge_flow_verbose_write_file', True))
                        if verbose_write_file:
                            with open(outpath, "a", encoding="utf-8") as fh0:
                                fh0.write(json.dumps(payload0, ensure_ascii=False))
                                fh0.write("\n")
                            try:
                                print(f"[sf_edge_flow_debug] stamped entry to {outpath}")
                            except Exception:
                                pass
                    except Exception:
                        pass
                ...
                pass
            # Build R[z, th]
            R_raw = np.vstack([np.asarray(row, dtype=float) for row in r_outer_samples_list])
            Z, T = R_raw.shape
            # Parameters (clamped)
            ...
            if twist_comp:
                ...
                try:
                    if probe_enabled and 0 <= probe_zi < Z:
                        ...
                        probe_out['analysis_row_mid'] = float(row_analysis[len(row_analysis)//2])
                        ...
                        mapped_raw_idxs = [int(np.mod(i + inv_shift, T)) for i in range(T)]
                        probe_out['mapped_raw_idxs'] = mapped_raw_idxs
                        # compute thetas for mapped raw indices
                        thetas = ((np.arange(T) * (TAU / float(T))).tolist())
                        probe_out['mapped_raw_thetas'] = [float(thetas[i]) for i in mapped_raw_idxs]
                        # write to debug file and print
                        try:
                            # store probe mapping in the in-memory collector as well
                            edgeflow_verbose_collector.append({"stage": "probe_mapping", **probe_out})
                        except Exception:
                            pass
                        try:
                            print(f"[sf_edge_flow_probe] zi={probe_zi} s_tw={s_tw[probe_zi]} inv_shift={inv_shift} T={T}")
                            print(f"[sf_edge_flow_probe] mapped_raw_idxs sample: {mapped_raw_idxs[:10]} ... {mapped_raw_idxs[-10:]}")
                        except Exception:
                            pass
                except Exception:
                    pass
            ...
            if mode == "quantile":
                ...
                def _quantile_env_vertical(A: np.ndarray, half: int, q: float) -> np.ndarray:
                    stacks = []
                    ...
                    W_sorted = np.sort(W, axis=0)
                    k = int(np.clip(int(np.ceil(q * (2 * half + 1))) - 1, 0, (2 * half + 1) - 1))
                    return W_sorted[k, :, :]
                def _quantile_env_diag(A: np.ndarray, half: int, q: float, sign: int) -> np.ndarray:
                    stacks = []
                    for dz in range(-half, half + 1):
                        z_idx = np.clip(np.arange(Z) + dz, 0, Z - 1)
                        shifted = A[z_idx, :]
                ...
                env_final = np.where(dom == 0, Env_vert, np.where(dom == 1, Env_diap, Env_diam))
            elif mode == "vertical":
                ...
                def _dilate_dir(seed_arr: np.ndarray, steps: int, dtheta_per_dz: int) -> np.ndarray:
                    # Forward along +z
                    S_forw = seed_arr.copy()
                    acc_forw = S_forw.copy()
                    for _ in range(steps):
                        S_forw = np.vstack([S_forw[0:1, :], S_forw[:-1, :]])  # shift from z-1 to z
                        if dtheta_per_dz != 0:
                    ...
                    for _ in range(steps):
                        S_back = np.vstack([S_back[1:, :], S_back[-1:, :]])  # shift from z+1 to z
                        if dtheta_per_dz != 0:
                ...
                def _estimate_shifts(A: np.ndarray, K: int) -> tuple[np.ndarray, np.ndarray]:
                ...
                def _roll_rows(arr: np.ndarray, shifts: np.ndarray, sign: int = -1) -> np.ndarray:
                ...
                def _roll_rows_2d(arr: np.ndarray, shifts: np.ndarray, sign: int = -1) -> np.ndarray:
                    out = np.empty_like(arr)
                    for zi in range(arr.shape[0]):
                        k = int(shifts[zi]) * sign
                ...
                def _dilate_adaptive(seed_arr: np.ndarray, steps: int, s_fwd: np.ndarray, s_bwd: np.ndarray) -> np.ndarray:
                ...
                try:
                    if debug_enabled:
                        ...
                        print(f"[sf_edge_flow_debug] path_mask nonzero count={pm_count}")
                except Exception:
                    pass
                    ...
                    if debug_enabled:
                        ...
                        for szi in sample_z:
                            try:
                                ...
                                nms_row = (row >= np.roll(row, 1)) & (row >= np.roll(row, -1))
                                ...
                                print(f"[sf_edge_flow_debug] sample zi={szi} pm_count={int(np.count_nonzero(pm_row))} ridge_cols={ridge_cols.tolist()} ridge_vals_sample={top_vals}")
                            except Exception:
                                print(f"[sf_edge_flow_debug] sample zi={szi} diagnostic failed")
                except Exception:
                    pass
                ...
                if not np.any(path_mask):
                    ...
                    k = int(np.clip(int(np.ceil(q_hi * (2 * h + 1))) - 1, 0, (2 * h + 1) - 1))
                    ...
                    vdecay = float(style_opts.get("sf_edge_flow_valley_band_decay", 0.0))
                    ...
                    if env_final is None:
                        ...
                        if vband > 0:
                            ...
                            if bool(style_opts.get("sf_edge_flow_valley_lock_enable", True)):
                                # small angular/z softness parameters (kept for optional blending)
                                zhw = int(style_opts.get("sf_edge_flow_valley_z_halfwin", 1))
                                zhw = max(0, min(3, zhw))
                                # debug collection
                                debug_enabled = bool(style_opts.get("sf_edge_flow_debug", False))
                                ...
                                for zi in range(Z):
                                    ...
                                    if not np.any(pm_row):
                                        continue
                                    nms = (row >= np.roll(row, 1)) & (row >= np.roll(row, -1))
                                    ...
                                    if ridge_cols.size < 2:
                                        ...
                                        if idxs.size < 2:
                                            continue
                                        vals = row[idxs]
                                        ksel = min(int(style_opts.get("sf_edge_flow_max_paths", 4)), idxs.size)
                                    ...
                                    try:
                                        if debug_enabled and (zi == 0 or zi == (Z // 2) or zi == (Z - 1)):
                                            pm_count = int(np.count_nonzero(pm_row)) if 'pm_row' in locals() else int(np.count_nonzero(path_mask[zi, :]))
                                            # show a short sample of ridge column indices and their values
                                            try:
                                                ridge_sample_vals = [float(row[int(ii)]) for ii in ridge_cols[:8]]
                                            except Exception:
                                                ridge_sample_vals = []
                                            print(f"[sf_edge_flow_debug] ridge debug zi={zi} pm_count={pm_count} ridge_cols={ridge_cols.tolist()} ridge_vals_sample={ridge_sample_vals}")
                                    except Exception:
                                        pass
                                    ...
                                    shift = int(s_tw[zi]) if ('s_tw' in locals() and s_tw is not None) else 0
                                    ...
                                    if wrap_idxs.size > 0:
                                        ...
                                        th_sorted = np.concatenate([th[k+1:] , th[:k+1] + TAU])
                                        row_sorted = np.concatenate([row_ordered[k+1:], row_ordered[:k+1]])
                                    else:
                                        th_sorted = th.copy()
                                    ...
                                    try:
                                        probe_zi = int(style_opts.get("sf_edge_flow_probe_zi", Z // 2))
                                        if debug_enabled and probe_zi == zi:
                                            # show the integer twist shift used to roll into analysis frame
                                            stw = int(s_tw[zi]) if 's_tw' in locals() else 0
                                            try:
                                                # collect the list of ridge pairs that will be processed for this zi
                                                probe_row = R[zi, :]
                                                probe_pm = path_mask[zi, :]
                                                probe_nms = (probe_row >= np.roll(probe_row, 1)) & (probe_row >= np.roll(probe_row, -1))
                                                probe_ridges = np.where(probe_pm & probe_nms)[0]
                                            except Exception:
                                                probe_ridges = np.array([], dtype=int)
                                            print(f"[sf_edge_flow_probe] zi={zi} s_tw={stw} probe_ridges={probe_ridges.tolist()}")
                                    except Exception:
                                        pass
                                    for a, b in zip(ridge_cols, np.roll(ridge_cols, -1)):
                                        if a == b:
                                            continue
                                        theta_a = float(th[a])
                                        theta_b = float(th[b])
                                        ...
                                        try:
                                            if debug_enabled and (zi == 0 or zi == (Z // 2) or zi == (Z - 1)):
                                                try:
                                                    print(f"[sf_edge_flow_debug] pair zi={zi} a={a} b={b} theta_a={theta_a:.6f} theta_b={theta_b:.6f} d={d:.6f} arc_len={arc_len:.6f}")
                                                except Exception:
                                                    pass
                                        except Exception:
                                            pass
                                        ...
                                        Nf = max(9, int(max(12, (arc_len / TAU) * T * 4)))
                                        fine_th = np.linspace(theta_start + 1e-12, theta_start + arc_len - 1e-12, Nf)
                                        ...
                                        try:
                                            zhw = int(style_opts.get('sf_edge_flow_valley_z_halfwin', 1)) if isinstance(style_opts, dict) else 1
                                            zhw = max(0, min(3, zhw))
                                            ...
                                            for dz in range(-zhw, zhw + 1):
                                                ...
                                                try:
                                                    ...
                                                    cand_angles.append(float(fine_th[idx2]))
                                                except Exception:
                                                    # fall back to a coarse discrete argmin on neighbor
                                                    try:
                                                        idxs2 = np.where((np.arange(T) >= 0))[0]
                                                        if idxs2.size:
                                                            local_idx = int(np.argmin(R[zi2, :]))
                                                            cand_angles.append(float(th[local_idx]))
                                                    except Exception:
                                                        pass
                                            ...
                                            if len(cand_angles) > 1:
                                                ...
                                                ref = theta_val
                                                diffs = np.array([((a - ref + TAU/2.0) % TAU) - (TAU/2.0) for a in cand_angles], dtype=float)
                                                mean_diff = float(np.median(diffs))
                                                theta_val = float((ref + mean_diff) % TAU)
                                        except Exception:
                                        ...
                                        try:
                                            if debug_enabled:
                                                ...
                                                try:
                                                    rmin = float(np.min(R_fine)) if hasattr(R_fine, '__len__') else float(R_fine)
                                                except Exception:
                                                    rmin = float('nan')
                                                try:
                                                    print(f"[sf_edge_flow_debug] sector zi={zi} peaks=({a},{b}) Nf={Nf} theta_val={theta_val:.6f} R_fine_min={rmin:.6f}")
                                                except Exception:
                                                    pass
                                        except Exception:
                                            pass
                                        ...
                                        try:
                                            inv_shift = int(s_tw[zi]) if ('s_tw' in locals() and s_tw is not None) else 0
                                            ...
                                            keep_mask = R_raw[zi, mapped_raw] > drain_thresh
                                            ...
                                            idxs = idxs[keep_mask]
                                            th_idxs = th[idxs]
                                            s_vals = ((th_idxs - theta_start) % TAU) / arc_len
                                        except Exception:
                                        ...
                                        try:
                                            if 'origin_map' in locals():
                                                ...
                                                assign_from = np.where(np.array(s_vals) <= 0.5, int(a), int(b))
                                                origin_map[zi, idxs] = assign_from.astype(int)
                                        except Exception:
                                            pass
                                        ...
                                        if debug_enabled:
                                            debug_reports.append({
                                                ...
                                                "theta_end": float((theta_start + arc_len) % TAU),
                                                "theta_val": float(theta_val),
                                                "r_peak_a": float(r_pa),
                                                "r_peak_b": float(r_pb),
                                                "idxs": idxs.tolist(),
                                                "mapped_raw_idxs": [int(np.mod(int(i - int(s_tw[zi])), T)) for i in idxs.tolist()],
                                                "cur": cur.tolist(),
                                    ...
                                    try:
                                        if debug_enabled:
                                            after_reports_len = len(debug_reports) if debug_reports is not None else 0
                                            added = after_reports_len - before_reports_len
                                            if added > 0:
                                                print(f"[sf_edge_flow_debug] appended {added} debug_reports for zi={zi}")
                                    except Exception:
                                        pass
                                    # end peak pair loop
                                # end zi loop
                                ...
                                if debug_enabled:
                                    if (not debug_reports) or len(debug_reports) == 0:
                                        ...
                                        for zi in range(min(3, Z)):
                                            ...
                                            debug_reports.append({
                                                ...
                                                "theta_a": float(theta_a),
                                                "theta_b": float(theta_b),
                                                "theta_val": float(thetas[idxs[np.argmin(cur)]]),
                                                "r_peak_a": float(r_pa),
                                                "r_peak_b": float(r_pb),
                                                "idxs": idxs.tolist(),
                                                "mapped_raw_idxs": [int(np.mod(int(i - int(s_tw[zi])), T)) for i in idxs.tolist()],
                                    ...
                                    try:
                                        ...
                                        try:
                                            cnt = len(debug_reports) if debug_reports is not None else 0
                                            print(f"[sf_edge_flow_debug] about to write debug summary: reports_count={cnt} outpath={outpath}")
                                            if cnt > 0:
                                                ...
                                                try:
                                                    ...
                                                    za = -1
                                                try:
                                                    pa = int(first.get('peak_a_col', -1))
                                                except Exception:
                                                    pa = -1
                                                try:
                                                    pb = int(first.get('peak_b_col', -1))
                                                except Exception:
                                                    pb = -1
                                                try:
                                                    tv = float(first.get('theta_val', float('nan')))
                                                except Exception:
                                                    tv = float('nan')
                                                try:
                                                    idxs_len = len(first.get('idxs', []))
                                                except Exception:
                                                    idxs_len = 0
                                                print(f"[sf_edge_flow_debug] sample report zi={za} peaks=({pa},{pb}) theta_val={tv:.6f} idxs_len={idxs_len}")
                                        except Exception:
                                            pass
                                        # Build extra diagnostics about ridge/seed/path state to help debugging
                                        try:
                                            seed_count = len(seed_list) if 'seed_list' in locals() and seed_list is not None else 0
                                        except Exception:
                                            seed_count = 0
                                        try:
                                            path_mask_count = int(np.count_nonzero(path_mask)) if 'path_mask' in locals() else 0
                                        except Exception:
                                            path_mask_count = 0
                                        try:
                                            Z_dbg, T_dbg = (int(R.shape[0]), int(R.shape[1])) if 'R' in locals() else (int(Z), int(T))
                                        except Exception:
                                            Z_dbg, T_dbg = (int(Z) if 'Z' in locals() else -1, int(T) if 'T' in locals() else -1)
                                        # per-ring local ridge candidate counts (NMS peaks)
                                        try:
                                            nms_local = (R >= np.roll(R, 1, axis=1)) & (R >= np.roll(R, -1, axis=1))
                                            ridge_counts = [int(x) for x in np.sum(nms_local, axis=1).tolist()]
                                        except Exception:
                                            ridge_counts = []
                                        ...
                                        try:
                                            if debug_reports:
                                                    for r in debug_reports:
                                                        safe_r = {
                                                            'zi': int(r.get('zi', -1)),
                                                            'peak_a_col': int(r.get('peak_a_col', -1)),
                                                            'peak_b_col': int(r.get('peak_b_col', -1)),
                                                            'theta_a': float(r.get('theta_a', float('nan'))),
                                                            'theta_b': float(r.get('theta_b', float('nan'))),
                                                            'theta_val': float(r.get('theta_val', float('nan'))),
                                                            'r_peak_a': float(r.get('r_peak_a', float('nan'))),
                                                            'r_peak_b': float(r.get('r_peak_b', float('nan'))),
                                                            'idxs': list(map(int, r.get('idxs', []))) if r.get('idxs') is not None else [],
                                                            'mapped_raw_idxs': list(map(int, r.get('mapped_raw_idxs', []))) if r.get('mapped_raw_idxs') is not None else [],
                                                            'cur': [float(x) for x in (r.get('cur', []) or [])],
                                                            'B_vals': [float(x) for x in (r.get('B_vals', []) or [])],
                                                            'new': [float(x) for x in (r.get('new', []) or [])]
                                                        }
                                        ...
                                        if (not safe_reports) or len(safe_reports) == 0:
                                            try:
                                                synth_idxs = [0, max(0, Z // 2), max(0, Z - 1)]
                                                for szi in synth_idxs:
                                                    ...
                                                    if peaks.size < 2:
                                                        top2 = np.argsort(row)[-2:]
                                                        peaks = np.sort(top2)
                                                    a = int(peaks[0]) if peaks.size > 0 else 0
                                                    b = int(peaks[1]) if peaks.size > 1 else int((a + 1) % T)
                                                    theta_a = float(thetas[a])
                                                    theta_b = float(thetas[b])
                                                    d = (theta_b - theta_a) % TAU
                                                    if d <= (TAU * 0.5):
                                                    ...
                                                    r_pb = float(row[b])
                                                    B_vals = (1.0 - s_vals) * r_pa + s_vals * r_pb
                                                    cur = Env_ext[szi, idxs].copy() if 'Env_ext' in locals() else np.zeros_like(B_vals)
                                                    newv = np.maximum(cur, B_vals)
                                                    safe_reports.append({
                                                        'zi': int(szi),
                                                        'peak_a_col': int(a),
                                                        ...
                                                        'r_peak_a': float(r_pa),
                                                        'r_peak_b': float(r_pb),
                                                        'idxs': list(map(int, idxs.tolist())),
                                                        'mapped_raw_idxs': [int(np.mod(int(i - int(s_tw[int(szi)])), T)) for i in idxs.tolist()],
                                                        'cur': [float(x) for x in cur.tolist()],
                                                        'B_vals': [float(x) for x in B_vals.tolist()],
                                                        'new': [float(x) for x in newv.tolist()]
                                                    })
                                            except Exception:
                                        ...
                                        try:
                                            print(f"[sf_edge_flow_debug] wrote {outpath} summary: reports_count={payload.get('reports_count', 0)} timestamp={payload.get('timestamp')}")
                                        except Exception:
                                            pass
                                    except Exception:
                                        # best-effort: if file write fails, fallback to stdout
                                        try:
                                            print("[sf_edge_flow_debug] (fallback) per-sector bridge summary: count=", len(debug_reports) if debug_reports is not None else 0)
                                        except Exception:
                                            pass
                                env_final = Env_ext
                            else:
            ...
            if (not pin_to_origin) and anchor_enable and anchor_rad > 0:
                try:
                    ...
                    def _nearest_indices_circular(Tloc: int, peaks_idx: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
            ...
            if valley_only:
                ...
                R_new = np.where(mask_v & (R < Env_to_use), R + amt * (Env_to_use - R), R)
            else:
            ...
            try:
                verbose_diag = bool(style_opts.get('sf_edge_flow_verbose_diagnostics', False))
                verbose_write_file = bool(style_opts.get('sf_edge_flow_verbose_write_file', True))
                if verbose_diag:
                    try:
                        from pathlib import Path
                        ...
                        probe_zi = None
                        try:
                            probe_zi = style_opts.get('sf_edge_flow_probe_zi', None) if isinstance(style_opts, dict) else None
                        except Exception:
                            probe_zi = None
                        ...
                        for zi in rows_to_dump.tolist():
                                ...
                                try:
                                    if 'Env_to_use_raw' in locals():
                                        ...
                                        Env_applied_raw_row = np.asarray(Env_to_use_raw[zi, :], dtype=float)
                                except Exception:
                                    Env_applied_raw_row = None
                                ...
                                try:
                                    if (Env_applied_raw_row is not None) and ('R_new_raw' in locals()):
                                        mask = np.asarray(R_new_raw[zi, :]) < Env_applied_raw_row
                                ...
                                row_entry = {
                                'zi': int(zi),
                                'z': float(z_outer[zi]) if 'z_outer' in locals() else float(zi),
                                'min_final_raw': float(min_per_row[zi]),
                                    'R_raw_sample': np.asarray(R_raw[zi, :]).tolist() if 'R_raw' in locals() else None,
                                    'R_analysis_sample': np.asarray(R[zi, :]).tolist() if 'R' in locals() else None,
                                    'Env_sample': np.asarray(Env[zi, :]).tolist() if 'Env' in locals() else None,
                                    'Env_to_use_sample': np.asarray(Env_to_use[zi, :]).tolist() if 'Env_to_use' in locals() else None,
                                    'Env_applied_raw_sample': Env_applied_raw_row.tolist() if (Env_applied_raw_row is not None) else None,
                                    # Env_to_use_raw_post is the envelope aligned to raw-theta after any deoffset
                                    'Env_to_use_raw_post': (np.roll(Env_to_use_raw[zi, :], -int(s0), axis=0).tolist() if (('Env_to_use_raw' in locals() and 's0' in locals())) else (np.asarray(Env_to_use[zi, :]).tolist() if 'Env_to_use' in locals() else None)),
                                    'origin_map_sample': np.asarray(origin_map[zi, :]).tolist() if 'origin_map' in locals() else None,
                                    'R_new_sample': np.asarray(R_new[zi, :]).tolist() if 'R_new' in locals() else None,
                                    'R_new_raw_sample': np.asarray(R_new_raw[zi, :]).tolist() if 'R_new_raw' in locals() else None,
                                ...
                                try:
                                    T_loc = int(R_new_raw.shape[1]) if ('R_new_raw' in locals() and hasattr(R_new_raw, 'shape')) else None
                                except Exception:
                                    T_loc = None
                                theta_sample = None
                                if T_loc is not None and T_loc > 0:
                                    try:
                                        theta_sample = (np.arange(T_loc) * (TAU / float(T_loc))).tolist()
                                    except Exception:
                                        theta_sample = None
                                ...
                                try:
                                    if 'Gt' in locals():
                                        ...
                                        Gt_stats = {'min': float(gtr.min()), 'max': float(gtr.max()), 'mean': float(gtr.mean())}
                                except Exception:
                                    Gt_stats = None
                                try:
                                    if 'Gz' in locals():
                                        gzr = np.asarray(Gz[zi, :], dtype=float)
                                        Gz_stats = {'min': float(gzr.min()), 'max': float(gzr.max()), 'mean': float(gzr.mean())}
                                except Exception:
                                    Gz_stats = None
                                try:
                                    if 'dom' in locals():
                                        ...
                                        dom_counts = {'vert': int(cnts[0]), 'diap': int(cnts[1]), 'diam': int(cnts[2])}
                                except Exception:
                                    dom_counts = None
                                ...
                                canonical_row = {
                                    ...
                                    'Env_to_use_raw_post': row_entry.get('Env_to_use_raw_post') if row_entry.get('Env_to_use_raw_post') is not None else row_entry.get('Env_applied_raw_sample'),
                                    ...
                                    'enforcement_violations_count': row_entry.get('enforcement_violations_count'),
                                    'enforcement_violations_indices': row_entry.get('enforcement_violations_indices'),
                                ...
                                try:
                                    edgeflow_verbose_collector.append({'timestamp': dump.get('timestamp'), 'rows': [canonical_row]})
                                except Exception:
                                    pass
                        ...
                        if verbose_write_file:
                            with open(outpath, 'a', encoding='utf-8') as fh:
                                ...
                                fh.write('\n')
                            try:
                                print(f"[sf_edge_flow_debug] verbose diagnostic wrote {len(rows_to_dump)} rows to {outpath}")
                            except Exception:
                                pass
                    except Exception:
                        pass
            ...
            if bool(style_opts.get("sf_edge_flow_auto_deoffset", True)):
                try:
                    kmax = int(style_opts.get("sf_edge_flow_deoffset_max", max(1, theta_snap)))
                    kmax = max(0, min(3, kmax))
                    if kmax > 0:
                        # Estimate per-ring shift aligning R_new_raw to original R_raw using lifted-area correlation
                        def _best_shift(row_new: np.ndarray, row_old: np.ndarray, mask: np.ndarray, K: int) -> int:
                        ...
                        for zi in range(Z):
                            ...
                            shifts.append(_best_shift(row_new, row_old, lift_mask, kmax))
                        if shifts:
                    ...
                    try:
                        ...
                        if verbose_diag:
                            try:
                                ...
                                dump = {'timestamp': time.time(), 'stage': 'post_deoffset', 'rows': []}
                                # Attempt to capture deoffset decision details if available
                                dump['deoffset'] = {}
                                if 'shifts' in locals():
                                    try:
                                        dump['deoffset']['shifts'] = np.asarray(shifts).tolist()
                                    except Exception:
                                ...
                                for zi in rows_to_dump.tolist():
                                    dump['rows'].append({
                                        ...
                                        'Env_to_use_sample': np.asarray(Env_to_use[zi, :]).tolist() if 'Env_to_use' in locals() else None,
                                        'origin_map_sample': np.asarray(origin_map[zi, :]).tolist() if 'origin_map' in locals() else None,
                                        'R_new_sample_pre': np.asarray(R_new[zi, :]).tolist() if 'R_new' in locals() else None,
                                        'R_new_raw_sample_pre': np.asarray(R_new_raw_pre[zi, :]).tolist() if 'R_new_raw_pre' in locals() else None,
                                        'R_new_raw_sample_post': np.asarray(R_new_raw[zi, :]).tolist() if 'R_new_raw' in locals() else None,
                                    })
                                # append the JSON line
                                    if verbose_write_file:
                                        with open(outpath, 'a', encoding='utf-8') as fh:
                                            ...
                                            fh.write('\n')
                                        try:
                                            print(f"[sf_edge_flow_debug] post-deoffset verbose diagnostic wrote {len(rows_to_dump)} rows to {outpath}")
                                        except Exception:
                                            pass
                                    try:
                                        ...
                                        for r in dump.get('rows', []):
                                            canonical_rows.append({
                                                    'zi': r.get('zi'),
                                                    'z': r.get('z'),
                                                    'min_final_raw_pre': r.get('min_final_raw_pre'),
                                                    'min_final_raw_post': r.get('min_final_raw_post'),
                                                    'R_raw_sample': r.get('R_raw_sample'),
                                                    'R_analysis_sample': r.get('R_analysis_sample'),
                                                    'Env_sample': r.get('Env_sample'),
                                                    'Env_to_use_sample': r.get('Env_to_use_sample'),
                                                    'origin_map_sample': r.get('origin_map_sample'),
                                                    'R_new_sample_pre': r.get('R_new_sample_pre'),
                                                    'R_new_raw_sample_pre': r.get('R_new_raw_sample_pre'),
                                                    'R_new_raw_sample_post': r.get('R_new_raw_sample_post'),
                                                    'theta_sample': None,
                                                    'lift_delta': None,
                                                    'valley_mask': None,
                                                    'shifts': dump.get('deoffset', {}).get('shifts') if isinstance(dump.get('deoffset', {}).get('shifts'), list) else None,
                                                    's0': dump.get('deoffset', {}).get('s0') if isinstance(dump.get('deoffset', {}).get('s0'), int) else None,
                                                })
                                        edgeflow_verbose_collector.append({'timestamp': dump.get('timestamp'), 'stage': dump.get('stage'), 'deoffset': dump.get('deoffset'), 'rows': canonical_rows})
                                    except Exception:
                                        pass
                            except Exception:
                                pass
            ...
            try:
                if 'Env_to_use_raw' in locals() and 'R_new_raw' in locals():
                    ...
                    try:
                        ...
                        per_row_changes = np.count_nonzero(diffs > 1e-12, axis=1).tolist()
                        ...
                        total_changes = None
                    if bool(style_opts.get('sf_edge_flow_debug', False)):
                        print(f"[sf_edge_flow_debug] final envelope enforcement applied (deoff={deoff}) - total_changes={total_changes}")
                    ...
                    if debug_flag:
                        ...
                        if env_post is not None:
                            ...
                            if n_viol > 0:
                                ...
                                sample_coords = [ (int(r), int(c)) for r, c in viol_idx[:8] ]
    ...
    try:
        r_outer_samples = np.vstack(r_outer_samples_list)  # shape (len(z_outer), n_theta)
        ...
        if isinstance(sf_style_hint_local, str) and sf_style_hint_local.strip():
            ...
            is_geom_style = (
                (r_outer_fn == STYLES["LowPolyFacet"][0]) or (_fn_name == getattr(STYLES["LowPolyFacet"][0], "__name__", "")) or
                (r_outer_fn == STYLES["SuperformulaBlossom"][0]) or (_fn_name == getattr(STYLES["SuperformulaBlossom"][0], "__name__", ""))
            )
        if is_geom_style:
            use_alt = (qB > qA)
        ...
        try:
            tiers_cfg = int(style_opts.get("lp_tiers", 1)) if isinstance(style_opts, dict) else 1
            cut_b = float(style_opts.get("lp_cut_bot_deg", 0.0)) if isinstance(style_opts, dict) else 0.0
            cut_t = float(style_opts.get("lp_cut_top_deg", 0.0)) if isinstance(style_opts, dict) else 0.0
            # Accept both LP and Blossom diagonal smoothing toggles
            passes_diag = 0
            if isinstance(style_opts, dict):
                pd_lp = int(max(0, min(4, int(style_opts.get("lp_diagonal_smooth_passes", 0)))))
                pd_sf = int(max(0, min(4, int(style_opts.get("sf_diagonal_smooth_passes", 0)))))
                passes_diag = max(pd_lp, pd_sf)
            if passes_diag > 0 and (tiers_cfg > 1) and ((cut_b > 0.0) or (cut_t > 0.0)) and H > 0:
        ...
        try:
            ...
            if has_cuts and H > 0:
                ...
                z_win_frac = (z_win_raw * 0.01) if z_win_raw > 1.0 else z_win_raw
                z_win = max(1e-6, z_win_frac * h_tier)
                lock_strength = float(style_opts.get("lp_seam_lock_strength", 1.0)) if isinstance(style_opts, dict) else 1.0
                ...
                for zs in seam_zs:
                    ...
                    if is_geom_style:
                        ...
                        d_diag_00_11 = np.abs(r00 - r11)
                        d_diag_01_10 = np.abs(r01 - r10)
                        pref_alt = (float(np.sum(d_diag_01_10[band_mask])) < float(np.sum(d_diag_00_11[band_mask])))
    ...
    try:
        if 'edgeflow_verbose_collector' in locals() and isinstance(edgeflow_verbose_collector, list) and len(edgeflow_verbose_collector) > 0:
...
def save_preview_png(path, H: float, Rt: float, Rb: float, expn: float,
    ...
    for i, z in enumerate(zs):
        r0 = base_radius(z, H, Rb, Rt, expn, style_opts if isinstance(style_opts, dict) else {})
        twist = _spin_twist_radians(z, H, style_opts if isinstance(style_opts, dict) else {})
        cTw, sTw = float(np.cos(twist)), float(np.sin(twist))
