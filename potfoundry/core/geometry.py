...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    v10 = outer_idx[1:, :][:, j]
    v11 = outer_idx[1:, :][:, jn]
    # Winding chosen so face normals point outward (+radial) for export-grade
    # meshes (Rhino/Grasshopper require unified, outward normals). See
    # potfoundry.core.mesh_quality.orient_outward for the reference algorithm.
    ...
    vi10 = inner_idx[1:, :][:, j]
    vi11 = inner_idx[1:, :][:, jn]
    # Inner wall normals point inward (toward the cavity), i.e. outward from
    # the solid material — reversed relative to the outer wall.
    ...
    # Rim cap normals point up (+Z), outward from the top of the solid.
    ...
    # Bottom underside normals point down (-Z), outward from the base.
