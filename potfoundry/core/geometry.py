...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    v10 = outer_idx[1:, :][:, j]
    v11 = outer_idx[1:, :][:, jn]
    # Outward winding: normals point radially away from the central axis.
    ...
    # Vectorized faces for inner wall: outward normal points toward the axis
    # (the cavity is empty), i.e. the reverse of the outer-wall winding.
    ...
    # Rim cap (top annulus): outward normal points up (+z).
    ...
    # Bottom underside: outward normal points down (-z).
