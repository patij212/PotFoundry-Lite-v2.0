...
def compute_tier_phase(tier_idx: int, jitter_amt: float, facets: int) -> float:
...
def compute_triangle_wave(
    theta: npt.NDArray[np.float64], total_phase: float, facets: int
...
def apply_bevel(
    tri: npt.NDArray[np.float64], bevel: float
...
def compute_modulation_factor(
    tri_s: npt.NDArray[np.float64], amp: float, outward_dir: bool
