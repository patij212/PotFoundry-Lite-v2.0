...
def apply_lift_valleys_antialiasing(
    ...
    try:
        ...
        lift_strength = max(
            0.0, min(1.0, float(opts.get("lp_lift_valleys_strength", 0.5)))
        )
        ...
        if lift_passes > 0 and lift_strength > 0.0:
            ...
            if np.any(w_any > 0.0):
                ...
                for _ in range(lift_passes):
                    ...
                    arr = (1.0 - lift_strength * blend) * arr + (
                        lift_strength * blend
                    ) * lifted
        ...
        if med3_passes > 0 and med3_strength > 0.0:
            ...
            if np.any(w_any > 0.0):
                ...
                for _ in range(med3_passes):
                    ...
                    blend = np.power(np.clip(w_any, 0.0, 1.0), 1.2)
                    arr = (1.0 - med3_strength * blend) * arr + (
                        med3_strength * blend
                    ) * smoothed
                    arr = np.minimum(arr, base_guard)
        ...
        if med5_passes > 0 and med5_strength > 0.0:
            ...
            if np.any(w_any > 0.0):
                ...
                for _ in range(med5_passes):
                    smoothed = med5(arr)
                    blend = np.power(np.clip(w_any, 0.0, 1.0), 1.2)
                    arr = (1.0 - med5_strength * blend) * arr + (
                        med5_strength * blend
                    ) * smoothed
                    arr = np.minimum(arr, base_guard)
        ...
        if avg3_passes > 0 and avg3_strength > 0.0:
            ...
            if np.any(w_any > 0.0):
                ...
                for _ in range(avg3_passes):
                    ...
                    blend = np.power(np.clip(w_any, 0.0, 1.0), 1.2)
                    arr = (1.0 - avg3_strength * blend) * arr + (
                        avg3_strength * blend
                    ) * reduced
                    arr = np.minimum(arr, base_guard)
...
def apply_outward_mode(
    ...
    if has_cut:
        ...
        rb = (
            smooth_max_func(r_vals, r_req_bot, s_bot) if np.any(w_bot > 0.0) else r_vals
        )
...
def apply_uniform_ring_guard(
    r_vals: float | NDArrayFloat, r_base_local_in_orig: Any, uniform_ring: bool
) -> float | NDArrayFloat:
