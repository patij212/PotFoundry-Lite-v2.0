...
def vectorized_vertex_generation(
    ...
    r0_array = np.array(
        [base_radius_fn(float(z), H, Rb, Rt, expn, style_opts) for z in z_array],
        dtype=np.float64,
    )

    twist_array = np.array(
        [spin_twist_fn(float(z), H, style_opts) for z in z_array], dtype=np.float64
    )
...
def build_vertices_vectorized(
    ...
    cx = (
        cos_th[np.newaxis, :] * cTw[:, np.newaxis]
        - sin_th[np.newaxis, :] * sTw[:, np.newaxis]
    )
    sy = (
        sin_th[np.newaxis, :] * cTw[:, np.newaxis]
        + cos_th[np.newaxis, :] * sTw[:, np.newaxis]
    )
...
def accelerated_build_pot_mesh(
    ...
    twist_inner = np.array(
        [spin_twist_fn(float(z), H, style_opts) for z in z_inner], dtype=np.float64
    )
    ...
    # Outer wall faces
    outer_faces = build_faces_vectorized(
        n_z + 1, n_theta, outer_offset=0, reverse_winding=False
    )

    # Inner wall faces
    inner_faces = build_faces_vectorized(
        n_z + 1, n_theta, outer_offset=n_outer, reverse_winding=True
    )
    ...
    est_bottom_od = 2.0 * float(
        np.max(np.linalg.norm(outer_bottom_verts[:, :2], axis=1))
    )
