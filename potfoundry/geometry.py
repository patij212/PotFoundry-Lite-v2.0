...
def superformula_r(theta: float, m: float, n1: float, n2: float, n3: float,
...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    jn = (j + 1) % n_theta
    # Winding convention: triangles are emitted so normals point OUTWARD from
    # the solid shell (consistent, positive-volume closed manifold) for clean
    # CAD import (Rhino/Grasshopper) and slicing. Outer wall, inner wall, rim,
    # and bottom-underside patches are reversed from their natural (inward)
    # winding; top-of-slab and drain-cylinder are already outward.
    # See tests/test_mesh_orientation.py for the enforced invariants.
    for i in range(rows):
        v00 = outer_idx[i, j]; v01 = outer_idx[i, jn]
        v10 = outer_idx[i+1, j]; v11 = outer_idx[i+1, jn]
        faces.extend(list(zip(v11, v10, v00)))
        faces.extend(list(zip(v01, v11, v00)))
    ...
    for i in range(rows_in):
        ...
        faces.extend(list(zip(v10, v11, v00)))
        faces.extend(list(zip(v11, v01, v00)))

    # ---- Rim cap
    outer_top = outer_idx[-1]; inner_top = inner_idx[-1]
    v00 = outer_top[j]; v01 = outer_top[jn]
    vi0 = inner_top[j]; vi1 = inner_top[jn]
    faces.extend(list(zip(vi1, vi0, v00)))
    faces.extend(list(zip(v01, vi1, v00)))
    ...
    faces.extend(list(zip(vd0, vd1, v00)))
    faces.extend(list(zip(vd1, v01, v00)))
