...
def build_pot_mesh(H: float, Rt: float, Rb: float, t_wall: float, t_bottom: float, r_drain: float,
    ...
    for i in range(rows):
        ...
        # Wind so the exterior-wall normal points away from the Z axis (outward).
    ...
    for i in range(rows_in):
        ...
        # Wind so the cavity-wall normal points toward the Z axis (outward of
        # the solid material, into the empty interior).
    ...
    # Wind so the rim (top annulus) normal points up (+Z, outward).
    ...
    # Wind so the underside (base disc at z=0) normal points down (-Z, outward).
