...
def validate_preview_modules():
    ...
    for module_name, max_loc in modules.items():
        ...
        print(
            f"{status} {module_name:<35} {code:>4} LOC  "
...
def validate_main_file():
    ...
    print(
        f"  C/LOC Ratio:    {ratio:.3f} {check_mark(complexity_ok)} (target: ≤{target_ratio})"
    )
...
def validate_structure():
    ...
    checks.append(
        ("Preview package exists", Path("pfui/tabs/interactive/preview").is_dir())
    )

    checks.append(
        ("Sidebar package exists", Path("pfui/tabs/interactive/sidebar").is_dir())
    )
