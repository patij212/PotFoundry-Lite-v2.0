...
def main() -> None:
    ...
    for zi in zi_list:
        ...
        verts, faces, diag = build_pot_mesh(H, 70.0, 45.0, 3.0, 3.0, r_drain,
                                            expn=1.1, n_theta=n_theta, n_z=n_z, style_opts=style_opts)
        out_lines.append({'zi': zi, 'diag': diag})
        print(f"Completed zi={zi}; diagnostics keys: {list(diag.keys())}")

    print('Done running probes; verbose JSONL should contain entries for these zi values.')
