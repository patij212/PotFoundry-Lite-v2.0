...
def plot_row(row, outpath: Path, title: str = ''):
    ...
    env_post = np.asarray(row.get('Env_to_use_raw_post') or row.get('Env_to_use_sample') or [], dtype=float)
    ...
    if violations:
        ...
        plt.scatter(thetas[vi], r_new_raw[vi], color='red', marker='x', s=60, label='violations')
