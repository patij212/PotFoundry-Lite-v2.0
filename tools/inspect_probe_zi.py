...
def _as_seq(x):
    if x is None:
        return None
    # Try to coerce numpy arrays or other sequences to a Python list for safe indexing/len
    try:
        return list(x)
    except Exception:
        return None
...
for i in range(n):
    ...
    rnew_raw_val = R_new_raw[i] if (R_new_raw is not None and i < len(R_new_raw)) else None
    ...
    try:
        if float(rnew_raw_val) < float(er) - 1e-9:
            ...
            rans_val = R_analysis[i] if (R_analysis is not None and i < len(R_analysis)) else None
            rnew_val = R_new[i] if (R_new is not None and i < len(R_new)) else None
            om_val = origin_map[i] if (origin_map is not None and i < len(origin_map)) else None
...
if not viol:
    print("No violations: all final raw radii >= envelope_to_use (using Env_to_use_raw_post if available)")
else:
    print(f"Found {len(viol)} violating theta columns (final_raw < Env_to_use). Showing up to 50:")
    def _fmt(v):
        try:
            return f"{float(v):.6f}"
        except Exception:
    ...
    for idx, rraw, rans, envv, rnew, rnew_raw, om in viol[:50]:
        print(f"idx={idx:3d} raw={_fmt(rraw)} analysis={_fmt(rans)} env_to_use={_fmt(envv)} R_new={_fmt(rnew)} R_new_raw={_fmt(rnew_raw)} origin_map={om}")
