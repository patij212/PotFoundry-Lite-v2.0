...
rules = [
    ...
    (
        re.compile(r"cast\(Optional\[float\],\s*cast\(Any,\s*(ss\.get\([^)]*\))\)\)"),
        r"cast(Any, \1)",
    ),
    (
        re.compile(r"cast\(Optional\[int\],\s*cast\(Any,\s*(ss\.get\([^)]*\))\)\)"),
        r"cast(Any, \1)",
    ),
    ...
    (
        re.compile(
            r"cast\(Optional\[bytes\],\s*cast\(Optional\[bytes\],\s*(ss\.get\([\s\S]*?\))\s*\)\s*\)",
            flags=re.DOTALL,
        ),
        r"cast(Optional[bytes], \1)",
    ),
    (
        re.compile(
            r"cast\(Optional\[dict\],\s*cast\(Optional\[dict\],\s*(ss\.get\([\s\S]*?\))\s*\)\s*\)",
            flags=re.DOTALL,
        ),
        r"cast(Optional[dict], \1)",
    ),
    # Optional bytes/dict outer wrapping cast(Any, ss.get(...)) -> collapse to Optional[...] around inner ss.get
    (
        re.compile(r"cast\(Optional\[bytes\],\s*cast\(Any,\s*(ss\.get\([^)]*\))\)\)"),
        r"cast(Optional[bytes], \1)",
    ),
    (
        re.compile(r"cast\(Optional\[dict\],\s*cast\(Any,\s*(ss\.get\([^)]*\))\)\)"),
        r"cast(Optional[dict], \1)",
    ),
]
