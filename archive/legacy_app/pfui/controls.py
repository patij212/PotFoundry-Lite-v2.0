...
def _render_control(style: str, key: str, meta: dict[str, Any]) -> object:
    ...
    if mtype == "select":
        ...
        default_choice = (
            value if value in options else (options[0] if options else None)
        )
        try:
...
def twist_controls(style: str) -> dict[str, Any]:
    ...
    spin_turns = float(
        c1.slider(
            "Twist turns (negative = left, positive = right)",
            -3.0,
            3.0,
            v_turns,
            0.05,
    ...
    return {
        ...
        "spin_phase_deg": spin_phase,
        "spin_curve_exp": spin_curve,
    }
