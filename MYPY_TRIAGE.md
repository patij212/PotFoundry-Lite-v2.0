...
- [2025-10-30] Micro-fix: app.py — Export/Publish sweep: converted remaining `ss.get(...)` numeric hotspots to use `_to_float_scalar` / `_to_int_scalar`; focused mypy (app.py): Success.
 ...
 - [2025-10-30] Micro-fix: app.py — Preview & Appearance sweep: coerced `preview_res_scale`, `preview_dpi`, and mesh lighting slider defaults to use `_to_float_scalar` / `_to_int_scalar`; focused mypy (app.py): Success. Focused pytest: `tests/test_preview_snapshot.py` passed.
 - [2025-10-30] Micro-fix: app.py — Dimensions & Export prep: converted session-derived dimension defaults (`H`, `top_od`, `bottom_od`, `t_wall`, `t_bottom`, `r_drain`) to `_to_float_scalar` to centralize numeric coercion; focused mypy (app.py): Success. Focused pytest: `tests/test_preview_snapshot.py` passed.
 - [2025-10-30] Micro-fix: app.py — Style params sweep: coerced `expn` and dynamic style params (`k1`..`k5`) to `_to_float_scalar` to ensure numeric stability when passing to geometry; focused mypy (app.py): Success. Focused pytest: `tests/test_preview_snapshot.py` passed.
 - [2025-10-30] Integration test: `tests/test_integration_binary_stl.py` — ran end-to-end export tests to validate binary STL export; result: 4 tests passed. No regressions observed.
 - [2025-10-30] Tests: added `tests/test_preview_full_resolution.py` with stricter checks for `make_preview_arrays` and `render_preview_png_cached` (verifies exact full-resolution shapes, dtype, and finiteness; ensures PNG bytes returned). Ran locally: 2 tests passed.
