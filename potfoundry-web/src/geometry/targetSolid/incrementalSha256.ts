/** Browser-safe incremental SHA-256 used for large final-artifact proof streams. */
...
export class IncrementalSha256 {
  ...
  update(data: Uint8Array, offset = 0, length = data.byteLength - offset): this {
    ...
    if (
      ...
      throw new RangeError('SHA-256 update range is outside the supplied byte view');
    }
    if (length === 0) return this;
    if (this.bytesHashed + length > Number.MAX_SAFE_INTEGER) {
      throw new RangeError('SHA-256 input exceeds the exact JavaScript byte-count range');
    }
    ...
    if (this.pendingLength > 0) {
      const take = Math.min(64 - this.pendingLength, end - position);
      for (let index = 0; index < take; index += 1) {
        this.pending[this.pendingLength + index] = data[position + index];
      }
    ...
    while (position + 64 <= end) {
      this.compress(data, position);
      position += 64;
    }
    if (position < end) {
      this.pendingLength = end - position;
      for (let index = 0; index < this.pendingLength; index += 1) {
        this.pending[index] = data[position + index];
      }
    }
    return this;
  }
  ...
  digest(): Uint8Array {
    ...
    let bitLength = BigInt(this.bytesHashed) * 8n;
    for (let index = 63; index >= 56; index -= 1) {
      this.pending[index] = Number(bitLength & 0xffn);
      bitLength >>= 8n;
    }
    this.compress(this.pending, 0);
  ...
  private compress(bytes: Uint8Array, offset: number): void {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + sum1 + choice + ROUND_CONSTANTS[index] + w[index]) >>> 0;
