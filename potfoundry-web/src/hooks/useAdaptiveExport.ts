    ...
    useEffect(() => {
        ...
        const initGPU = async () => {
            try {
                ...
                if (!styleIdVal) {
                    throw new Error(
                        `[style-runtime] Unknown style '${style.name}'; adaptive shader initialization never falls back`
                    );
                }
                ...
                if (!functionName) {
                    throw new Error(
                        `[style-runtime] ${styleIdVal} has no adaptive shader function for registry index ${styleIndex}`
                    );
                }
...
`;
    ...
    }, [style.name]); // Re-init when style (shader) changes (debounced)
