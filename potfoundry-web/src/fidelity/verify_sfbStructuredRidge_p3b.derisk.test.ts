/**
 * verify_sfbStructuredRidge_p3b.derisk.test.ts — PHASE 3b: close the seam WITHOUT
 * periodic-corridor surgery, by ROTATING the parametrization so the u-seam falls in a
 * feature-FREE gap. Then every petal ridge/valley is INTERIOR ⇒ the proven corridor
 * covers them all (no seam-wrapping loops). The geometry is identical (a relabel):
 *   build_u = (true_u − gapU) mod 1,  surface(build_u,t) = trueSurface((build_u+gapU)%1, t)
 * Picks gapU = the u maximally far from every feature point, builds the shifted corridor,
 * flank-flips (ridge-locked) + reorients, re-evaluates EXACTLY. Reports seam clearance,
 * coverage, allFollowed, watertight, %<20. Writes STL. Pure CPU, PF_DERISK.
 */
...
function splitAtSeam(pts: UTPoint[]): UTPoint[][] { const out: UTPoint[][] = []; let cur: UTPoint[] = [pts[0]]; for (let i = 1; i < pts.length; i++) { if (Math.abs(pts[i].u - pts[i - 1].u) > 0.5) { out.push(cur); cur = [pts[i]]; } else cur.push(pts[i]); } out.push(cur); return out; }

describe.skipIf(!process.env.PF_DERISK)('SFB structured-ridge PHASE 3b — seam-in-gap (full coverage, no periodic surgery)', () => {
  it('rotates the seam into a feature-free gap; corridor covers ALL ridges', () => {
    ...
    const curves = graph.lines.filter((l) => l.kind === 'general-curve').map((c) => c.points.map((p) => ({ u: wrap1(p.u), t: p.t })));

    // pick gapU = candidate u maximally far from every feature point (max-min periodic dist).
    const allU: number[] = []; for (const c of curves) for (const p of c) allU.push(p.u);
    let gapU = 0, bestD = -1;
    for (let k = 0; k < 1440; k++) { const cu = k / 1440; let mn = 1; for (const u of allU) { const a = Math.abs(cu - u); const d = Math.min(a, 1 - a); if (d < mn) mn = d; } if (mn > bestD) { bestD = mn; gapU = cu; } }

    // build-frame surface = true surface rotated so build_u=0 ↔ true_u=gapU.
    const trueSampler = styleSampler('SuperformulaBlossom', { sf_strength: 1 }, STYLE_DIMS);
    const shiftedSampler: SurfaceSampler = { position: (u, t) => trueSampler.position(wrap1(u + gapU), t) };
    const radiusFn = STYLE_FUNCTIONS['SuperformulaBlossom'];
    const opts = { ...DEFAULT_STYLE_PARAMS['SuperformulaBlossom'], sf_strength: 1 };
    const exactPosB = (bu: number, t: number): V3 => { const u = wrap1(bu + gapU), z = t * H, r0 = baseRadius(z, H, R0, R0, 1, opts), th = 2 * Math.PI * u, r = radiusFn(th, z, r0, H, opts); return [r * Math.cos(th), r * Math.sin(th), z]; };
    const analyticN = (bu: number, t: number): V3 => { const h = 1e-3, tu = Math.min(0.999, Math.max(0.001, t)); let n = nrm(cross(sub(exactPosB(bu + h, tu), exactPosB(bu - h, tu)), sub(exactPosB(bu, Math.min(1, tu + h)), exactPosB(bu, Math.max(0, tu - h))))); const u = wrap1(bu + gapU), th = 2 * Math.PI * u; if (n[0] * Math.cos(th) + n[1] * Math.sin(th) < 0) n = [-n[0], -n[1], -n[2]]; return n; };

    // shifted features: build_u = (true_u − gapU) mod 1; split at the (now empty) seam; widen t.
    const specsPts: UTPoint[][] = []; let kept = 0;
    for (const c of curves) {
      if (c.length < 6) continue;
      for (const piece of splitAtSeam(c.map((p) => ({ u: wrap1(p.u - gapU), t: p.t })))) {
        let tMin = 1, tMax = 0; for (const p of piece) { tMin = Math.min(tMin, p.t); tMax = Math.max(tMax, p.t); }
        if (tMax - tMin < 0.35) continue;
        const s = piece.filter((p) => p.t >= 0.05 && p.t <= 0.95);
        if (s.length >= 4) { specsPts.push(s); kept++; }
      }
    }
    const featLines: FeatureLine[] = specsPts.map((c, i) => ({ kind: 'general-curve', label: `c${i}`, points: c.map((p) => ({ u: p.u, t: p.t })) }));
    const specs: MultiFeatureSpec[] = specsPts.map((polyline) => ({ polyline }));

    let co: ReturnType<typeof realFeatureCorridorPerLoop> | null = null; let crash = '';
    try { co = realFeatureCorridorPerLoop(shiftedSampler, specs, { featureLevel: FL, widthMm: 3, dims: DIMS, assemblyFeatureLines: featLines, baseOptions: { maxSagMm: 0.05, maxEdgeMm: 1, minEdgeMm: 0.1, gradeRatio: 2, maxLevel: 12, resU: 128, resT: 128, nRing: 1 << FL, targetTriangles: 8_000_000, budgetMode: 'cap', uBias: 2 } }); }
    catch (e) { crash = String((e as Error).message).slice(0, 160); }
    // eslint-disable-next-line no-console
    console.log(`[P3b] gapU=${gapU.toFixed(4)} seam-clearance=${bestD.toFixed(4)} (band≈0.01) | curves=${curves.length} → pieces=${kept}${crash ? ` | CRASH: ${crash}` : ''}`);
    expect(crash).toBe('');
    if (!co) return;

    const UT = co.merged.vertexUT; const nV = UT.length;
    const P: V3[] = UT.map(([u, t]) => exactPosB(u, t));
    ...
    const eset = new Set<number>(); for (const [k] of edgeC) eset.add(k); let ridgeKept = 0; for (const k of ridgeLock) if (eset.has(k)) ridgeKept++;

    const buf = Buffer.alloc(80 + 4 + nT * 50); buf.write('SFB sf1 structured-ridge P3b (seam-in-gap)', 0, 'ascii'); buf.writeUInt32LE(nT, 80); let off = 84;
    ...
    fs.writeFileSync(path.join(OUT, 'SuperformulaBlossom_sf1_structured_p3b.stl'), buf);

    /* eslint-disable no-console */
    console.log(`[P3b RESULT] loops=${co.hole.loops.length} tris=${nT} | outer %<20=${(100 * lt20 / outer).toFixed(2)} %<5=${(100 * lt5 / outer).toFixed(2)} | bnd=${bnd} nonMan=${nonman} | ridgeKept=${ridgeKept}/${ridgeLock.size}`);
    console.log(`  ⇒ ${ridgeKept === ridgeLock.size && nonman === 0 && bnd <= 4100 ? 'PASS — ALL ridges clean + watertight (seam closed)' : 'CHECK'}`);
    console.log('  wrote export-deliverables/SuperformulaBlossom_sf1_structured_p3b.stl');
