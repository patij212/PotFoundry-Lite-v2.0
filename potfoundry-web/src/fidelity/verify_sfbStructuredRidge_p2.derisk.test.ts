/**
 * verify_sfbStructuredRidge_p2.derisk.test.ts — PHASE 2: clean the corridor FLANKS.
 *
 * Phase 1 gave a smooth ridge (corridor, allFollowed, turning 0.6°) but the flank fills
 * carry slivers (%<10°≈5.7). This applies the research metric-flip to clean the flanks,
 * with a PRECISE lock on the ridge-chain edges (co.paved.featureChains) so the smooth
 * ridge is preserved, then reorients outward. Gate: %<20° down, ridge still allFollowed,
 * watertight. Writes STL. Pure CPU, PF_DERISK.
 */
...
function minAngle3D(a: V3, b: V3, c: V3): number {
  ...
  const lab = d(a, b), lbc = d(b, c), lca = d(c, a); if (lab < 1e-12 || lbc < 1e-12 || lca < 1e-12) return 0;
  const law = (a1: number, a2: number, op: number): number => Math.acos(Math.max(-1, Math.min(1, (a1 * a1 + a2 * a2 - op * op) / (2 * a1 * a2))));
  return Math.min(law(lca, lab, lbc), law(lab, lbc, lca), law(lbc, lca, lab)) * 180 / Math.PI;
}
...
function pickCusps(): UTPoint[][] {
  ...
  for (const c of graph.lines.filter((l) => l.kind === 'general-curve')) {
    ...
    const s = pts.filter((p) => p.t >= 0.12 && p.t <= 0.88).map((p) => ({ u: p.u, t: p.t }));
    if (s.length >= 4) out.push(s);
  }
  return out;
}

describe.skipIf(!process.env.PF_DERISK)('SFB structured-ridge PHASE 2 — clean flanks, keep smooth ridge', () => {
  it('metric-flips the flanks (ridge-locked) + reorients; ridge stays continuous', () => {
    ...
    // PRECISE ridge lock = the exact edges of the feature chains.
    const ridgeLock = new Set<number>();
    for (const chain of co.paved.featureChains) for (let i = 0; i + 1 < chain.length; i++) ridgeLock.add(ek(chain[i], chain[i + 1]));
    ...
    for (let pass = 0; pass < 14; pass++) {
      ...
      flips += f; if (f === 0) break;
    }
    // reorient outward
    const nT = T.length / 3; let reor = 0;
    for (let t = 0; t < nT; t++) { const a = T[3 * t], b = T[3 * t + 1], c = T[3 * t + 2]; const uc = (uv[a * 2] + uv[b * 2] + uv[c * 2]) / 3, tc = (uv[a * 2 + 1] + uv[b * 2 + 1] + uv[c * 2 + 1]) / 3; if (dot(nrm(cross(sub(P[b], P[a]), sub(P[c], P[a]))), analyticN(uc, tc)) < 0) { const tmp = T[3 * t + 1]; T[3 * t + 1] = T[3 * t + 2]; T[3 * t + 2] = tmp; reor++; } }
    ...
    let ridgeKept = 0, ridgeTot = 0; for (const k of ridgeLock) { ridgeTot++; if (eset.has(k)) ridgeKept++; }

    // write STL
    const buf = Buffer.alloc(80 + 4 + nT * 50); buf.write('SFB sf1 structured-ridge P2 (clean flanks)', 0, 'ascii'); buf.writeUInt32LE(nT, 80); let off = 84;
    ...
    fs.writeFileSync(path.join(OUT, 'SuperformulaBlossom_sf1_structured_p2.stl'), buf);

    /* eslint-disable no-console */
    console.log(`[P2 FLANK CLEAN] flips=${flips} reoriented=${reor} | outer=${outer} %<20=${(100 * lt20 / outer).toFixed(2)} %<5=${(100 * lt5 / outer).toFixed(2)} | bnd=${bnd} nonMan=${nonman} | ridgeKept=${ridgeKept}/${ridgeTot}`);
    console.log(`  ⇒ ${ridgeKept === ridgeTot && nonman === 0 ? 'PASS — flanks cleaned, smooth ridge intact, watertight' : 'CHECK'}`);
    console.log('  wrote export-deliverables/SuperformulaBlossom_sf1_structured_p2.stl');
    /* eslint-enable no-console */
    expect(ridgeKept).toBe(ridgeTot);
