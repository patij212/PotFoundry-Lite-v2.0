// s105BandsCone.ts — *** THE H1 PAIR, WITH A CONFIDENCE BAND. ***
//
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// PROVENANCE. VERBATIM COPY of `research/tools/s94ConeRefine.ts` with exactly TWO additions, both
// default-inert: (1) a PER-PARENT NDJSON dump (PF_BC_OUT) appended the instant each parent finishes;
// (2) resume-by-line-count. Nothing else is touched — same `score` (5698d023 sign convention), same
// `splitK`, same `bisectLepp`, same `refine`, same depth caps, same MAXLEAF/KCAP, same scope filters.
// It is a COPY and not an edit because s94ConeRefine.ts is a published instrument and a shared bundle
// path already caused a cross-agent collision in this campaign (2026-08-05).
//
// FIDELITY CHECK (run it, do not assume): with PF_BC_OUT unset this file must reproduce
// `frontier/S94_REF_GOTHSMOOTH.report.txt` (N=2000, SKIPFOLD=1, MAXSLOPE=1.0, maxDepth 8) and
// `frontier/S94_REF_VORCHORD.report.txt` (N=800, maxDepth 6) line for line. Both diffs are printed
// in S105_BANDS.md.
//
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// WHY THIS FILE EXISTS. *** H1 WAS REFUTED ON A 1.35x RATIO. *** Gothic scoped: lepp 3.120x vs cone
// 4.211x. Voronoi shape-gated: lepp 8.650x vs cone 16.076x (1.86x). Both are single golden-stride point
// estimates with no band. If the bands on those two numbers overlap, the refutation is not established.
//
// PRE-REGISTERED KILL (written before the first run):
//   *** if the 95% phase-block interval for cone/lepp CONTAINS 1.000 on either style, H1's refutation
//   is NOT established at that N and must be re-stated with its band. ***
// The ratio is measured TWO ways and both are reported:
//   (a) UNPAIRED — R disjoint phase blocks, ratio of block means. This is what the campaign's procedure
//       would have produced if it had been repeated.
//   (b) PAIRED — both operators run on the SAME parents, so per-parent (cone - lepp) has far less
//       variance than the difference of two independent estimates. The paired band is the SHARPER and
//       more honest test of "does cone beat lepp", and it is reported as such.
//
// Usage: bash research/tools/run-s105-bands-cone.sh
//   env (inherited, unchanged): PF_S94R_* exactly as s94ConeRefine
//   env (new):                  PF_BC_OUT (ndjson path)  PF_BC_RESUME (1)
...
const H = DIMS.H;
// ── S105 ADDITIONS ─────────────────────────────────────────────────────────────────────────────
const OUTND = process.env.PF_BC_OUT ?? '';
const RESUME = (process.env.PF_BC_RESUME ?? '1') === '1';
// ───────────────────────────────────────────────────────────────────────────────────────────────
...
log(`STL ${STL}`);
log(`S105 pool: ndjson '${OUTND}'  resume ${RESUME ? 'ON' : 'OFF'}`);
...
if (OUTND !== '' && RESUME && existsSync(OUTND)) {
  // *** RESUME MUST READ THE LAST RECORD'S `q`, NOT THE LINE COUNT. *** The scope filter drops parents,
  // so line count != sequence index; a line-count resume would re-walk and DUPLICATE thousands of rows.
  // (I wrote the line-count version first and caught it on the CGOTH pool before extending it.)
  const lines = readFileSync(OUTND, 'utf8').split('\n').filter((l) => l.length > 2);
  let lastQ = -1;
...
/** SIGN CONVENTION — PORTED FROM `orientRuler.ts` @ 5698d023 (2026-08-06). See s94ConeRefine.ts. */
...
/** k-WAY barycentric subdivision of a footprint, every new point LIFTED onto the surface. */
...
for (let q = QSTART; q < NS; q += 1) {
  ...
  // SCOPE FILTER (unchanged; S105 also RECORDS the covariates so the scope can be re-applied offline)
  let sl0 = 0;
  {
    ...
    sl0 = Math.hypot(rt0 / Math.max(1e-9, r00), rz0);
  ...
  P0.push({ chordUm: s0.chordUm, angDeg: s0.angDeg, diam: s0.diam, minAng: s0.minAng, slope: sl0 });
  ...
  for (const op of OPS) {
    for (const st of stopList) {
      ...
      perOp[`${op}@${st}`] = [r.tris, r.unc, r.overArea, r.leafArea, r.slack];
    }
  }
  // ── S105 ADDITION: per-parent record, appended the instant it is computed ────────────────────
  if (OUTND !== '') {
    const rec = {
      q, i: idx[q], a0: s0.area, c0: s0.chordUm, g0: s0.angDeg, dm: s0.diam, mn: s0.minAng,
      qs: (2 * Math.sqrt(s0.area)) / Math.max(1e-300, s0.diam), sl: sl0, cu: s0.coneUBdeg, op: perOp,
    };
    appendFileSync(OUTND, `${JSON.stringify(rec)}\n`);
  }
  // ────────────────────────────────────────────────────────────────────────────────────────────
  if ((q + 1) % 1000 === 0) {
    const el = (Date.now() - T0) / 1000; const done = q + 1 - QSTART;
    log(`  ${q + 1}/${NS}  [${el.toFixed(1)}s]  ${(done / Math.max(1e-9, el)).toFixed(1)} par/s  ETA ${((NS - q - 1) / Math.max(1e-9, done / el) / 60).toFixed(1)} min`);
  }
}
...
if (QSTART > 0) log('*** WARNING: RESUMED RUN — the aggregate lines above cover only this process\'s parents. ***');
