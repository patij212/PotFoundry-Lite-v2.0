// s117ApcrReach.ts — S117 P4: HOW FAR DOES APCR REACH?
//
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// WHAT THIS IS — AND WHAT IT IS NOT
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// This is s116BestGothic.ts generalised to ANY admitted style, plus the three things S116 did not need
// because it only ever ran on one style:
//   (1) a SIGNED PARAMETRIC COVERING CENSUS on the baseline AND on the operator mesh. APCR's
//       fold-freeness is "by construction" ONLY because radial projection preserves (theta, z), so a
//       child inherits its parent's footprint SIGN. That argument is silent about a mesh whose signs
//       are ALREADY mixed. GothicArches is a proven exact single cover; nothing else is known to be.
//       The covering number is therefore the candidate BOUNDARY of the operator and it is measured,
//       not assumed.
//   (2) a PERPENDICULAR adjudication of PRECOND. |r - rA| is the RADIAL ruler; on a cliff it reads the
//       height of the cliff and not the distance to the surface (S115: 612,236x; S117's own gate:
//       5152.4x on CelticTriquetra). Radial >= perpendicular pointwise, so radial is an UPPER BOUND
//       and only the perpendicular number may be quoted as a verdict (scar 5).
//   (3) PRE-REGISTERED KILL LINES, evaluated in-process against a FRESH baseline census computed by
//       the SAME census() code path. See S117_PREREGISTRATION.txt.
//
// GothicArches is carried as the REPRODUCTION CONTROL. If its numbers move relative to S116, this
// harness is not that harness and every S117 number in this file is VOID.
//
...
//
// Usage: bash research/tools/run-s117-apcr.sh
//   env PF_S117_STL(abs) PF_S117_STYLE PF_S117_OUTDIR PF_S117_TAG PF_S117_ROUNDS PF_S117_MAXT
//       PF_S117_BARHI PF_S117_BARLO PF_S117_PLACEBO(0/1) PF_S117_R3FULL(0/1)
...
const STYLE = envS('PF_S117_STYLE', 'GothicArches');
const STL = envS('PF_S117_STL', '');
const TAG = envS('PF_S117_TAG', 'GOTH');
const OUTDIR = envS('PF_S117_OUTDIR', 'research/exchange/_strataConformBisect/s116');
...
const TBAR = envF('PF_S117_TBAR', envF('PF_S117_BARHI', 0.01));
const ROUNDS = envI('PF_S117_ROUNDS', 14);
const MAXT = envI('PF_S117_MAXT', 5_000_000);
const K_LOOP = envI('PF_S117_KLOOP', 4);       // lattice order inside the refinement loop
const K_FINAL = envI('PF_S117_KFINAL', 8);     // lattice order for the census
const PERP_TOP = envI('PF_S117_PERPTOP', 4);
const DO_PLACEBO = envI('PF_S117_PLACEBO', 1) === 1;
const R3FULL = envI('PF_S117_R3FULL', 0) === 1; // exhaustive R3 down to the LO bar (slow: ~16 min/mesh)
const ORIENT_N = envI('PF_S117_ORIENTN', 4000);
...
log('════════════════════════════════════════════════════════════════════════════════════════════════════');
log(`===== S117 P4 — APCR REACH: ${STYLE} (tag ${TAG}) =====`);
log('════════════════════════════════════════════════════════════════════════════════════════════════════');
log(`stl    ${STL}`);
 ...
 *
 * MEASURED, not assumed. With the 3D chord metric (PF_S117_METRIC=3d) the operator halved the position
 ...
 */
const METRIC = envS('PF_S117_METRIC', 'par');
...
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// THE SIGNED PARAMETRIC COVERING CENSUS — the candidate BOUNDARY of the operator.
// Exhaustive, analytic-free apart from the z band. Transcribed from s116Adaptive.ts PART A so the
// numbers are directly comparable to S116's published covering numbers.
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
interface Cover { nPos: number; nNeg: number; nZero: number; aPos: number; aNeg: number; sPos: number; sNeg: number; domain: number; covering: number; net: number; inverted: number; }
function coverCensus(M: Mesh): Cover {
  let zLo = Infinity; let zHi = -Infinity;
  for (let t = 0; t < M.nT; t += 1) {
    if (M.alive[t] === 0) continue;
    for (let q = 0; q < 3; q += 1) { const z = M.vz[M.tv[t * 3 + q]]; if (z < zLo) zLo = z; if (z > zHi) zHi = z; }
  }
  const domain = 2 * Math.PI * (zHi - zLo);
  let sPos = 0, sNeg = 0, nPos = 0, nNeg = 0, nZero = 0, aPos = 0, aNeg = 0;
  const nb = new Float64Array(3);
  for (let t = 0; t < M.nT; t += 1) {
    if (M.alive[t] === 0) continue;
    const a = M.tv[t * 3], b = M.tv[t * 3 + 1], c = M.tv[t * 3 + 2];
    const ta = Math.atan2(M.vy[a], M.vx[a]);
    const tb = ta + dThRaw(ta, Math.atan2(M.vy[b], M.vx[b]));
    const tc = ta + dThRaw(ta, Math.atan2(M.vy[c], M.vx[c]));
    const a3 = 0.5 * triNormal(M, t, nb);
    const s = 0.5 * ((tb - ta) * (M.vz[c] - M.vz[a]) - (tc - ta) * (M.vz[b] - M.vz[a]));
    if (s > 0) { sPos += s; nPos += 1; aPos += a3; }
    else if (s < 0) { sNeg += -s; nNeg += 1; aNeg += a3; }
    else nZero += 1;
  }
  return { nPos, nNeg, nZero, aPos, aNeg, sPos, sNeg, domain, covering: (sPos + sNeg) / domain, net: (sPos - sNeg) / domain, inverted: Math.min(sPos, sNeg) / domain };
}
function printCover(C: Cover, label: string): void {
  log(`── SIGNED PARAMETRIC COVERING CENSUS — ${label} (EXHAUSTIVE, no tolerance) ──`);
  log(`   domain ${C.domain.toFixed(4)} rad*mm    POSITIVE ${C.nPos} facets / ${C.aPos.toFixed(3)} mm2    NEGATIVE ${C.nNeg} facets / ${C.aNeg.toFixed(3)} mm2    ZERO ${C.nZero}`);
  log(`   COVERING NUMBER sum|footprint|/domain = ${C.covering.toFixed(6)}   NET WINDING = ${C.net.toFixed(6)}   INVERTED SHEET = ${C.inverted.toExponential(4)} (${(C.inverted * 100).toFixed(6)}% of the chart double-covered with opposite orientation)`);
  log('');
}
...
interface Census {
  ...
  precondPerpMax: number; precondPerpN: number;
...
function census(M: Mesh, label: string, fullR3: boolean): Census {
  ...
  // SCAR 5 — every RADIAL precond flag is re-measured PERPENDICULARLY. On a cliff the radial ruler
  // reads the height of the cliff, not the distance to the surface. Only this number is a verdict.
  let precPerpMax = 0; let precPerpN = 0;
  if (precMax > 1e-3) {
    for (let j = 0; j < prec.length; j += 1) {
      if (prec[j] <= 1e-3) continue;
      const i = (j / 3) | 0; const q = j % 3;
      const v = M.tv[live[i] * 3 + q];
      precPerpN += 1;
      const d = proj.project(M.vx[v], M.vy[v], M.vz[v]).dist;
      if (d > precPerpMax) precPerpMax = d;
    }
  }
  ...
  return {
    ...
    precondPerpMax: precPerpMax, precondPerpN: precPerpN,
...
function printCensus(C: Census): void {
  ...
  log(`   PRECOND (EXHAUSTIVE, every corner, no stride) RADIAL = UPPER BOUND ONLY: MAX ${(C.precondMax * 1000).toFixed(4)} um   p99 ${(C.precondP99 * 1000).toExponential(3)} um   facets with a corner over ${BAR_LO} mm: ${C.precondOverLo} (${pct(C.precondOverLo, C.nT)}%)`);
  if (C.precondPerpN > 0) log(`   PRECOND PERPENDICULAR adjudication of the ${C.precondPerpN} radially-flagged corners (scar 5, THE VERDICT RULER): MAX ${(C.precondPerpMax * 1000).toFixed(4)} um   radial over-read ${(C.precondMax / Math.max(1e-12, C.precondPerpMax)).toFixed(1)}x`);
  else log(`   PRECOND PERPENDICULAR adjudication: not needed, radial MAX already under 1 um => the mesh IS this analytic surface.`);
...
const TCAP = Math.min(MAXT * 2 + 64, envI('PF_S117_TCAP', 6_000_000));
const VCAP = envI('PF_S117_VCAP', 3_200_000);
...
printCensus(cBase);
const covBase = coverCensus(B0);
printCover(covBase, 'BASELINE');
// free
...
printCensus(cOp);
const covOp = coverCensus(opRun.M);
printCover(covOp, 'OPERATOR APCR');
log(`   COVERING DELTA base -> operator: covering ${covBase.covering.toFixed(6)} -> ${covOp.covering.toFixed(6)}   inverted sheet ${covBase.inverted.toExponential(4)} -> ${covOp.inverted.toExponential(4)}`);
log(`   (APCR preserves the footprint sign of every child BY CONSTRUCTION, so these two lines must agree to the f32 floor. If they do not, the construction argument is broken and the run is VOID.)`);
log('');
...
function writeSTL(M: Mesh, path: string): number {
  ...
  buf.write(`S117 APCR ${STYLE} — analytic-projected conforming refinement`, 0, 79, 'ascii');
...
const OP_STL = `${OUTDIR}/S117_APCR_${TAG}.stl`;
...
// write a per-facet R3 scalar for the renderer heatmap (STL file order == live order)
{
  ...
  writeFileSync(`${OUTDIR}/S117_APCR_${TAG}_R3.f64`, Buffer.from(s.buffer));
  log(`   R3 scalar -> ${OUTDIR}/S117_APCR_${TAG}_R3.f64  (${s.length} f64)`);
}
const opLive = cOp.nT;
...
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
// THE PRE-REGISTERED KILL LINES (S117_PREREGISTRATION.txt, written before any non-Gothic arm was run)
// ══════════════════════════════════════════════════════════════════════════════════════════════════════
const K1_BAR = envF('PF_S117_K1', 5.0);
const K2_BAR = envF('PF_S117_K2', 3.0);
const K3_BAR = envF('PF_S117_K3', 1.25);
const k1 = cBase.hiA / Math.max(1e-12, cOp.hiA);
const k2 = cPla === null ? NaN : cPla.hiA / Math.max(1e-12, cOp.hiA);
const k3 = cOp.nT / cBase.nT;
const k4 = cOp.badA <= cBase.badA;
const k5um = cOp.precondPerpN > 0 ? cOp.precondPerpMax * 1000 : cOp.precondMax * 1000;
const k5 = k5um <= 1.0;
const k6 = cOp.nonMan === 0 && cOp.incons === 0;
const p1 = k1 >= K1_BAR; const p2 = cPla === null ? false : k2 >= K2_BAR; const p3 = k3 <= K3_BAR;
log('════════════════════════════════════════════════════════════════════════════════════════════════════');
log(`   PRE-REGISTERED KILL LINES — ${STYLE}`);
log('════════════════════════════════════════════════════════════════════════════════════════════════════');
log(`   K1 HEADLINE  >0.01mm AREA improvement vs FRESH baseline >= ${K1_BAR.toFixed(1)}x   MEASURED ${k1.toFixed(3)}x   ${p1 ? 'HOLD' : '*** FIRED ***'}`);
log(`   K2 PLACEBO   operator-vs-placebo AREA ratio        >= ${K2_BAR.toFixed(1)}x   MEASURED ${Number.isFinite(k2) ? k2.toFixed(3) : 'n/a'}x   ${p2 ? 'HOLD' : '*** FIRED ***'}`);
log(`   K3 COST      live triangles                        <= ${K3_BAR.toFixed(2)}x   MEASURED ${k3.toFixed(4)}x   ${p3 ? 'HOLD' : '*** FIRED ***'}`);
log(`   K4 FOLDS     >CEIL BAD area must not exceed baseline (${cBase.badA.toFixed(4)} mm2)   MEASURED ${cOp.badA.toFixed(4)} mm2   ${k4 ? 'HOLD' : '*** FIRED ***'}`);
log(`   K5 PRECOND   operator PERPENDICULAR precond MAX    <= 1.0 um   MEASURED ${k5um.toFixed(4)} um   ${k5 ? 'HOLD' : '*** FIRED ***'}`);
log(`   K6 TOPOLOGY  non-manifold ${cOp.nonMan}, inconsistent winding ${cOp.incons}   ${k6 ? 'HOLD' : '*** FIRED ***'}`);
const verdict = !p1 ? 'REFUTED ON ITS OWN BAR' : !p2 ? 'REFUTED BY PLACEBO' : !(p3 && k4 && k5 && k6) ? 'REFUTED BY SIDE EFFECT' : 'WIN';
log('');
log(`   *** VERDICT for ${STYLE}: ${verdict} ***`);
log(`   (baseline covering number ${covBase.covering.toFixed(6)}, inverted sheet ${covBase.inverted.toExponential(4)} — H-COVER predicts the win tracks this.)`);
log('');
const out = {
  style: STYLE, stl: STL, ceilDeg: CEIL_RAD * DEG, LMAX, graphSlope: GRAPH_SLOPE,
  verdict, k1, k2, k3, k4, k5um, k6,
  coverBase: covBase, coverOp: covOp,
  arms: rows.map((C) => ({
    label: C.label, nT: C.nT, area: C.area, precondMaxUm: C.precondMax * 1000, precondPerpMaxUm: C.precondPerpMax * 1000, precondPerpN: C.precondPerpN, precondOverLo: C.precondOverLo,
...
writeFileSync(`${OUTDIR}/S117_APCR_${TAG}.json`, JSON.stringify(out, null, 1));
log(`json -> ${OUTDIR}/S117_APCR_${TAG}.json`);
log('S117 P4 DONE');
