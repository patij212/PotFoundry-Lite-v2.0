#!/usr/bin/env bash
# run-s115-hceltic.sh — S115: the h-ladder on the >45 deg class. Own bundle path; never share a runner.
#
...
cd "$(dirname "$0")/../.."
echo "cwd: $(pwd)"
TOOL=research/tools/s115hCeltic.ts
BASE="$(basename "$TOOL" .ts)"
OUT=research/bridge/out
BUNDLE="$OUT/_run_${BASE}.cjs"
OUTDIR=research/exchange/_strataConformBisect/s115h
REPORT="$OUTDIR/S115H_${PF_S115_TAG:-RUN}.report.txt"
mkdir -p "$OUT" "$OUTDIR"
