...
arms.push(P1);
log('   A2 FLIP (2-2, textbook)');
const A2 = runArm('A2 FLIP (2-2, textbook)', 'flip', bladeSel(BLADE_BAR), PASSES);
log(`     => applied ${A2.applied} flips over ${A2.perPass.length} passes  facets ${A2.mesh.nF}  new ${A2.newFacets.length}`);
arms.push(A2);
log('   P2 FLIP-RAND (placebo, cost-matched)');
const P2 = runArm('P2 FLIP-RAND (placebo)', 'flip', randSel(Math.min(TOPO0.edges.length, Math.max(1, A2.applied) * 4), 98765), PASSES, A2.applied);
log(`     => applied ${P2.applied} flips  facets ${P2.mesh.nF}  new ${P2.newFacets.length}`);
arms.push(P2);
...
const pairFor: Record<string, string> = { 'A1 WELD (fin excision)': 'P1 WELD-RAND (placebo)', 'A2 FLIP (2-2, textbook)': 'P2 FLIP-RAND (placebo)' };
for (const arm of arms) {
  const r = scores.get(arm.name) as ScoreA;
  const ds = deltas.get(arm.name) as DeltaScore;
...
writeFileSync(`${OUTDIR}/S115OP_${TAG}.json`, `${JSON.stringify(OUT, null, 2)}\n`);
log(`wrote ${OUTDIR}/S115OP_${TAG}.json  done ${el()}`);
