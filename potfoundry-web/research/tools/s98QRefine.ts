// s98QRefine.ts — *** IS REFINABILITY DECIDED BY PARENT SHAPE? AND IF SO, WHERE IS THE THRESHOLD? ***
//
// PROVENANCE: this file is a VERBATIM COPY of `research/tools/frontierRefine.ts` (S93's instrument)
// with ONE addition — the LEPP arm's per-parent result is binned by the parent's shape index
//     q = h_min / sqrt(area) = 2*sqrt(area) / longest_edge      (equilateral q = 1.316, sliver q -> 0)
// Nothing else is touched: the same `score`, the same `lift`, the same `bisect`, the same bars.
// It is a COPY and not an edit because `frontierRefine.ts` is in use by a concurrent agent and a
// shared bundle path already caused one cross-agent collision in this campaign (2026-08-05).
//
// FIDELITY CHECK (run it, do not assume): every aggregate line this tool prints must match
// `frontierRefine.ts` on the same STL/seed/N. If any differs, the copy has drifted and the binning
// is void.
//
// WHY: S95 showed Voronoi's refinement failure tracks facet SHAPE (q), not wall angle — 112x binned
// on angle, 0.30x binned on q. That was measured ACROSS two meshes. This measures it WITHIN ONE mesh,
// where nothing else can differ, and reads off the q at which `uncleared` reaches zero. That number is
// what `PF_CB_SHAPE_AR` should be set from.
//
// PRE-REGISTERED KILL: if `uncleared%` does NOT fall monotonically with q, or if the top q bin still
// carries >= 5% uncleared, then parent shape does NOT decide refinability and the S95 reading was a
// between-mesh confound. Report either way.
//
// Usage:  bash research/tools/run-s98-qrefine.sh
...
// ── S98 ADDITION: per-parent shape bins ────────────────────────────────────────────────────────
...
for (let q = 0; q < NS; q += 1) {
  ...
  // ── S98 ADDITION: bin THIS parent's lepp result by its own shape index ───────────────────────
  {
    ...
    const qIdx = (2 * Math.sqrt(s0.area)) / Math.max(1e-300, Math.max(e0, e1, e2));
    const b = qBin(qIdx);
    qParents[b] += 1; qTris[b] += bl.tris; qUnc[b] += bl.unc; qArea[b] += s0.area;
    const r1 = adapt((s) => s.angDeg > 1);
...
log('');
log('FIDELITY: every line ABOVE this S98 block is frontierRefine.ts verbatim. Diff them on the same');
log('input before trusting the binning.');
