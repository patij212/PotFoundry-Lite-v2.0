...
## 8. SCOUT ADDENDUM 2026-07-04 — the wall MOVED: the last wall is the ZERO-WIDTH CUSP on COUNT-UNSTABLE networks

_Compiled 2026-07-04 (SOTA mechanism-mine, READ+WEB only). The DRIVE-0.01 campaign (scorecard `research/lab/2026-07-03-drive-001-scorecard.md`; registry `E-2026-07-04-GF-GOTHIC`, `-GD-GOTHIC`, `-CU-GOTHICSEG`, `-COL-SUBDIV`) settled §1–§7's walls and exposed a NARROWER one that §1–§7 does NOT address. Citations verified via `WebSearch`/`WebFetch` on 2026-07-04 (venue+authors checked)._

**The wall §1–§7 did NOT know about (now the ONLY residual).** 17/20 styles reach literal true-3D p99 ≤0.01 + zero-serration + watertight; SFB reaches all buildable gates. **FACES ARE CAD-GRADE ON ALL 20** — the constraint-model axis (§2), the sizing-metric axis (§6 Axis 2) and the cut-graph axis (§4) are all effectively SOLVED for the surface. The lone residual is **GothicArches + GeometricStar**, and it is proven (`E-2026-07-04-GF-GOTHIC`, 5 refuted levers: doubled-crest → local-segment → collinear-subdivide → panel-density → flank-tessellation) to be ONE mechanism: **a flat triangle in the (u,t) UV domain cannot follow a designed zero-width `ridge(sharp)` cusp — 56% of Gothic's worst facets are knife-edge cusps (flankSpanArc=0), floor 0.058 flank-pitch-INVARIANT** — occurring on **count-UNSTABLE feature networks** (Gothic rib crests recon 0→12→…→72; GeoStar chevrons 6→32) where NO fixed edge-chain set exists to embed. This is a **FEATURE-TOPOLOGY + REPRESENTATION** wall, orthogonal to §1–§7 (which assume a count-stable embeddable feature complex and a flat-triangle element). The proven WIN mechanism everywhere it applies (SFB seam, BasketWeave, LowPoly, Bamboo → 0 outliers) is **explicit feature-edge embedding**; the frontier = generalize it to count-unstable cusps.

**Two NEW load-bearing SOTA axes §1–§7 lacked, verified 2026-07-04:**

**Axis 3 — the ELEMENT model (a CURVED element follows the cusp; flat cannot).** The scorecard's own escape line ("a curved/anisotropic crest-ribbon primitive — out of flat-triangle-in-UV").
- **Curved PN triangles — Vlachos, Peters, Boyd, Mitchell, I3D 2001** [C21]. A single flat triangle + its 3 vertex normals is replaced by a 3-sided **cubic Bézier patch**; the element is curved and hugs relief a flat facet chords across. **Key for us:** a per-edge flag can BREAK normal continuity across an edge to produce a **sharp edge** — i.e. a curved patch on each flank of a crest that stays C0-sharp AT the crest. Directly transplantable to (u,t): our vertices already sit on the true surface (faces CAD-grade); lift each crest-adjacent triangle to a Bézier patch whose control points are placed by the analytic `rA` normal, evaluate the patch (not the flat facet) when STL-tessellating the last row — the interior of the exported micro-facets then hugs the cusp instead of chording it. This is the most direct "0 outliers on the cusp" mechanism and reuses the exact-vertex mesh we already have.
- **Higher-order / curved metric-based adaptation — MMG/mmgs curvature-metric + Hausdorff (Dobrzynski–Frey) [C18]; "Curved Mesh Adaptation for High-Order FE" arXiv 2606.10763 [C22] (verify author list).** Bézier-based curved edges/faces are curved to the CAD surface to the element order, and the **Hausdorff `hausd` parameter directly bounds mesh-to-surface distance** — the literature's native "interior deviation ≤ tol" knob (exactly our per-triangle-interior ruler). Transplant: run the crest-ribbon strip as **curved (P2/P3) elements under the curvature metric with `hausd`=0.01**, not flat P1.

**Axis 4 — the LAYOUT model for COUNT-UNSTABLE junctions (feature-line-as-patch-boundary, T-junctions ALLOWED).** The count-instability (ribs/chevrons born+merged across the surface) is precisely what breaks a fixed-column primitive; the SOTA that is DESIGNED for it:
- **Reliable Feature-Line Driven Quad-Remeshing (quadwild) — Pietroni, Nuvoli, Alderighi, Cignoni, Tarini, ACM TOG / SIGGRAPH 2021** [C23]. Builds a **perfectly feature-aligned cross field** + a coarse layout where **every feature-line IS a patch boundary by construction**, and — the load-bearing part — it **deliberately ALLOWS T-junctions and non-quadrilateral patches in the layout** so a merging/splitting feature count is representable (a rib that dies becomes a T-junction, not a degenerate collapsed column). Code: `github.com/nicopietroni/quadwild` (GPL-3). This is the direct answer to "the count is unstable so no fixed edge-chain exists": the layout ABSORBS the count change into T-junctions instead of fighting it. Transplant caveat: it is a 3D-surface quad remesher — feed it the dense-truth (u,t)→3D of Gothic (single-valued, no self-occlusion) and treat its feature-line-patch-boundaries as the crest edges to embed back in (u,t).
- **Navigating Intrinsic Triangulations (signpost) — Sharp, Soliman, Crane, SIGGRAPH 2019** [C7]. The fallback the thesis already names, now sharpened for THIS wall: intrinsic (geodesic) edges cross a junction **legally along the exact surface**, so a count-unstable crossing that a planar CDT collinearly self-blocks (the `E-2026-07-04-COL-SUBDIV` refutation: recovery stalled 74.5%, kernel interior points collinear-block their own constraint segments) can be a legal intrinsic edge. Intrinsic Delaunay refinement guarantees a 30° min-angle. `geometry-central` ships it.
- **Frontal-Delaunay size-optimal Steiner on curved surfaces — Engwirda, Ivers, CAD/IMR23 2015** [C24] (JIGSAW). Off-centre Steiner points on Voronoi faces + advancing-front hybrid for bounded radius-edge on curved surfaces — the size-optimal insertion that avoids the sliver the circumcenter would make near a feature; relevant if the crest-ribbon strip needs its own quality guarantee.

**Restricted-Delaunay fidelity guarantee (the theory that a mesh CAN hug to a bound) — Boissonnat–Oudot [C2] confirmed 2026-07-04:** loose ε-sample ⇒ restricted Delaunay is homeomorphic to the surface, **Hausdorff distance made arbitrarily small, facet aspect bounded** — the theorem underwriting "0 outliers is achievable if you sample by local feature size." Our per-triangle-interior deviation IS the (one-sided) Hausdorff our exported facets must satisfy; the missing piece is only that a FLAT facet cannot realize the bound across a zero-width cusp (Axis 3 fixes that) on a count-unstable locus (Axis 4 fixes that).

### Ranked TRANSPLANTABLE mechanisms → 0 outliers on Gothic's cusps (most-direct first)

| # | Mechanism (verified) | 2–3-line transplant to our (u,t)-radial setting | Directness to 0-outlier-on-cusp |
|---|---|---|---|
| **1** | **Curved PN-triangle / Bézier crest-ribbon element** [C21] + **`hausd` curvature-metric curved adaptation** [C18,C22] | Our vertices already sit on the true surface (faces CAD-grade). Lift each cusp-adjacent triangle to a cubic Bézier patch: control points from the analytic `rA` surface normal; **break normal continuity across the crest edge → C0-sharp cusp**; evaluate the PATCH (not the flat facet) when emitting the final STL micro-facets, with `hausd`≤0.01 setting the subdivision depth. The interior then HUGS the cusp instead of chording it. Reuses the exact-vertex mesh; no re-mesh, no count solve. | **HIGHEST** — directly kills "flat facet chords the zero-width cusp"; the only mechanism that attacks the proven root cause (flatness) rather than density/recovery, all 5 of which refuted. |
| **2** | **quadwild feature-line-driven layout, T-junctions ALLOWED** [C23] | Feed dense-truth (u,t)→3D Gothic (single-valued) to quadwild; its cross-field layout makes every rib/chevron crest a **patch boundary by construction** and absorbs the birth/merge count into **T-junctions** (a dying rib = a T, not a collapsed half-ring facet — the exact degeneracy that killed doubled-crest at 0.042 and 26.7mm). Import the feature-line-patch-boundaries back as the (u,t) crest edges to embed. | **HIGH** — the only SOTA built for count-UNSTABLE feature topology; directly dissolves "no fixed edge-chain exists." Cost: 3D-surface remesh + a re-embed step; validate on Gothic (no self-occlusion). |
| **3** | **Intrinsic (signpost) Delaunay for legal junction crossings** [C7] | Replace the planar-CDT constraint recovery (proven collinear-self-blocking, `E-2026-07-04-COL-SUBDIV`) with intrinsic edges that cross the rib-lattice junction **legally along the exact surface**; 30°-min-angle guaranteed. Crests that a flat CDT gives up on become legal intrinsic mesh edges. | **MED-HIGH** — directly fixes the 74.5%-recovery collinear block, but does NOT by itself curve the facet (pair with #1); attacks the RECOVERY sub-wall, not the flatness sub-wall. |
| **4** | **Frontal-Delaunay size-optimal Steiner on curved surfaces (JIGSAW)** [C24] | Insert crest-ribbon Steiner points as off-centre Voronoi-face points (advancing-front hybrid) sized by local feature size, so the ribbon strip has bounded radius-edge WITHOUT the circumcenter sliver near the cusp. | **MED** — a quality/insertion improvement for the ribbon; needs #1 for the fidelity. |
| **5** | **Restricted-Delaunay ε-sample Hausdorff guarantee** [C2] as the ACCEPTANCE THEORY | Use it as the ORACLE bound, not a mesher: sample the crest by local-feature-size, prove the resulting facets meet a two-sided Hausdorff ≤0.01 — confirms whether 0-outlier is even attainable for a given cusp before building. | **THEORY** — tells us the target is achievable off the cusp and pinpoints the cusp as the sole obstruction; frames the go/no-go for #1. |

**One-line synthesis for the frontier loop:** the last wall is no longer constraint-model / sizing / occlusion (§1–§7, solved) — it is the **flat-element × count-unstable-cusp** pair. The two NEW axes are **curved elements** (Axis 3: PN-triangle/Bézier crest-ribbon with a normal-discontinuity flag = C0-sharp cusp, `hausd`-bounded) and **feature-line-driven T-junction-tolerant layout** (Axis 4: quadwild). Bet #1 (curved crest-ribbon) is the most direct and the cheapest to transplant (reuses the exact-vertex CAD-grade mesh; only the final STL emission of cusp-adjacent facets changes), and it is the one mechanism that attacks the PROVEN root cause (flatness) that all 5 refuted Gothic levers left untouched.

**New bibliography (verified 2026-07-04):**
- **[C21]** A. Vlachos, J. Peters, C. Boyd, J. L. Mitchell. *Curved PN Triangles.* ACM SIGGRAPH Symposium on Interactive 3D Graphics (I3D) 2001. Cubic Bézier patch from 3 vertices+normals; per-edge normal-discontinuity flag → sharp edges. `research.cs.wisc.edu/graphics/Papers/Vlachos/`
- **[C22]** *Curved Mesh Adaptation for High-Order Finite Element Simulations.* arXiv:2606.10763 (2026) — Bézier-based curved edges/faces curved to CAD to element order; **(verify author list before load-bearing)**. `arxiv.org/abs/2606.10763`
- **[C23]** N. Pietroni, S. Nuvoli, T. Alderighi, P. Cignoni, M. Tarini. *Reliable Feature-Line Driven Quad-Remeshing.* ACM TOG 40(4), SIGGRAPH 2021. Feature-lines = patch boundaries by construction; **T-junctions + non-quad patches allowed** (handles count-unstable feature topology). Code: `github.com/nicopietroni/quadwild` (GPL-3). `dl.acm.org/doi/10.1145/3450626.3459941`
- **[C24]** D. Engwirda, D. Ivers. *Size-Optimal Steiner Points for Delaunay-Refinement on Curved Surfaces.* Computer-Aided Design (IMR23 special issue), 2015. Off-centre Voronoi-face Steiner + advancing-front hybrid (Frontal-Delaunay); JIGSAW. `arxiv.org/abs/1501.04002`
- (**[C7]** Sharp–Soliman–Crane 2019, **[C2]** Boissonnat–Oudot 2005, **[C18]** MMG/mmgs — already in §7; re-confirmed 2026-07-04 as directly on-target for THIS wall.)

_No meshes were run and no code was touched to produce this addendum. Empirical basis: `research/lab/2026-07-03-drive-001-scorecard.md` + registry `E-2026-07-04-GF-GOTHIC`/`-GD-GOTHIC`/`-CU-GOTHICSEG`/`-COL-SUBDIV`._
