...
## V12 — PRODUCTION-INTEGRATION arm (E-2026-07-09-REBASELINE20): GeoStar Tier-C PRODUCTION gate = LITERAL 0 (preferred); the 20-style rebaseline FIRED THE KILL — the Tier-C DISPATCH PREDICATE over-triggers (17/20 styles), the one gating defect before any flag-flip (2026-07-09)

**FRAME.** The DRIVE-ALL-20 fidelity campaign is TERMINAL (§V11ae; scorecard `research/lab/2026-07-09-drive-final-scorecard.md`, commit 83fe4c36; USER DECISION 2026-07-09 accepts the Tier-2/3 certs + the Gothic production frontier). This arm runs the two remaining ENGINEERING items (scorecard "Remaining engineering" §1) with PER-STYLE acceptance, flag `__pfPerfectMesher` default-OFF throughout. Back-port plan T6.

**ITEM 1 — GeoStar PRODUCTION Tier-C gate (the 2nd count-unstable style, never gated in production). VERDICT: LITERAL 0 (the PREFERRED acceptance), NOT the frontier fallback.**
- Interior band (u[0,0.05]×t[0.45,0.55], nTheta 256, base kernel): **literal 0 outliers, max 0.01000, 7792 tris, capped=false** — GeoStar's finite-width chevron (130–137°) rides flat-P1 to <tol (vindicates E-…-GEOSTAR-CRESTSTRIP).
- Full-relief band (u[0,0.1]×t[0.4,0.6], nTheta 1024): the DEFAULT `runPatchGate` config (no banked levers) **PLATEAUS at 2 outliers @0.02808, p99 0.00912<tol, capped**. The 2 residuals classified: BOTH at um=0.1 (the domain uHi BOUNDARY, constraint-chain clip), gradU 0.2 / 0.9 (near-FLAT, not steep flank) ⇒ a benign domain-clip artifact = the Gothic-frontier PATTERN, NOT a new mechanism class (the kill for "new mechanism" did NOT fire).
- **Banked-lever escalation, SAME full band:** aniso edgeSag → 1 out @0.01061 (28599t, half the growth of iso); **aniso + `dedupeCellMm` 0.001 → LITERAL 0 outliers, worst 0.01, capped=FALSE, 28762t, 221s.** ⇒ the plateau was the **0.004mm DEDUPE-LATTICE floor** (the Gothic §V11p diagnosis, now CONFIRMED on a 2nd style), NOT a fidelity floor. The banked levers close GeoStar to literal-0 with FEWER tris than the base plateau.

**ITEM 2/3 — 20-style production re-baseline (`tierC/rebaseline20.test.ts`, PF_REBASELINE20=1, acceptance UPDATED to the scorecard per-style table). VERDICT: KILL-CRITERION FIRED — DISPATCH PREDICATE OVER-TRIGGERS.**
- The dispatch predicate `isCountUnstableStyle` = `countJunctionNodes(graph) > 0` (production, `TIER_C_DETECT_OPTS` coarseRes40/fineRes120) selects **17 of 20** styles as count-unstable, not the intended 2. Only **SuperellipseMorph, FourierBloom, LowPolyFacet** are count-stable (junctions = 0).
- ROOT CAUSE (`_dispatchDiag`, PF_DISPATCHDIAG=1 — junction counts): SuperellipseMorph j=0 (maxDeg2) · FourierBloom j=0 (maxDeg2) — the ONLY true-0; HarmonicRipple j=84 · WaveInterference j=219 · Voronoi j=263 · ArtDeco j=349 (smooth/relief styles → SPURIOUS deg≥3 junctions from detected feature-crossing noise); **GeometricStar j=230 · GothicArches j=163 (the INTENDED pair) sit IN THE MIDDLE of the "stable" range (84..349)**. No raw junction-COUNT threshold separates the intended pair from the noise ⇒ a predicate DESIGN defect, not a tuning miss.
- CLASS: ONE shared root cause (the kill ">2 styles ⇒ report the pattern, not 20 problems" fired exactly). PRODUCTION IMPACT = **ZERO today** (flag default-OFF; byte-identical-off proven; fast tierC suite 11f/21t GREEN; Gothic smoke gate literal-0 GREEN). But it is the ONE gate BLOCKING any flag-flip: flag-on routes 17/20 styles through the intractable full-pot Tier-C path (documented seam-unshared, multi-day single-thread). The scorecard/back-port premise "Tier-C fires ONLY for Gothic/GeoStar" is VIOLATED by the current predicate.
- Full-pot Tier-C on the count-unstable styles is INTEGRATION scope + intractable in-gate (patch @2% wall area ≈ 6 min; full-pot ≈ 50× facets, superlinear brute scorer ⇒ multi-day). The updated gate therefore asserts the TRACTABLE ship properties: (a) 18-style byte-identical fallback [would-be, once the predicate is fixed], (b) the dispatch predicate selects exactly the 2 count-unstable styles [the assertion that CAUGHT the defect]; fidelity for the 2 is DELEGATED to `wholeMesh0Outlier.test.ts` (patch scale).

**KILL-CRITERIA STATUS (§V12):** item-1 "new mechanism class" — did NOT fire (GeoStar residual = Gothic-frontier boundary-clip pattern; literal-0 reachable with banked levers). item-2/3 ">2 styles regressing ⇒ stop + report the pattern" — FIRED (17/20 dispatched; ONE shared predicate defect). Reported before any predicate fix (fix = a production-symbol change ⇒ separate pre-registered experiment + gitnexus impact on `isCountUnstableStyle`/`TIER_C_DETECT_OPTS`).

**VERDICT (§V12):** GeoStar production Tier-C = LITERAL-0 achievable (preferred acceptance); the DISPATCH PREDICATE is the single blocking defect for the flag-flip. The updated per-style acceptance tests are the honest ship gate and correctly RED on the defect. GeoStar/Gothic FIDELITY is not the blocker.

**RECOMMENDATION:** (1) SHIP the updated per-style acceptance tests (this arm — `research/bridge/_pf_rebaseline20.test.ts` header + `src/…/tierC/rebaseline20.test.ts`). (2) NEXT EXPERIMENT (separate, pre-registered, PRODUCTION-symbol change ⇒ gitnexus impact FIRST): replace the raw junction-COUNT predicate with a count-INSTABILITY signal that separates Gothic-rib-net / GeoStar-chevron from smooth/relief junction noise — candidates: junction density normalized by feature arc-length; family birth/merge ACROSS the two detector scales (the actual "count changes with t" definition the predicate was meant to encode); or a per-style allow-list as the conservative interim. (3) Flag stays default-OFF until the predicate separates cleanly AND the full-pot seam-share integration lands.

**LEDGER:** registry E-2026-07-09-REBASELINE20 (pre-reg committed feea5e89, BEFORE the gates ran). Probes: `src/renderers/webgpu/parametric/conforming/tierC/{wholeMesh0Outlier,rebaseline20}.test.ts` (PF_TIERC_WHOLEMESH / PF_REBASELINE20) + dev diagnostics `_geostarProbe` (PF_GSPROBE) / `_geostarFrontier` (PF_GSFRONT) / `_dispatchDiag` (PF_DISPATCHDIAG); research probe `research/bridge/_pf_rebaseline20.test.ts` header updated to the scorecard. Data `research/exchange/_rebaseline20_final/{FINDINGS.md, geostar_frontier.ndjson, geostar_gate.log, rebaseline20_run.log}`. Flag `__pfPerfectMesher` default-OFF; all edits DEV/test-gated (byte-identical off held). Finish commit [this].

## V12b — DISPATCH-PREDICATE FIX (E-2026-07-09-DISPATCH-PREDICATE): the §V12 kill closed — 4 graph signals REFUTED, per-style allow-list SHIPPED, rebaseline20 GREEN (2026-07-09, commit 990065cd)

**FRAME.** §V12's kill was the ONE gate blocking any flag-flip: `isCountUnstableStyle = countJunctionNodes(graph) > 0` selected 17/20. This arm replaces the predicate (a PRODUCTION-symbol change ⇒ gitnexus impact FIRST; MCP tool unavailable in-session ⇒ hand-traced the call graph — see below).

**IMPACT (hand-traced; gitnexus MCP not loaded this session).** `isCountUnstableStyle` + `TIER_C_DETECT_OPTS` are consumed ONLY inside `tierC/`. `isCountUnstableStyle` → called by `buildTierCOuterWall` (index.ts, flag-ON branch only) + `rebaseline20.test.ts` + `countUnstable.test.ts`. `buildTierCOuterWall` → called by `ParametricExportComputer.compute` (line 2207, inside the `__pfConformingProbe`-gated dev block) but the predicate is reached ONLY when `__pfPerfectMesher===true`; flag-off = pure delegation BEFORE the graph is built. `TIER_C_DETECT_OPTS` → `index.ts`/`morseComplex.ts` (both flag-ON) + 3 dev probes. **RISK: LOW** — no production reachability with the flag off; byte-identical-off is structural.

**SIGNAL MEASUREMENT (`_dispatchSignals.test.ts`, PF_DISPATCHSIG=1; data `research/exchange/_dispatch_predicate/signals.ndjson`).** Measured on ALL 20 (detector-level, no meshing) the 4 pre-registered candidates: (a) junction DENSITY = j/arc-len(mm); (b) count-INSTABILITY across scale j@fineRes120 vs j@240; (c) CHAIN-weighted junction fraction (junctions on ≥20mm connected components); (d) CONDITIONED-graph residual junctions (conditionGraph prune+merge). **VERDICT: NONE separates cleanly.** No single-signal threshold isolates {Gothic, GeoStar} (brute-checked every gap, all 12 candidate keys). The intended pair sits mid-rank in every signal: density Gothic 0.0507 #3 / GeoStar 0.0158 #8; jRatio Gothic 7.39 / GeoStar 2.97 mid-pack; longFrac Gothic 0.847 / GeoStar 0.804 (two lowest non-zero, but 14 styles cluster 0.83–1.0); condJ Gothic 28 / GeoStar 74 mid-range. 2-signal axis-aligned boxes DO fit (14 key-pairs), but every one is a knife-edge overfit: tightest face 0.5% margin (longFrac 0.847 vs Gyroid 0.851), density face 2.5% (0.0158 vs Hex 0.0154) — a 4-param box fitting exactly 2 of 20 points, NOT the structural / order-of-magnitude separation the pre-registration required.

**KILL fired (signal path) ⇒ ALLOW-LIST interim SHIPPED** (pre-registered fallback #2). `COUNT_UNSTABLE_STYLES = {GothicArches, GeometricStar}`, keyed on styleId (threaded from `params.styleId` → `buildTierCOuterWall(sampler, opts, styleId?)`). `countJunctionNodes` retained as diagnostic. Loud module-header comment + this ledger note: the general count-instability predicate is OPEN RESEARCH. An honest allow-list beats a fragile threshold that a res/param change or a new style would silently break (mis-routing a smooth style through the intractable full-pot Tier-C path).

**GATE STATUS.** TDD confusion matrix `countUnstable.test.ts` (20 styles, Gothic+GeoStar IN, 18 OUT) — 26/26 GREEN. Ship gate `rebaseline20.test.ts` (PF_REBASELINE20=1) — **GREEN**: dispatch selects EXACTLY `["GothicArches","GeometricStar"]`, 18 count-stable byte-identical fallback (all `byteIdentical:true`). Fast tierC suite 12f/45t GREEN (Gothic smoke still literal-0, pass7 out=0). flagOff.byteIdentical GREEN. eslint 0-warnings. No new typecheck errors (2 pre-existing unused-var errors in ParametricExportComputer confirmed on base). Flag `__pfPerfectMesher` default-OFF throughout.

**VERDICT (§V12b):** the §V12 blocking defect is CLOSED with an honest allow-list; the flag-flip's dispatch gate is GREEN. Remaining flip blockers (unchanged): the full-pot cdt2d seam-share integration + the sliver concession. General graph-signal count-instability predicate = deferred open research (no defensible-margin signal found).

**LEDGER:** registry E-2026-07-09-DISPATCH-PREDICATE; commit 990065cd; probe `_dispatchSignals.test.ts` (PF_DISPATCHSIG); data `research/exchange/_dispatch_predicate/{signals.ndjson, rebaseline20_run.log}` (dir gitignored — on-disk only). Files: `countUnstable.ts` (allow-list + COUNT_UNSTABLE_STYLES), `countUnstable.test.ts` (confusion matrix), `index.ts` (styleId thread), `rebaseline20.test.ts` (styleId dispatch), `ParametricExportComputer.ts` (pass params.styleId).
