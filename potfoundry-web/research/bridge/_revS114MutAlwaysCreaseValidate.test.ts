...
/**
 * THE ASYMMETRIC-CREASE SURFACE (S114). r = R0 + A*max(0, 1 - |th-thc|/w) — a linear TENT of half-width
 * `w` sitting on a cylinder of radius R0. It carries THREE C0 loci and they are NOT of the same kind:
 *   thc - w   an asymmetric INNER clamp  (slope 0 meets +A/w)
 *   thc       a SYMMETRIC crest          (slope +A/w meets -A/w)
 *   thc + w   an asymmetric OUTER clamp  (slope -A/w meets 0)
 * A symmetric crest is the easy case for any bisection: the two half-turns straddling it are equal by
 * construction. The clamps are the case that broke — see F11.
 */
const tent = (R0: number, A: number, thc: number, w: number) => ({
  r: (th: number) => R0 + A * Math.max(0, 1 - Math.abs(th - thc) / w),
  rTh: (th: number) => { const u = th - thc; return Math.abs(u) >= w ? 0 : (u < 0 ? A / w : -A / w); },
  rZ: () => 0,
});
...
/**
 * THE SCAN ORACLE — deliberately dumb, expensive and INDEPENDENT of the locator under test.
 *
 * It samples `cells+1` finite-difference normals uniformly across the whole segment and takes the largest
 * turn over pairs TWO APART. Nothing here brackets, bisects or discards a half, so it cannot lose a
 * feature the way a bisection can; it is the reference F11 measures the locator against.
 *
 * WHY TWO APART AND NOT ADJACENT. The FD step is a 64th of a cell, so at most ONE sample's window can
 * straddle the locus, and that sample returns an INTERMEDIATE normal that splits the turn across its two
 * adjacent cells — the very mechanism this whole fixture is about. The pair (i-1, i+1) skips that sample
 * and is clean on both sides whether the locus sits at a sample or inside a cell. Using adjacent pairs
 * here would give the oracle the same blind spot as the instrument it is supposed to check.
 */
function scanOracle(
  rA: (th: number, z: number) => number, ath: number, az: number, bth: number, bz: number,
  rRef: number, cells: number,
): { s: number; turnDeg: number } {
  ...
  return { s: ib / cells, turnDeg: best * DEG };
}
...
describe.skipIf(!RUN)('ORIENT-FTV — the orientation ruler, on closed forms', () => {
  ...
  it('F11 — ASYMMETRIC CREASE: the locator must find the one-sided CLAMP, not just the symmetric CREST', () => {
    // ── THE DEFECT (S113, found with this closed form). The bisection probes AT the midpoint. On a
    // finite-difference surface the midpoint's own window STRADDLES the locus and returns an INTERMEDIATE
    // normal, so the turn is split between the two halves. For a SYMMETRIC crest the split is 50/50 and
    // either half keeps the feature, so the search survives. For an ASYMMETRIC clamp the intermediate
    // normal sits far from the half-angle — the ramp is linear in the SLOPE r_th, and `atan(|r_th|/r)` is
    // not — so the half that actually CONTAINS the locus can read the SMALLER turn and the bracket throws
    // it away. What comes back is a plausible-looking `s` next to a `turn` of 0.000 deg: "no crease here",
    // AT a crease. That is the same silent-and-plausible signature as S74's, and `turn` is the only field
    // that shows it.
    //
    // TWO-SIDED, and the fixture says so: the CREST bar (which the old code passes) and the CLAMP bar
    // (which it fails) are asserted with the SAME closed form and the same tolerances, so a "fix" that
    // simply reports a large turn everywhere fails F12's ceiling, and one that reports zero everywhere
    // fails the floors here.
    const R0 = 40; const A = 2; const thc = 0.3; const w = 0.01;
    const S = tent(R0, A, thc, w);
    const rf = S.r as (th: number, z: number) => number;
    const nsE = exactNormals(S.r, S.rTh, S.rZ);
    const z = 60; const rRef = R0;
    // CLOSED FORM. For a z-independent radial surface phi(th) = th + atan2(-r_th, r), so the turn AT a
    // locus is exactly the difference of the two one-sided atan2 terms taken at the SAME theta.
    const turnCrestDeg = 2 * Math.atan((A / w) / (R0 + A)) * DEG;   // +200 meets -200, at r = R0+A = 42
    const turnClampDeg = Math.atan((A / w) / R0) * DEG;             // -200 meets    0, at r = R0   = 40
    const a0 = 0.292; const b0 = 0.318;          // FULL edge  — contains the crest AND the outer clamp
    const a1 = 0.305; const b1 = 0.318;          // RIGHT HALF — contains the outer clamp ONLY
    const sCrest = (thc - a0) / (b0 - a0);       // 0.30769231
    const sClamp = (thc + w - a1) / (b1 - a1);   // 0.38461538
    log('\nF11 ASYMMETRIC CREASE — tent R0=%s A=%s thc=%s w=%s', R0, A, thc, w);
    log('   CLOSED FORM: crest turn %s deg at s=%s (full edge)   clamp turn %s deg at s=%s (right half)',
      turnCrestDeg.toFixed(6), sCrest.toFixed(8), turnClampDeg.toFixed(6), sClamp.toFixed(8));
    // the INDEPENDENT oracle first, so the truth is not taken on my arithmetic alone
    const oc0 = scanOracle(rf, a0, z, b0, z, rRef, 8192);
    const oc1 = scanOracle(rf, a1, z, b1, z, rRef, 8192);
    log('   SCAN ORACLE (8192 cells, 2-apart): full edge  s=%s turn %s deg   right half  s=%s turn %s deg',
      oc0.s.toFixed(6), oc0.turnDeg.toFixed(4), oc1.s.toFixed(6), oc1.turnDeg.toFixed(4));
    // the locators
    const eCrest = locateTurn(nsE, a0, z, b0, z, 14);
    const eClamp = locateTurn(nsE, a1, z, b1, z, 14);
    const aCrest = locateTurnAdaptive(rf, H, a0, z, b0, z, rRef, 14);
    const aClamp = locateTurnAdaptive(rf, H, a1, z, b1, z, rRef, 14);
    log('   locateTurn (EXACT sampler)      CREST s=%s turn %s deg    CLAMP s=%s turn %s deg',
      eCrest.s.toFixed(8), (eCrest.turn * DEG).toFixed(6), eClamp.s.toFixed(8), (eClamp.turn * DEG).toFixed(6));
    log('   locateTurnAdaptive (FD)         CREST s=%s turn %s deg    CLAMP s=%s turn %s deg  hFinal %s',
      aCrest.s.toFixed(8), (aCrest.turn * DEG).toFixed(6), aClamp.s.toFixed(8), (aClamp.turn * DEG).toFixed(6),
      aClamp.hFinal.toExponential(2));
    log('   ^^ the CLAMP column is the defect: a plausible s beside a turn of ~0 where the oracle reads %s deg',
      oc1.turnDeg.toFixed(2));
    // ── the ORACLE must agree with the closed form, or the fixture is measuring my scan, not the surface
    expect(oc0.turnDeg).toBeGreaterThan(turnCrestDeg * 0.99);
    expect(oc0.turnDeg).toBeLessThan(turnCrestDeg * 1.01);
    expect(oc1.turnDeg).toBeGreaterThan(turnClampDeg * 0.99);
    expect(oc1.turnDeg).toBeLessThan(turnClampDeg * 1.01);
    // ── the SYMMETRIC crest: both locators, two-sided on BOTH s and turn
    for (const [nm, g] of [['exact', eCrest], ['adaptive', aCrest]] as Array<[string, { s: number; turn: number }]>) {
      expect(Math.abs(g.s - sCrest), `${nm} crest s`).toBeLessThan(0.01);
      expect(g.turn * DEG, `${nm} crest turn floor`).toBeGreaterThan(turnCrestDeg * 0.95);
      expect(g.turn * DEG, `${nm} crest turn ceiling`).toBeLessThan(turnCrestDeg * 1.05);
    }
    // ── the ASYMMETRIC clamp: the same bars, on both locators. THIS is what fails before the fix.
    for (const [nm, g] of [['exact', eClamp], ['adaptive', aClamp]] as Array<[string, { s: number; turn: number }]>) {
      expect(Math.abs(g.s - sClamp), `${nm} clamp s`).toBeLessThan(0.01);
      expect(Math.abs(g.s - 0.5), `${nm} clamp s is not just the middle`).toBeGreaterThan(0.05);
      expect(g.turn * DEG, `${nm} clamp turn floor`).toBeGreaterThan(turnClampDeg * 0.95);
      expect(g.turn * DEG, `${nm} clamp turn ceiling`).toBeLessThan(turnClampDeg * 1.05);
    }
    // ── and the two features must be TOLD APART, or "found both" is satisfied by reporting one number
    // twice. The crest is 1.986x the clamp by closed form.
    expect(aCrest.turn / aClamp.turn).toBeGreaterThan(1.8);
    expect(aCrest.turn / aClamp.turn).toBeLessThan(2.2);
  });

  it('F12 — SMOOTH NEGATIVE CONTROLS: no invented crease, and no plausible `s` on a featureless edge', () => {
    // The ceiling half of F11. A locator that answers "crease, full turn" everywhere would pass every
    // floor in F11; these two edges have NO C0 locus at all and pin it from the other side.
    //
    // C1 also pins DEFECT B (orientRuler.ts:529, S99): on a genuinely smooth segment every comparison is
    // a tie, the `>=` sends the bracket LEFT every time, and the locator returns an `s` pinned near the
    // left END of the edge — a number that looks like a located feature and is not. The honest answer on
    // a featureless segment is the MIDDLE, so that is what is asserted, two-sided.
    const R0 = 40; const A = 2; const thc = 0.3; const w = 0.01;
    const S = tent(R0, A, thc, w);
    const rf = S.r as (th: number, z: number) => number;
    const z = 60; const rRef = R0;
    const turnClampDeg = Math.atan((A / w) / R0) * DEG;
    // C1 — the FLAT skirt of the SAME tent: th 0.312 -> 0.317 is entirely at r = R0, no locus in sight.
    const flat = locateTurnAdaptive(rf, H, 0.312, z, 0.317, z, rRef, 14);
    const flatE = locateTurn(exactNormals(S.r, S.rTh, S.rZ), 0.312, z, 0.317, z, 14);
    const ocFlat = scanOracle(rf, 0.312, z, 0.317, z, rRef, 8192);
    // C2 — a short edge on the smooth SIN surface (curved, but C-infinity: no crease to find)
    const SM = sinS(45, 1.5, 24);
    const smf = SM.r as (th: number, z: number) => number;
    const sin1 = locateTurnAdaptive(smf, H, 0.1200, z, 0.1215, z, 45, 14);
    log('\nF12 SMOOTH CONTROLS   (clamp reference from F11 = %s deg)', turnClampDeg.toFixed(4));
    log('   C1 FLAT skirt  adaptive s=%s turn %s deg   exact s=%s turn %s deg   oracle turn %s deg',
      flat.s.toFixed(8), (flat.turn * DEG).toExponential(3), flatE.s.toFixed(8), (flatE.turn * DEG).toExponential(3),
      ocFlat.turnDeg.toExponential(3));
    log('   C2 SMOOTH SIN  adaptive s=%s turn %s deg', sin1.s.toFixed(8), (sin1.turn * DEG).toExponential(3));
    log('   discrimination clamp/flat = %sx', (turnClampDeg / Math.max(1e-12, flat.turn * DEG)).toExponential(2));
    // CEILING: no invented crease, on either locator or the oracle
    expect(ocFlat.turnDeg).toBeLessThan(0.05);
    expect(flat.turn * DEG).toBeLessThan(0.05);
    expect(flatE.turn * DEG).toBeLessThan(0.05);
    expect(sin1.turn * DEG).toBeLessThan(0.50);
    // FLOOR: the reading must still SEPARATE a real locus from a featureless one by a wide margin, so a
    // "return 0 always" implementation cannot pass this pair together with F11's floors
    expect(turnClampDeg / Math.max(1e-12, flat.turn * DEG)).toBeGreaterThan(100);
    // DEFECT B: on a featureless segment the answer must be the MIDDLE, not the left end. Two-sided.
    expect(flat.s).toBeGreaterThan(0.45);
    expect(flat.s).toBeLessThan(0.55);
    expect(flatE.s).toBeGreaterThan(0.45);
    expect(flatE.s).toBeLessThan(0.55);
  });
