// _weaveDG.test.ts — DEV-ONLY. FRONTIER weave STEP 1: validate the DOUBLED-GRID LATTICE primitive (the watertight
// fix). ONE regular (row x col) lattice; C0 step cliffs carried by doubled grid lines with radius overrides on the
// cliff rungs. Full scorecard. TARGET: int true-3D worst <= 0.012, %<20 < ~5%, rawNonMan 0.
//
// Env PF_WEAVE=1; PF_WEAVE_HROW; PF_WEAVE_WTGT; PF_WEAVE_CLIFF; PF_WEAVE_SUB; PF_WEAVE_SEAM; PF_WEAVE_RENDER.
...
describe.skipIf(!RUN)('WEAVE doubled-grid validate — watertight lattice + radius-override cliffs', () => {
  it('build BasketWeave via doubled-grid; full scorecard (resumable)', () => {
    ...
    const sub = process.env.PF_WEAVE_SUB ?? 'dg1';
    const name = `dg_BasketWeave_${sub}_h${String(hRowMm).replace('.', 'p')}`;
    ...
    const build = buildWeaveDoubledGrid(rA, DIMS.H, grid, { hRowMm, wTargetMm, cliffChordMm, seamMode });
    const mesh = build.mesh; const idxA = mesh.idx; const nF = mesh.nF;
    console.log(`BasketWeave DG: seam=${build.seamMode} rows=${build.nRow} cols=${build.nCol} tris=${nF} verts=${mesh.nV} (${((Date.now() - t0) / 1000).toFixed(1)}s)`);
    ...
    const rec = {
      style: 'BasketWeave', primitive: 'doubled-grid', config: name, hRowMm, wTargetMm, cliffChordMm, seamMode: build.seamMode, elapsedSec: +((Date.now() - t0) / 1000).toFixed(1),
      rows: build.nRow, cols: build.nCol, tris: nF, verts: mesh.nV,
