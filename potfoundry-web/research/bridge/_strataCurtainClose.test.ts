// _strataCurtainClose.test.ts — FORK of _strataConformBisect.test.ts (@022b2c3d) + the GENERIC θ-CURTAIN (h0 closure).
// Gated PF_STRATA_CURTAIN=1. RESEARCH ONLY — never touches src/, never touches the parent harness.
//
// ────────────────────────────────────────────────────────────────────────────────────────────────────────────────
...
const RUN = process.env.PF_STRATA_CURTAIN === '1';
...
describe('STRATA conforming-bisection + θ-curtain', () => {
  it.runIf(RUN)('meshes any style by feature-directed conforming bisection with double-valued θ curtains', () => {
    ...
    const BR_EVAL = CURTAIN || envOn('PF_CB_CURTAIN_EVAL'); // branch-aware analytic evaluation (ruler + detectors)
    ...
    const addV = (thetaRaw: number, z: number, feat = false, br = 0, locTh = NaN): number => {
      ...
      vLocTh.push(Number.isNaN(locTh) && BR_EVAL ? locusNear(theta) : locTh); vBranch.push(br);
      const key = `${cx},${cy},${cz}`;
    ...
    const sagOfN = (t: number, n: number): number => {
      ...
      let locU = NaN; let locB = 0;
      if (BR_EVAL) {
        if (onLoc(a)) locU = th0; else if (onLoc(b)) locU = th0 + dB; else if (onLoc(c)) locU = th0 + dC;
        if (!Number.isNaN(locU)) {
    ...
    for (let b = 0; b + 1 < bounds.length; b += 1) {
      ...
      for (let j = 0; j <= rows; j += 1) {
        ...
        for (let i = 0; i < nc; i += 1) {
          if (!colLoc[i]) { const v = addV(colTh[i], z); rowL.push(v); rowR.push(v); continue; }
          const th = colTh[i];
          ...
          if (sep < PINCH_MM) {
            ...
            const v = addV(th, z, true, 0, th);
            if (addVNew) pinchVerts += 1;
            rowL.push(v); rowR.push(v); continue;
          }
          const vM = addV(th, z, true, -1, th);
          const vP = addV(th, z, true, +1, th);
      ...
      for (let j = 0; j < rows; j += 1) for (let i = 0; i < nc; i += 1) {
        const i1 = (i + 1) % nc;
      ...
      if (CURTAIN) for (let i = 0; i < nc; i += 1) {
        if (!colLoc[i]) continue;
        for (let j = 0; j < rows; j += 1) {
    ...
    const bisectAt = (a: number, b: number, tPar: number, feat: boolean): boolean => {
      ...
      const m = br !== 0 ? addV(mth, mz, true, br, vLocTh[a]) : addV(mth, mz, feat);
    ...
    const stitchRings = (loopA: P3[], loopB: P3[]): number => {
      if (CURTAIN) return stitchWalk(loopA, loopB);
