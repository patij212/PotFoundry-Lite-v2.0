...
// L-FORK ONLY. The constrained edge-flip repair pass, DEFAULT OFF (PF_LAND_FLIP). See §5.3b below.
import { landConstrainedFlip, type LandPosRuler } from '../tools/landFlipPass';
...
describe('STRATA conforming-bisection', () => {
  it.runIf(RUN)('meshes any style by feature-directed conforming bisection', async () => {
    ...
    // ══════════ S34 — SEED-TIME CHAIN RE-SOLVE. PF_CB_ALIGNED_RESOLVE_UM, DEFAULT 0 = OFF ══════════
    // THE ONLY BEHAVIOURAL DIFFERENCE BETWEEN THIS FORK AND ITS PARENT. Passes the builder's stage
    // 1a-bis lever (committed 71d76c39) through to the seed call below. At 0 the two options are
    // absent from the object, so the arm is the unforked driver's arithmetic exactly — which is what
    // the A/B's control row has to demonstrate before its treatment row means anything.
    //
    // Seed-stage A/B (s34ResolveSeedAB.ts): actionable crease crossings 9,363 -> 1,392 (-85.1%) at
    // 50 um, zero PSLG planarity breaks, overCap and worstAR IDENTICAL, +9.4% seed triangles.
    // ⚠ 50 IS THE MEASURED SHIPPING VALUE. 100/200 cut the residual further but BREAK PSLG planarity
    //   (13/10) — a wider transverse probe reaches a NEIGHBOURING locus.
    ...
    if (ALIGNED_SEED) {
      ...
      const rep = buildAlignedSeedRepaired(rA, alignedLoci, {
        ...
        // S34: absent at 0, so the control arm's options object is byte-identical to the parent's.
    ...
    // ─── §5.3b THE CONSTRAINED FLIP (L-FORK ONLY, PF_LAND_FLIP, DEFAULT OFF) ────────────────────────────
    // A connectivity-only orientation repair over the FINISHED soup: it re-cuts diagonals and moves ZERO
    // vertices, adds ZERO triangles, and preserves edge / boundary / non-manifold / winding counts exactly.
    // Measured offline on this style's own S39CTL output: orientation over-10um 129,757 -> 37,157 (3.49x;
    // 3.73x by AREA), theta>90deg 1,506 -> 828, topology byte-identical. REFUTED on Voronoi (1.33x) — that
    // class is 124,245 high-aspect MIS-ORIENTED facets, which a diagonal re-cut cannot repair.
    //
    // PLACED HERE, AFTER EVERY AUDIT, ON PURPOSE. The driver's own numbers (`headlineMax`, `sagNowUm`, the
    // unresolved list, `liveIdx`) are computed upstream against `ta/tb/tc`; running the pass earlier would
    // silently desync them from the mesh that ships. So the manifest's provenance columns (nTri, alloc,
    // unresolvedLeft) stay EXACTLY the flag-OFF values — which is precisely what makes the A/B attributable
    // to this one flag — and every driver-reported fidelity number in an ON run is a PRE-FLIP number and is
    // labelled as such in `<tag>.landflip.json`. The post-flip verdict is taken offline by the audit tools
    // (s60ConstrainedFlip for orientation, s80HonestPos for the H1 position re-score), never self-reported.
    //
    // BYTE-IDENTICAL WHEN OFF: the block below does not execute and `soup` is untouched.
    const LAND_FLIP = process.env.PF_LAND_FLIP === '1';
    if (LAND_FLIP) {
      const lfRuler = (process.env.PF_LAND_FLIP_RULER ?? 'plane') as LandPosRuler;
      const lfSel = Number(process.env.PF_LAND_FLIP_SEL_UM ?? 0);
      const lfRounds = Number(process.env.PF_LAND_FLIP_ROUNDS ?? 40);
      const lfBar = Number(process.env.PF_LAND_FLIP_BAR_UM ?? 10);
      const flat = new Float64Array(soup.length * 9);
      for (let t = 0; t < soup.length; t += 1) {
        const [a, b, c] = soup[t];
        flat[t * 9] = a[0]; flat[t * 9 + 1] = a[1]; flat[t * 9 + 2] = a[2];
        flat[t * 9 + 3] = b[0]; flat[t * 9 + 4] = b[1]; flat[t * 9 + 5] = b[2];
        flat[t * 9 + 6] = c[0]; flat[t * 9 + 7] = c[1]; flat[t * 9 + 8] = c[2];
      }
      const lfStats = landConstrainedFlip(flat, soup.length, {
        rA: (th: number, z: number): number => rA(th, z), H,
        barUm: lfBar, jbarUm: 1, rounds: lfRounds, posRuler: lfRuler, h1SelectUm: lfSel,
        useDet: true, detMode: 'rel', gateMm: 0.05, nMax: 512,
        // eslint-disable-next-line no-console
        log: (s: string): void => { console.log(s); },
      });
      for (let t = 0; t < soup.length; t += 1) {
        soup[t] = [
          [flat[t * 9], flat[t * 9 + 1], flat[t * 9 + 2]],
          [flat[t * 9 + 3], flat[t * 9 + 4], flat[t * 9 + 5]],
          [flat[t * 9 + 6], flat[t * 9 + 7], flat[t * 9 + 8]],
        ];
      }
      writeFileSync(join(outDir, `${tag}.landflip.json`), JSON.stringify({
        schema: 'pf.strata.landFlip/1',
        note: 'Connectivity-only orientation repair applied AFTER every driver audit. The driver\'s own '
          + 'fidelity columns in <tag>.run.json / the report are PRE-FLIP. The post-flip verdict is taken '
          + 'offline by s60ConstrainedFlip (orientation) and s80HonestPos (H1 position), never self-reported.',
        opts: { ruler: lfRuler, selUm: lfSel, rounds: lfRounds, barUm: lfBar },
        stats: lfStats,
      }, null, 1));
    }
    ...
    const report = [
      ...
      ...(ALIGNED_SEED && alignedStats !== null && alignedLoci !== null ? [
        ...
        `    *** S34 SEED-TIME CHAIN RE-SOLVE: PF_CB_ALIGNED_RESOLVE_UM=${(AL_RESOLVE_MM * 1000).toFixed(0)}`
          + `${AL_RESOLVE_MM === 0 ? ' (OFF — this is the control arm)' : ''}`
