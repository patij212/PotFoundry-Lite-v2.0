...
(async function(){
  ...
  for(let i=0;i<cells.length;i++){
    ...
    // S50 GRAZING / SILHOUETTE VARIANT: look ALONG the tangent at the pot's left limb, so the wall is
    // seen edge-on and a facet standing proud of it projects as a spike against the background —
    // the view the operator was using in Cura. Near-orthographic (narrow FOV, far camera).
    const Rp=sz.x/2;
    const cam=new THREE.PerspectiveCamera(9,PW/PH,0.1,R*40);
    cam.up.set(0,0,1); cam.position.set(-Rp*0.985,-R*7,0); cam.lookAt(-Rp*0.985,0,0);
