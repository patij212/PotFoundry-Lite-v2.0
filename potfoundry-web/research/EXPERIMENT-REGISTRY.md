...
**VERDICT: REFUTED** (13th refuted sliver lever). Three measured units on the CONFIRMED whole-mesh Gothic mesh (30,323t, 0-outlier, 19% <20°), honest full-azimuth brute throughout:

- **(diag) The task premise ("last 1-2 rings at the apex") is REFUTED by localization.** The 5757 needle facets (19%) are NOT a thin apex ring: **5123 (89%) are high-gradU crest-flank** (needle centroid gradU p10/p50/p90 = 54.9/133.8/204) forming just **6 large connected clusters** (~850 facets each), plus 634 low-gradU panel needles. A "scoped apex-only" element has no thin apex ring to target — the needles are the whole near-vertical crest-flank BAND.
- **(apexPN re-tess) NO-OP on slivers; HELD fidelity+watertight.** buildApexPnTess re-tessellated only isolated simple-boundary clusters (4 retess / 2 skipped) → touched **5 of 5757 needle facets** (large clusters have non-simple/pinched boundaries → safe-skip). pctBelow20 **19.0→19.0**, minAngle 0→0. wholeMeshOutliers **0** (max 0.01, p99 0.00824, full 45-pt acceptanceGuardWhole, 6321 brute calls), watertight nonMan=0 non-vacuous (inj 0→1). The mechanism did not engage the population.
- **(pnflip, DECISIVE element-level discriminator) The one-sided Vlachos PN element is WORSE than the flat chord at riding the Gothic cusp.** Over 6069 internal near-apex-needle edges, 1801 flips are rounder (beforeMin p50 7.0° → flippedMin p50 18.4°), but scoring the flipped facets' true-3D interior dev: **flatDev p50/p90 = 0.0023/0.1287 vs pnDev p50/p90 = 0.0285/0.2602** — the curved PN patch is ~12× WORSE at p50 and 2× worse at p90. **nPnRidesFlat = 3/200** (PN within-tol where flat exceeds it in only 3 of 200 cases); nFlatWithinTol = 160/200. ROOT CAUSE: the Vlachos PN cubic is built from the near-vertical flank's surface normals, which make it OVERSHOOT the concave `pow(sharp)` knife-edge rather than follow it — a convex-biased element cannot represent a concave cusp. **The "curved element rides the cusp without chording" premise is FALSE for this geometry.**

**Net:** the scoped apex-PN lever fails on BOTH counts of the kill-criterion — it cannot even ENGAGE the needle population (they are a connected crest-flank band, not apex rings), and where a rounder connectivity IS available the PN element does not hold fidelity better than the flat chord it was supposed to beat (it is worse). This UPHOLDS the V8 structural-tension conclusion from the ELEMENT side: at the zero-width concave cusp the fidelity-holding cells are needles, and a convex-normal PN patch does not rescue them. The honest pctBelow20 at whole-mesh 0-outlier stays **19%** (Gothic remains print-usable-with-finite-area-needles + collapse, the documented concession). Render: `research/exchange/_pf_apex_pn/apexpn_true3d.png` (crest-flank band fidelity-green, worst 0.020mm — the needle zone is on-surface, confirming the tension is angle-only).

**FOLLOW-UP (not this lever):** the 80% of rounder flips that ARE flat-fidelity-safe (nFlatWithinTol 160/200) suggests a FIDELITY-GUARDED flat flip (flip only where flatDev≤tol) could shave some panel/moderate needles — but that is the V2-§3b Lawson-flip lever (already REFUTED: guarded flips reopened 0→57 outliers because the guard understated true-3D). The genuinely untried representation move remains a CONCAVE-aware element (e.g. a subdivision-surface or a normal-corrected/one-sided quadratic that respects the cusp sign), NOT the convex Vlachos PN — but the element bench here shows Vlachos PN specifically is a dead end for concave cusps.
...
**VERDICT: REFUTE — surface-preserving Laplacian-under-M relaxation is a MARGINAL reducer that PLATEAUS at ~20% <20°, and it REOPENS whole-mesh outliers; the OFF-crest panel needles are a CONNECTIVITY floor (seed+free-cdt2d), NOT a free-vertex-placement artifact. 13th refuted sliver lever.** GeoStar 5-bay whole-mesh (58969v/116889t, patch fam=2 crestV=956), honest resumable whole-mesh guard (EVERY free facet, 45-pt brute nTheta=1024):

| gate | BEFORE (CONFIRMED wholemesh) | AFTER (relax-Laplacian-under-M-qdir, 3 sweeps) | verdict |
|---|---|---|---|
| wholeMeshOutliers (true-3D >0.01, honest whole-mesh brute) | 0 | **427** (max 0.0123mm, onCrest 236 / off 191) | **REOPENED** ✗ |
| minAngleDeg | 0 | 2.5° | — |
| **pctBelow20** | 21.7% | **20.0%** (internal quality snapshot plateaued 19.74→19.74→19.99% over the 3 sweeps) | **FAIL** ✗ (−1.7pt, ≫ single-digit) |
| watertight auditNonManByIndex | 0 (nonVac inj 0→1) | 0 (nonVac inj 0→1) | HELD ✓ |

- **Sliver CONFIRM UNMET → REFUTE (both branches).** (1) pctBelow20 21.7→20.0% is a MARGINAL reduction that PLATEAUS (sweeps 1/2/3 → 19.74/19.74/19.99% — sweep 2 identical to sweep 1, sweep 3 slightly WORSE; minAngle 0→2.5° only), nowhere near single-digit. (2) The move guard (`facetInteriorBrute` per-move reject at coarser nTheta=384) let ~57% of moves through (moved 28–35k of 56972 free; **43% guard-REJECTED**), but those accepted moves drift free panel verts to the tol BOUNDARY where the honest nTheta=1024 whole-mesh guard catches **427 marginal reopenings** (max 0.0123, just over 0.01). Both the "single-digit %<20" and the "HOLD 0 outliers" pre-registered CONFIRM conditions FAIL.
- **ROOT CAUSE (measured):** GeoStar's OFF-crest panel needles are a CONNECTIVITY floor baked into the seed+free-cdt2d point set (the `_pf_geostar_hybrid_diag` finding: the 91%-off-crest sliver set is IDENTICAL on the pre-refine brute mesh). Relaxation MOVES the free panel verts (43% rejected by the fidelity guard) but the connectivity-locked needles persist — the SAME mechanism that refuted Lever#2 on Gothic (E-…-RELAX: 56.2→55.0%). GeoStar's panel class is NOT the wrong-ruler/accept class either — minAngle 2.5° is a genuine 3D needle. The relaxation reject-guard (43% rejects) also REFUTES the "near-isotropic panel ⇒ freely relaxable" premise: enough panel verts sit where a metric-equilateral move over-chords the local relief that the guard blocks the needle-fattening.
- **12→13 sliver levers now refuted.** Density / placement / connectivity-flips / strips / hybrid / graded-seed / M-square / Laplacian-under-M (both styles) are EXHAUSTED. The gate table (SLIVERS = the ONE open gate) is unchanged; GeoStar's off-crest panel class is now shown to resist the LAST placement/relaxation lever the V8 synthesis pointed to.

**HONESTY CAVEATS:** (a) the finalized mesh's "sweeps:4" label is an accumulator artifact — the sweep telemetry shows exactly 3 clean sweeps from a single node instance (one lifetime survived ~9min through the env's ~3.5min watchdog); pctBelow20 measured internally at sweep 3 = 19.99%, the honest whole-mesh guard reads 20.0%. (b) The per-move reject-guard ran at nTheta=384 (coarser, for per-move tractability) while the final verdict guard is the honest nTheta=1024 whole-mesh brute — the 427 reopenings are the coarse-vs-honest ruler gap on drifted panel verts, itself a finding: a relaxation guard MUST use the honest ruler or it drifts verts past tol. (c) No visual render was produced (metric unambiguous: minAngle 2.5°, 20% <20°, plateaued). (d) The graded-seed arm was NOT run — the identical mechanism was a WASH on Gothic (Lever#1, 56.2→56.5%) and the relaxation refute already establishes the connectivity-floor root cause.

**RECOMMENDATION:** Accept the 13th refutation and re-scope. GeoStar's off-crest panel needle class is a CONNECTIVITY floor of the seed+free-cdt2d, NOT closable by any vertex-placement/relaxation lever (now proven, matching Gothic's crest-flank class from the OTHER class). Two remaining moves, both PRIMITIVE-level and substantial: (i) an explicit structured-quad PANEL re-mesher that bypasses free cdt2d on the smooth panel between chevrons (the connectivity that bakes the needles) — analogous to the crest-strip idea but on the PANEL; (ii) ACCEPT+DOCUMENT the finite-area panel needles as the print-usable concession (GeoStar is already literal-0 fidelity + watertight + zeroArea=0 whole-mesh). Given 13 refuted levers across both styles and both sliver classes (crest-flank AND off-crest panel), the honest campaign state is that the sliver gate is a STRUCTURAL property of flat-P1-under-free-cdt2d, and the print-usable concession is the pragmatic close.

**LEDGER:** probe `research/bridge/_pf_perfect_geostar_relax.test.ts` (PF_GSRELAX=1) + standalone resumable runner `research/bridge/_pf_gsrelax_runner.mts` (esbuild-bundled → node, per-chunk resumable whole-mesh guard to survive the env's ~3.5min watchdog), lib `_pf_relaxLib.ts` (relaxLaplacianUnderM + gradedSeed, banked) + `_pf_wholeMeshGuardLib.ts` (facetInteriorBrute45), config `vitest.pf_gsrelax.config.ts`. Scorecard `research/exchange/_pf_perfect_geostar_relax/{before,after}.json` + scorecard.ndjson + after_sweeps.ndjson. DEV-ONLY; no src/ edit.
...
**VERDICT: REFUTE — the localized apex refine CANNOT restore 0-outlier; the frozen worst FLOORS at ~0.39mm (honest brute), and the diagnostic proves WHY: the direct strip places a crest-COLUMN facet that STRADDLES the apex ridge, and a RED 1→4 of an apex-straddling facet keeps an apex-straddling child ⇒ the `pow(sharp)` chord is un-removable by local subdivision (needs a cdt2d flip — forbidden, re-needles — or a curved element AT the apex leaf).** Gothic, 1-bay/4mm, hCrest=dtRow=0.15 (the single-digit-sliver config). Three measured arms:

- **(a) GN-anchored STOP (fast) — terminated BLIND, guard exposes 67 outliers.** Loop reached "0 outliers" (worst 0.0097) under a 45-pt GN-anchored STOP, watertight nonVac ✓, zeroArea=0, pctBelow20 10.5%, 4251 tris. But the honest post-loop full-azimuth-brute `acceptanceGuardWhole` (nTheta=1024) read **wholeMeshOutliers=67, max 0.21941mm** at HIGH gradU (124–178 = the near-vertical apex flank). GN's single-seed descent lands in the WRONG azimuthal foot at the zero-width apex and UNDER-states the chord ~20× (0.0097 vs 0.219) — the documented steep-flank GN gotcha (LAB-CHEATSHEET; the registry's "GN EXACT on single-valued Gothic" holds on the panel, NOT at the pow(sharp) apex). ⇒ a GN STOP is invalid here.
- **(b) FULL-AZIMUTH-BRUTE STOP (honest, nTheta=192–320) — the worst FREEZES at ~0.39mm.** With the honest brute driving splits, outliers drop 410→341→168 but the **worst is FROZEN across passes: 0.4709→0.3854→0.39263→0.39262** while tris GROW 4543→8009 (a whole-scan pass 4 re-found 342, proving the incremental set missed re-straddled facets). This is the pre-registered REFUTE condition (worst floors ≫0.02 after the loop, never reaches 0). No amount of local RED reduces it.
- **(c) DIAGNOSTIC (`_pf_hybrid_diag`, near-crest full-azimuth brute) — the frozen worst is an apex-STRADDLING crest-column facet.** The worst strip facet (0.203mm, at u≈0.0258, t≈0.748) has **all 3 vertices near-crest** (radii 47.65/48.00/47.66 vs rMean 45 — the apex spikes ~3mm). It STRADDLES the apex ridge. A RED 1→4 of an apex-straddling facet emits a corner child that STILL touches the apex vertex ⇒ still chords the zero-width cusp ⇒ the chord cannot shrink below the `pow(sharp)` floor by subdivision alone.

**WHY the hybrid floors where the CONFIRMED WHOLEMESH edge-mode reached 0:** the WHOLEMESH kernel (E-…-WHOLEMESH-GOTHIC) reached 0-outlier because it seeds a metric-Delaunay mesh with the crest LOCKED as constraint edges and **re-triangulates with cdt2d every pass** — cdt2d FLIPS the apex-straddling facet into two flank-only facets that meet AT (not across) the apex. The direct strip forbids cdt2d (that re-chord is what re-needled every prior strip), so its fixed crest-column connectivity BAKES IN an apex-straddling facet that local RED cannot flip away. The two halves are thus INCOMPATIBLE: the strip's clean structured connectivity is EXACTLY what prevents the local refine from un-straddling the apex; only cdt2d (→needles) or a curved element (E-CRESTRIBBON one-sided PN/P2 at the apex leaf) can. This is the 12th refuted sliver lever and it CLOSES the "graft strip+brute-apex" move named in E-…-CRESTSTRIP-DIRECT §NEXT: it does NOT co-resolve — the fidelity↔angle tension at the zero-width pow(sharp) apex is a PROVEN flat-P1 wall requiring an ELEMENT change, not connectivity/density/placement.

**RECOMMENDATION:** The hybrid is the last density/placement/connectivity lever and it is REFUTED. The ONLY remaining fidelity+angle-closing move for Gothic's zero-width apex is a CURVED element: the E-CRESTRIBBON one-sided PN/P2 element AT the apex-straddling leaf (benched UNNECESSARY for fidelity when the strip is dropped for cdt2d — now shown NECESSARY to hold BOTH gates on the structured strip). Alternatively ACCEPT+DOCUMENT the concession banked by the campaign: ship the CONFIRMED WHOLEMESH edge-mode (literal-0 fidelity + watertight + zeroArea=0 via collapse, 19% <20° minAngle 0 finite-area needles = print-usable-with-needles) OR the direct strip (0.4% angles + 0.22mm apex chord) — the two INVERT the tension and neither closes both by a flat element. GeoStar hybrid NOT RUN (Gothic REFUTE is a representation floor at the zero-width apex; GeoStar's finite-width kink may differ but the strip already overlaps sub-pitch there — E-…-GEOSTAR-STRUCTSTRIP — so the strip substrate itself fails first on GeoStar).
...
**KILL-CRITERION (committed BEFORE measuring):** CONFIRM iff pctBelow20 → SINGLE DIGITS (<~10%) AND minAngle sane WHILE the honest WHOLE-MESH brute holds interiorOutliers=0 (EVERY free facet, 45-pt) AND zeroAreaFaces=0 AND watertight non-vacuous. REFUTE iff the strip cannot hold 0-outlier OR cannot reach single-digit %<20 (report the honest %<20 at outliers=0).

**VERDICT: REFUTED — by a NEW mechanism (NOT the V6 re-CDT one, which the direct emission correctly avoids).** The direct-emission strip does bypass cdt2d (bgTris=0, no Delaunay re-chording of strip interiors — the V6 defeat is eliminated). But the strips OVERLAP and interpenetrate on GeoStar's dense count-unstable chevron field:

- **Cheap discriminator (E-…-GSSTRIP-SPACING, `_pf_gsstrip_spacing.test.ts`):** on the GeoStar band (3-bay, t≈0.08, uSpan 26.5mm, 213 crest verts) the nearest-OTHER-crest 3D distance is **min 0.010 / p10 0.046 / p50 0.088 / mean 0.117 / max 0.265 mm** ⇒ max non-overlapping strip HALF-width ≈ p50/2 = **0.044mm = 0.7 columns at h=0.06mm**. The chevron straps are packed so tightly the "flank" between adjacent crests is SUB-PITCH — no square column fits between two straps.
- **Measured build (`_pf_geostar_structstrip.test.ts`, PF_SMOKE=1, base = the CONFIRMED whole-mesh 0-outlier mesh):**
  - width 0.9mm, no clamp → 1,057,434 strip tris, **watertightNonMan = 284,102** (massive interpenetration), pctBelow20 17.3%, zeroArea 99. FAST-REFUTE (guard not run — a non-manifold mesh cannot CONFIRM).
  - width 0.9mm, valley-clamped at the Voronoi midline → 737,520 strip tris, **watertightNonMan = 219,536**, pctBelow20 15.7% (from 21.7%), minAngle 0, zeroArea 0. Still massively non-manifold: valley-clamping stops each strip at the midline but adjacent strips do NOT place SHARED boundary vertices there (their perpendicular marches from different crests reach different (u,t) points), so faces interpenetrate instead of sharing edges.

**ROOT CAUSE (measured, style-agnostic for dense count-unstable fields):** a direct-emission structured strip assumes strips are SPARSE relative to feature spacing. GeoStar's chevron field has median crest 3D spacing 0.088mm — a strip wide enough to carry ≥1 square column (≥0.06mm) overlaps its neighbour. The needles the strip was meant to remove are INTRINSIC to this spacing: with the crest a mesh edge and the flank sub-pitch, any triangle spanning crest-to-crest is either a chord (kills fidelity, forbidden) or a needle (thin because along-crest extent ≫ across-crest extent) — the same fidelity-vs-angle structural tension E-…-ANISO-RULER measured (76° cross-curvature needles). **This is now the 9th/10th refuted sliver lever; direct emission removes the V6 re-CDT defeat but exposes an overlap wall on the dense field.** pctBelow20 did improve 21.7→15.7% (the strip is squarer WHERE it fits) but never watertight and never single-digit ⇒ REFUTE on all three counts (not watertight, not single-digit, guard moot).

**RECOMMENDATION:** The sliver gate on GeoStar's dense chevron field is NOT closable by strips-over-a-fixed-crest-graph (they overlap). The next primitive is a **valley-partitioned SHARED-boundary mesh**: build ONE structured mesh per flank BAND between two consecutive crests (a warped quad grid whose two u-edges ARE the two bounding crests, so adjacent bands SHARE the crest columns → watertight by construction, no overlap), rather than independent per-crest strips. This is a band-parametrization, not a strip; it needs the count-unstable birth/death handled per-band (a band ends where a strap dies). Deferred: substantial build; the overlap refutation is the closing result for the direct-emission-strip lever.
...
### RESULT (2026-07-05) — CONFIRMED: GeoStar reaches LITERAL whole-mesh 0 (the "0" WAS a guard-population artifact; residuals were under-refinement, NOT a wall)
...
**The guard-population artifact was FAR bigger than the spec's "~3-4" estimate — 791, not 4.** WHOLE-MESH guard on the CONFIRMED mesh (before): **wholeMeshOutliers = 791** (max 0.02954, onCrest 289 / off 502, p99 0.00961, brute 52465/112863 facets), at **outlierGradU 0.41–207.17** — i.e. the residual outliers live at MODERATE-to-LOW gradU, ENTIRELY below the top-400 guard's population floor (gradUmin 232.94). The top-400-worst-gradU guard scored 0 because it never looked at the moderate-gradU flank facets carrying 0.01–0.03mm interior sag. This RECONFIRMS + quantifies the GEOSTAR-CRESTSTRIP metrology catch at whole-mesh scale.

**AFTER whole-mesh refine (5 passes, edge-mode 1→4, honest-brute STOP over ALL facets):** `wholeMeshOutliers = 0` (raw max 0.01 = the single worst facet AT the tolerance line, 0 facets strictly > 0.01), `onCrest 0 / off 0`, guardP99 0.00875, guardP50 0.00003, outlierDetail empty. **reachesLiteralZero = TRUE.** Watertight `auditNonManByIndex = 0` NON-VACUOUS (injected crack 0→1). `finalTris = 116889`, converged (capped=false).
- Pass trace (worst / nOut): p1 whole-scan 0.02954/791 → p2 incr 0.01109/6 → p3 incr 0.01151/2 → **p4 whole-scan 0.01096/10** (caught 10 facets a DISTANT re-triangulation reshaped — the periodic whole-scan honesty guard firing exactly as designed) → p5 0.00599/0. Mesh grew 112863→116889 (+4026 tris, +2016 verts) — a SMALL delta ⇒ the residuals were genuine under-refinement, NOT a representation wall (max started at 0.03, halved by edge-refine to ≤0.01 in a few passes).
- **RENDER:** `research/exchange/_pf_perfect_geostar_wholemesh/geostar_wholemesh_true3d.png` — whole patch GREEN under the true-3D perpendicular ruler (render's own coarse facet ruler: p99 0.018 / worst 0.020 / 0.00% >0.03). No red on the chevron strap flanks. The metric (0 outliers, max 0.01) and the render AGREE.
- **SLIVERS (honest caveat, NOT in kill-criterion — SEPARATE open gate per E-…-ANISO-RULER):** minAngle 0°, median 41°, pctBelow20 21.7% (≈ the CONFIRMED baseline's 21.3% — the whole-mesh refine did NOT worsen slivers, since edge 1→4 on moderate-gradU flanks is squarer than the apex-knife-edge refine). Still print-usable-adjacent but not angle-clean; needs the metric-aware refine-time spacing lever (V4-flagged), unchanged by this gate.

**COST/HONESTY:** whole-mesh guard/scan is ~30 min/pass over 112–117k facets, but the two-stage GN-screen→brute ruler keeps only the near-crest tail reaching the brute (52465→37135 brute calls before→after, ~35% of facets). Incremental passes (changed-cavity active set) are seconds-to-minutes; whole-scans every 4th pass + the post-loop guard provide the honest literal-0 proof. The env killed the run once mid-loop (proven resilience scenario) — per-pass mesh + ndjson checkpoints resumed it cleanly (before-guard banked to `guard_before.json`, mesh to `refined_mesh.bin`).

**WHAT THIS CLOSES:** GATE 1 (FIDELITY-TO-LITERAL-0) for GeometricStar. Flat-P1, honest whole-mesh brute, EVERY facet ≤0.01, watertight non-vacuous. The E-…-GEOSTAR "0 outliers" is now upgraded from top-400-guard to LITERAL whole-mesh 0 (116889 tris). The tournament/spec claim "flat-P1 follows the count-unstable cusp to CAD-grade true-3D" is VINDICATED at whole-mesh (not just top-gradU-population) honesty. RECONFIRMS the mandate: any future 0-outlier claim on any style MUST use the whole-mesh guard, not top-N-gradU (banked `wholeMeshGuard`). OPEN (unchanged): slivers (pctBelow20 21.7%, needs metric-aware spacing); 5-bay+ whole-MESH scale; Gothic whole-mesh literal-0 (the SAME whole-mesh guard should be run on the CONFIRMED Gothic mesh — likely a LARGER artifact given its zero-width apex).
...
**HYPOTHESIS (falsifiable):** On a multi-bay GeoStar patch, replacing the greedy flat-P1 crest-flank refinement with the PN-guided structured square strip (`refineCrestStrip`) drops `pctBelow20` to single digits + a sane minAngle WHILE `interiorOutliers` HOLD 0 (honest ≥36-pt full-azimuth brute) + `zeroAreaFaces`=0 + watertight non-vacuous.

**DISCRIMINATOR (cheapest that can refute):** `makeGeoStarPatch(bays, zBand, tCenter=0.08)` → the SAME extractProtectedComplex/seedMesh, then `refineCrestStrip` (the VERBATIM Gothic strip kernel) instead of the CONFIRMED refineInteriorBrute. Score with `acceptanceGuard` (≥36-pt brute, worst-gradU top-400) AND — banked from the CRESTSTRIP metrology catch — a WIDER whole-population guard (the top-400 was proven a population artifact blind to moderate-gradU residuals). Slivers = `triangleQualityDistribution` minAngle/pctBelow20. Zero-area = `countZeroAreaFaces`. SMOKE 1-bay for a cheap direction read; multi-bay confirm.

**KILL-CRITERION (pre-registered, committed BEFORE measuring — SAME as CRESTSTRIP):**
- FULL CONFIRM iff `pctBelow20` → single digits (<~10%) AND minAngle sane (>~15° OR median >~30°) AND `zeroAreaFaces`=0 AND `interiorOutliers` HOLD 0 (honest brute, BOTH top-400 AND wider population) AND watertight non-vacuous.
...
**INSTRUMENTS (honest):** interior ruler = `acceptanceGuard` (≥36-pt barycentric, full-azimuth `bruteNearestOnRadialSurface`, worst-gradU) — report BOTH top-400 and a wider population (CRESTSTRIP proved the top-400 blind to moderate-gradU residual outliers). Slivers = `triangleQualityDistribution` minAngle (iso 3D). Zero-area = `countZeroAreaFaces` (<1e-9 mm² + <1e-6 tally). Watertight = `auditNonManByIndex` NON-VACUOUS (injected crack must move the count).
...
**HYPOTHESIS (falsifiable):** On the FREE (non-crest) triangles of the two CONFIRMED 0-outlier meshes — Gothic `_pf_perfect_gothic_msquare/after_mesh.bin` (58365t) + GeoStar `_pf_perfect_geostar_brute/refined_mesh.bin` (112863t) — the isotropic-<20° facets are ANISOTROPY-APPROPRIATE (near-equilateral under the curvature-adapted anisotropic chord metric M from `creaseAlignedMetric.buildCreaseAlignedMetric`, the geometrically-correct (II,I)-generalized metric), FINITE-area (no near-zero-area degenerates), and STL/slicer-SAFE ⇒ ACCEPT+DOCUMENT (the perfect mesher is quality-COMPLETE for the print use case; isotropic %<20 was the wrong ruler). If instead they are degenerate under M too / near-zero-area / non-manifold ⇒ REAL DEFECT needing the scoped one-sided PN crest-flank element.

**DISCRIMINATOR (cheapest — REUSE the 0-outlier meshes, NO new meshing):** load both persisted meshes; reconstruct their PatchDefs verbatim (Gothic `makeGothicPatch(4,12)`; GeoStar `makeGeoStarPatch(5,10,0.08)`); classify each triangle crest-incident vs FREE via the same crest-vertex set the kernel built; then measure, PER TRIANGLE, on the free set: (1) **M-metric min-angle** — angle of the (u,t) edges in the metric M at the triangle centroid (acos of the M-inner-product of the edge pairs), M = the curvature-adapted anisotropic tensor; report the M-metric minAngle distribution vs the isotropic (3D) one. CONTROL: M = first-form g alone must reproduce the isotropic 3D angle (proves the instrument, since chart-under-I ≡ lifted-3D). (2) **AREA** distribution (mm² of the lifted 3D triangle) + count of near-zero-area (< εmm², ε from the printer floor) / degenerate faces. (3) **STL-safety** — degenerate / duplicate-vertex / non-manifold faces (auditNonManByIndex already 0 non-vacuous; confirm zero-area count too). (4) **Correlation** — are the isotropic-<20° facets predominantly the along-crest low-curvature elongated cells (⇒ artifact) or cross-curvature needles (⇒ real defect)? Split the isotropic-<20 set by their M-metric min-angle.

**KILL-CRITERION (pre-registered, committed BEFORE measuring):**
- ACCEPT+DOCUMENT (isotropic ruler was wrong) iff, on the FREE triangles of BOTH styles: the M-metric minAngle distribution is GOOD (M-metric pctBelow20 ≪ isotropic pctBelow20 — target M-metric pctBelow20 < ~10% AND M-metric median > ~30°) AND near-zero-area count ≈ 0 (no degenerates: < ~0.1% of faces below the printer-area floor) AND STL-safe (0 non-manifold non-vacuous, 0 zero-area) AND the isotropic-<20 set is DOMINANTLY high-M-angle (the along-crest anisotropy-appropriate cells, not cross-curvature needles).
...
**STATUS: REFUTED.** BOTH pre-registered levers HOLD 0 outliers + watertight non-vacuous but FAIL to close slivers — neither moves pctBelow20 off the ~56% plateau. The pre-registered CONFIRM (pctBelow20 <~10% AND minAngle >~15°) is UNMET; the REFUTE branch ("cannot get pctBelow20 below ~15% without reopening an outlier") is MET — indeed neither lever gets it below ~56% at all, and 0 outliers are held throughout (so the "without reopening an outlier" clause is satisfied trivially — the two gates are NOT in tension here either, the levers are simply INEFFECTIVE on this sliver family).

**EVIDENCE (4-bay Gothic patch, `acceptanceGuard` 45-pt brute worst-gradU top-400 for outliers; `triangleQualityDistribution` minAngle for slivers — the honest instruments):**

| arm | tris | interiorOutliers | interiorMax mm | minAngle° | median minAngle° | pctBelow20 | watertight (nonVac) |
|---|---|---|---|---|---|---|---|
| **BEFORE** = CONFIRMED M-square (reloaded) | 58365 | 0 | 0.006 | 0 | 16 | **56.2%** | 0 (inj 0→1) ✓ |
| **Lever #2** relax Laplacian-under-M, plain umbrella (2 sweeps) | 58365 | 0 | 0.006 | 0.10 | 16 | **56.9%** | 0 (inj 0→1) ✓ |
| **Lever #2** relax quality-directed smart-Laplacian (3 sweeps, 22224/19367/15324 verts moved) | 58365 | 0 | 0.006 | 0.60 | 17 | **55.0%** (56.47→55.08→54.97 per sweep — PLATEAU) | 0 (inj 0→1) ✓ |
| **Lever #1** smooth GRADED seed (hMin0.08/hMax0.5/β0.35) + M-square refine | 67256 | 0 | 0.006 | 0 | 16 | **56.5%** | 0 (inj 0→1) ✓ |

- **Lever #1 (smooth grading) — REFUTED.** A smoothly-graded background seed (bounded size-ratio, NO abrupt dense↔coarse RED transition; seed 8495t, distance-to-crest sizing field) refined through the SAME honest-brute M-square loop reaches interiorOutliers=0 @0.006 but produces **pctBelow20 = 56.5%, minAngle 0** — statistically IDENTICAL to the abrupt-RED M-square (56.2%). ⇒ **The slivers are NOT caused by the dense↔coarse grading-transition.** They are intrinsic to the honest-brute refine's near-vertical-flank insertion pattern (RED 1→4 midpoint inserts + crest-edge subdivision make needle cells regardless of how the background is seeded). The `_pf_msq_diag` "37.6% on the grading-transition" localization was a correlation, not the cause — removing the transition did not remove the needles. (Loop hit MAX_PASS=16 with 14 residual loop-outliers, `capped=true`, but the final worst-gradU acceptanceGuard = 0 outliers; the sliver figure is what the arm tests and it is unchanged.)
- **Lever #2 (surface-preserving Laplacian-under-M relaxation) — REFUTED.** Both a plain umbrella centroid AND a quality-directed smart-Laplacian (candidate search maximizing local min incident angle, guarded to never lower it AND to never let any incident facet's honest-brute interior dev exceed 0.01) are NO-OPS on the sliver fraction: 22224/28064 free verts MOVE (maxDisp 0.26mm, 21% rejected by the fidelity guard) yet over 3 quality-directed sweeps pctBelow20 only creeps 56.2→56.47→55.08→54.97% (PLATEAU), minAngle 0→0.6°, median 16→17.6° — nowhere near the <10%/>15° bar. **Position-only relaxation cannot reshape these needles** because they are a CONNECTIVITY imbalance (long thin triangles spanning the fine-crest to coarse-panel connectivity), not a vertex-placement one — moving ONE vertex cannot fatten a triangle whose two far vertices are pinned by their own 1-rings, and the fidelity guard forbids flattening the near-vertical near-crest cells. This is the SAME point-set the a-posteriori Lawson flips (E-…-TIERAB-SLIVERS §3b) could not escape; relaxation MOVES the points but the connectivity-locked needles persist. NOT the guard blocking (only 21% rejected) — the moves are simply ineffective.

**ROOT-CAUSE REFRAME (measured, corrects the MSQUARE `_pf_msq_diag` inference):** the Gothic sliver floor is NOT a grading-transition artifact and NOT a vertex-placement artifact — it survives BOTH a smooth graded seed AND aggressive guarded relaxation. It is a CONNECTIVITY property of the honest-brute (RED 1→4 + crest-subdivision) refine near the near-vertical crest flank: any near-equilateral cell there must chord across the concave cusp (bad fidelity, forbidden by the guard) so the fidelity-holding cells are FORCED to be needles. **The 0-outlier gate and the sliver gate ARE in structural tension at the near-vertical crest flank** (contradicting the MSQUARE §VALIDATION-3 "not in hard tension" reading — they co-exist only because the needles are TOLERATED, not resolved). The remaining lever is NOT more density/placement/flips — it is a genuinely different refine PRIMITIVE at the crest flank: (a) anisotropic elements (accept high-aspect cells whose LONG axis is ALONG the crest, where the surface is near-flat, so the min-angle metric — which penalizes them — is the wrong quality gate for this locus), OR (b) a curved/one-sided PN element at the crest flank that follows the cusp WITHOUT a chord (the E-CRESTRIBBON graft, benched but proved unnecessary for FIDELITY — it may be necessary for QUALITY), OR (c) accept-and-document the near-crest needles as an anisotropy-appropriate tessellation (the long-along-crest cells are geometrically FAITHFUL, just angle-metric-ugly).

**VERDICT: REFUTED** — smooth grading + surface-preserving Laplacian-under-M relaxation (the two levers the MSQUARE refutation pointed to) both HOLD 0-outlier fidelity + watertight but CANNOT close the Gothic sliver gate (both stay at ~56% <20°, minAngle 0, vs the <10%/>15° CONFIRM bar). The Gothic near-vertical-crest sliver floor is a fidelity-vs-min-angle STRUCTURAL tension, not a grading/placement defect. The sliver gate remains OPEN.

**RECOMMENDATION:** NEXT EXPERIMENT (not productionization — the sliver gate is unmet). Stop attacking slivers with density/grading/placement/flips (now 4 refuted levers: M-square spacing, smooth grading, Laplacian relaxation, Lawson flips). Two ordered moves: (1) RE-EXAMINE THE QUALITY METRIC — the near-crest cells long-ALONG-crest are geometrically faithful (0.006mm) and the surface there IS near-flat along the crest; measure whether they are ANISOTROPY-appropriate (high aspect but the min-angle penalty is a chart artifact) via an ANISOTROPIC quality metric (min-angle UNDER M=g/h², i.e. in the metric where the crest flank is isotropic) — if the slivers vanish under the correct metric, this is an ACCEPT+DOCUMENT close, not a defect. (2) If they are genuine 3D needles under the anisotropic metric, the lever is a curved/one-sided PN element AT the crest flank (E-CRESTRIBBON graft, scoped to quality not fidelity) — the ONLY remaining primitive change, since position + connectivity-flip + grading are all refuted. BANKED reusable: `gradedSeed` (smooth size-field seed, correct + reused), `relaxLaplacianUnderM` (guarded surface-preserving smart-Laplacian, LOCK-clean, holds 0 outliers — a correct instrument even though a no-op here). The honest-brute STOP driver holds fidelity through all of it.
...
**DISCRIMINATOR (cheapest-first):** (0) RECON — per-row full-ring crest count vs t: **0→7→16→16→32→8→0 per tile** (RECON-confirmed; relief lives only in narrow t-bands, amp ~1.72mm at t≈0.1, exactly 0 at tile centres t=0.15/0.20). (1) PN-MECH facet-scale flat-vs-PN reconstruction of the worst chevron apex (seconds, standalone). (2) end-to-end brute-driven flat-P1 refine + honest guard, same instruments as the Gothic crux. KILL-CRITERION (pre-registered in the probe header, committed BEFORE measuring): CONFIRM iff interiorOutliersFinal=0 AND watertight non-vacuous AND manifold at ≤6M-equiv; note usedPnAtApex. REFUTE iff BOTH flat-P1 AND scoped-apex-PN floor >0.02 on-crest, OR the count-instability breaks the topology pipeline.

**EVIDENCE (labkit honest instruments; TRUE-3D = acceptanceGuard ≥36-pt denseBary(8) barycentric lattice back-projected via full-azimuth bruteNearestOnRadialSurface, worst-gradU top-400 population; watertight = auditNonManByIndex NON-VACUOUS; slivers = triangleQualityDistribution.minAngle):**

- **PN-MECH (facet-scale, the mechanism screen):** devFlat/devPN vs facet arc-width — 0.57mm: 0.191/0.167 · 0.14mm: 0.030/0.024 · **0.071mm: 0.0085/0.0080 (flat-P1 ALREADY < tol)** · 0.035mm: 0.0026/0.0023. ⇒ **flat-P1 alone crosses below tol at facet arc ~0.07mm; PN gives only a ~10-30% extra, NOT the order-of-magnitude Gothic needed.** Confirms the spec: finite-width kink, flat-P1 rides it with modest density.
- **END-TO-END (step1, bays=5, zBand=10mm, tCenter=0.08, bg 0.11mm, edge-mode geometric convergence):** extract fam=2 segU=728 segT=215 **residualCrossings=0** (planarizeMM fully planar — topology pipeline HANDLES the count-instability, unlike doubled-crest E-2026-07-04-DCGS). Refine **CONVERGED in 5 passes, NOT capped** (947s): outliers 4824→4280→222→16→**0**, worst 0.16→0.056→0.047→0.013→**0.006**. Guard (400 worst-gradU facets, gradU 233-247): **interiorMax=0.006mm, interiorOutliers=0** (onCrest=0 off=0), p99 0.006. Watertight **nonMan=0, injected-crack=1 ⇒ NON-VACUOUS**. **usedPnAtApex=FALSE** (flat-P1 alone reached 0; scoped apex PN NOT needed). 112,863 tris, 56,953 verts.
- **RENDER:** `research/exchange/_pf_perfect_geostar_brute/geostar_true3d.png` — whole patch GREEN under the true-3D perpendicular ruler (heatmap centroid metric p99 0.018 / worst 0.023 / 0.00% >0.03; no red on the chevron strap flanks).

**VERDICT: CONFIRMED** (kill-criterion MET: 0 interior outliers + watertight non-vacuous + manifold, at 0.11M tris ≤ 6M, via flat-P1 alone — no apex PN). The count-unstable chevron cusp is admissible to the FGJ-Morse + CDT-junction-lock + honest-brute-refine kernel because that kernel is COUNT-AGNOSTIC (nearest-neighbour crest linking + planarizeMM X-split births/dies chains; it does NOT pin vanished straps like the fixed-nSlot doubled-crest column primitive that floored GeoStar at 26mm). GeoStar was EASIER than Gothic exactly as the spec predicted: flat-P1 converged (Gothic-BRUTE also flat-P1, both 0.006), no PN.

**CAVEAT (honest, NOT in the kill-criterion):** slivers — minAngle=0.00°, median 41°, **pctBelow20=21.3%** (Gothic-BRUTE 1-bay was worse at 81.4%). These are thin facets from the planarize junction fans + the strap-birth/death chain terminations, density-INVARIANT (same defect family flagged in E-2026-07-05-PERFECT-MESHER-TIERAB-SLIVERS). The FIDELITY + WATERTIGHT half is done; a metric-aware / Delaunay-quality apex-spacing pass (M=g/h² gate) is the SEPARATE next lever before this is print-usable — same recommendation as the Gothic tier.

**RECOMMENDATION:** (1) the kernel is now proven on BOTH count-unstable cusps (Gothic zero-width + GeoStar finite-width chevron) — the count-agnostic FGJ+CDT-lock+brute-refine architecture generalizes across the unstable-count family that DEFEATED the doubled-crest column primitive. Route unstable-count styles here, NOT to doubled-crest. (2) Next: the sliver gate (M=g/h² quality spacing at junction fans) — validate it drives pctBelow20→~0 while HOLDING the 0-outlier fidelity, on GeoStar+Gothic together. (3) NOTHING ships; dev-only.

**LEDGER:** probe `research/bridge/_pf_perfect_geostar_brute.test.ts` (PF_PERFECTGS=1: step1/step2; PF_PNMECH=1: pn-mech) + patch lib `research/bridge/_pf_geostarPatchLib.ts` (makeGeoStarPatch — the ONLY new code; downstream reused VERBATIM from `_pf_perfectMesherLib` + `_pf_perfectMesherBruteLib`) + recon `research/bridge/_pf_geostar_recon.test.ts` (PF_GSRECON=1) + render `research/bridge/_pf_geostar_render.test.ts` (PF_GSRENDER=1). Configs `vitest.pf_perfectgs.config.ts` / `vitest.pf_gsrecon.config.ts` / `vitest.pf_gsrender.config.ts`. Scorecard `research/exchange/_pf_perfect_geostar_brute/scorecard.ndjson` (step1+step2 rows) + `step1.json` + `refine_passes.ndjson` (5 passes) + `geostar_true3d.png`.
...
**EVIDENCE (measured, real vitest v4.0.17, canonical ut+indices sha256; guard = same `acceptanceGuard` ≥45-pt barycentric worst-gradU top-400 two-stage GN→full-azimuth `bruteNearestOnRadialSurface` box-refined; slivers = `triangleQualityDistribution` minAngle; watertight = `auditNonManByIndex` non-vacuous):**

- **(1) CLOSER-OFF BYTE-IDENTITY — REFUTED for BOTH styles.** The kernel has NO closer-off delegation branch (grep-verified: `_pf_perfectMesherLib`/`_pf_perfectMesherBruteLib` contain no closer/delegate/primitive path); its ONLY generator is `seedMesh` (uniform (u,t)-grid CDT). Measured at bgArcMm=1.2 vs `buildInhouseMetricMesh`(tolMm .1/hMin .2/hMax 4/maxPoints 400k):
  - **HarmonicRipple (Tier-A smooth):** closer-OFF `22989v/45312t` hash `84c5edc4…` vs primitive `21960v/43342t` hash `0c916666…` ⇒ **byteIdentical=FALSE.**
  - **ArtDeco (Tier-B):** closer-OFF `22989v/45312t` hash `84c5edc4…` vs primitive `14951v/29549t` hash `34788451…` ⇒ **byteIdentical=FALSE.**
  - **Decisive structural reason:** closer-OFF gives the SAME counts + SAME hash (`84c5edc4…`) for BOTH styles — because `seedMesh` with an empty complex builds a **style-BLIND uniform (u,t) grid** (the radius fn only enters the LIFT, not the (u,t) point set). "Today's primitive" the dispatch table names (`buildInhouseMetricMesh` = dense-M-square) is a **curvature-ADAPTIVE M=g/h² mesh** (Delaunator on median-anisotropy-scaled coords + true-3D flips + on-surface smoothing) — a fundamentally different generator (different point set, count, topology). deterministic=TRUE (closer-off path is stable), byteIdentical=FALSE. The §4/§6 "byte-identical when the flag is off ⇒ zero regression on Tier-A/B" claim is a **DESIGN CLAIM about an UNIMPLEMENTED code path**, not a measured property.

- **(2) M=g/h² SLIVER GATE holding 0-outlier — REFUTED (flips can't hold both gates).** On the CONFIRMED 1-bay edge-mode Gothic mesh (4987v/9885t, loaded from the persisted `_smoke/refined_mesh.bin`; alignment-guarded: rebuilt complex cEdges=141 matches, prefix verts aligned=true), a true-3D max-min-angle Lawson flip pass (`flipHE`, crest constraint edges + crest-vertex sub-edges LOCKED, guardManifold=true, 8 sweeps) was applied and re-measured with the honest guard. THREE variants:
  - **LOCK_ALL (0 flips, control):** mesh byte-identical, outliers stay 0, max 0.006, slivers unchanged — proves the halfedge build (asymmetricTwins=0, boundary=791) + orientation-normalize (134 CW→CCW) + flipHE relink are CLEAN (no instrument corruption).
  - **Pure max-min-angle flips:** 5295 flips fired, but **interior outliers REOPEN 0→57 (ALL 57 ON-crest, 0 off-crest), interiorMax 0.006→0.130, p99→0.128**; slivers barely move (minAngle 0→0, median 3→5, pctBelow20 **81.4→77.4%**). Watertight nonMan=0 non-vacuous (0→1). **outliersStay0=FALSE.**
  - **Fidelity-guarded flips** (reject a flip whose new facet chord-sag >0.01): 5271 flips, outliers 0→50 on-crest, max 0.194 (WORSE) — because the cheap in-flip facet-plane sag ruler UNDERSTATES true-3D on near-vertical flanks (the exact E-2026-07-04 GN-blindness gotcha), so it fails to reject the bridging flips; only the intractable full-azimuth brute would.

**VERDICT: REFUTED on BOTH sub-claims. reaches0outlier (byte-identity-holds AND slivers-closable-with-0-outliers) = FALSE.**
(1) Closer-OFF is NOT byte-identical to the named primitive — the kernel's uniform-grid seed is a style-blind generator, architecturally different from the curvature-adaptive M-mesh; zero-regression on Tier-A/B is UNPROVEN by measurement (and unimplemented as a delegation path). (2) A post-hoc metric-aware quality FLIP pass CANNOT recover the 81.4%-slivers while holding interior outliers at 0: max-min-angle flips near the zero-width apex swap to better-angled diagonals that BRIDGE the concave cusp (57 on-crest outliers → 0.130), and a cheap fidelity guard is blind (understates true-3D). The 0-outlier gate and the sliver gate are genuinely in TENSION for the edge-mode point set, and topology-only flips do not resolve it.

**ROOT CAUSE + why flips can't help:** the 81.4% slivers are baked into the edge-mode POINT SET — uniform 1→4 red-refine at a zero-width apex creates needle vertex configurations. Any triangulation of that point set that is near-equilateral (good angle) near the apex must chord across the concave cusp (bad fidelity); any triangulation that follows the cusp (good fidelity) must be a needle (bad angle). Flips only re-connect the SAME points ⇒ cannot escape this. The fix must be metric-aware SPACING at REFINEMENT time (place Steiner nodes to equalize the M=g/h² metric so triangles are BORN near-equilateral on the flank), NOT a post-hoc flip — exactly as the E-…-BRUTE recommendation stated.

**HONEST CAVEATS / SCOPE:** (1) byte-identity tested at a single bgArcMm/budget pairing — but the REFUTE is structural (style-blind vs adaptive), invariant to budget; no pairing makes a uniform grid equal an adaptive M-mesh. (2) The flip-pass REFUTE is on the 1-bay edge-mode mesh (the only persisted CONFIRM mesh); 5-bay+ is UNMEASURED but the mechanism (needle point set at apex) is scale-invariant. (3) NOT tested: metric-aware SPACING at refine time (the recommended fix) — that requires modifying `refineInteriorBrute`'s insertion to metric-equalized placement, a NEW experiment. (4) No visual render produced (the finding is numeric + topological; the sliver picture is fully captured by minAngle/pctBelow20).

**RECOMMENDATION:** (a) Do NOT ledger the spec's "byte-identical closer-off ⇒ zero regression" as proven — it is UNIMPLEMENTED + REFUTED as literally stated; if the perfect-mesher is ever productionized, the closer-off path must EXPLICITLY delegate to `buildInhouseMetricMesh` (not build a uniform grid) to earn the zero-regression claim, and that delegation must be byte-audited. (b) The sliver gate is NOT closable by flips — the next experiment is metric-aware SPACING: modify the edge-mode refine to insert flank Steiner nodes at M=g/h²-equalized positions (near-equilateral-by-construction) and re-measure minAngle WITH the honest guard holding outliers at 0. That is the one path that can hold both gates; the flip pass is a dead end for this point set. (c) Banked-clean: the halfedge build + orientation-normalize + flipHE-with-lock is a correct, reusable metric-flip instrument (LOCK_ALL control proved it byte-clean).
...
**KILL-CRITERION (pre-registered, committed fa1936e BEFORE measuring):** CONFIRM iff interiorOutliersFinal=0 (honest ≥36-pt brute, worst-gradU) AND watertight (auditNonManByIndex=0 non-vacuous) AND manifold, at ≤6M-equiv (note usedPnAtApex). REFUTE iff BOTH brute-driven flat-P1 AND scoped-apex-PN floor >0.02 on-crest ⇒ zero-width apex below a P2 element's reach. Report FLAT-P1 (interiorOutliersFlatP1) SEPARATELY from FINAL.

**EVIDENCE (measured, real vitest v4.0.17, honest full-azimuth `bruteNearestOnRadialSurface` STOP test + acceptance guard):**
- **STEP-1 element-level (pn-mech / pn-fine, well-shaped apex facet aspect~1, brute 2048×160):** flat-P1 interior true-3D CONVERGES with apex density — devFlat crosses <0.02 at ~0.035mm arc, **0.0178 @0.035mm → 0.0009 @0.0022mm arc**. One-sided Vlachos PN ≈ flat everywhere (0.0207/0.0033/0.0008 — sometimes WORSE than flat). ⇒ the Gothic zero-width apex is **NOT below a flat-P1 element's reach**; PN is **UNNECESSARY**. (First construction with dt=0.02 gave a spurious GROWING error — a z-ridge SLIVER facet measuring crest-in-z curvature, not the u-cusp; fixed by aspect~1 dt, diagnosed in `_pf_pndiag`.)
- **STEP-1 whole-mesh, POINT-mode (one-node-at-worst-sample, 5-bay×14mm, brute-driven STOP):** worstBrute floors ~0.26–0.31 in the ACTIVE set across 8 passes but the settled-mesh acceptance GUARD (worst-gradU top-400) reads **interiorMax=0.078, only 2 on-crest outliers, p99=0.006, wt=0 non-vacuous** — CONVERGING but CAPPED at pass 9 (the one-node-per-facet insertion is too slow at a persistent zero-width apex; the 2 residual leaves need ~6 more halvings).
- **STEP-1 whole-mesh, EDGE-mode (RED 1→4 edge-midpoint refine ⇒ geometric convergence, 1-bay×5mm patch):** **CONVERGED in 5 passes (capped=false), worstBrute 0.276→0.270→0.264→0.169→0.008.** Acceptance GUARD (worst-gradU top-297, gradU 211–235 near-vertical): **interiorOutliersFlatP1 = 0, interiorMaxFlatP1Mm = 0.006, p99 = 0.006.** Watertight auditNonManByIndex=0 **NON-VACUOUS control CONFIRMED (inject 3rd-tri-on-edge → 0→1)**, manifold, familyCount=2, residualCrossings=0. **usedPnAtApex = false** (STEP-2 short-circuited: flat-P1 already 0). 9885 tris (1-bay).
- **Sliver gate (SEPARATE, FAILS):** EDGE-mode minAngle 0.00°, median 3°, **pctBelow20 = 81.4%** (worse than point-mode's 33.5% / the prior GN kernel's 4.6%) — the unconditioned 1→4 apex red-refine makes needle slivers. The 0-outlier gate and sliver gate are separate; this mesh is NOT production-quality without metric-aware apex spacing.

**VERDICT: CONFIRMED (1-bay whole-mesh, flat-P1-alone, usedPnAtApex=false) — OVERTURNS the E-2026-07-04 "flat-P1 is NOT the Gothic closer / needs a curved element" REFUTE.** The prior REFUTE was a MEASUREMENT-DRIVER artifact, exactly as its own recommendation (i) predicted: the GN loop terminated blind at 0.0085; when the STOP test is the honest brute AND refinement is geometric (edge-halving, not one-node-per-pass), flat-P1 reaches **0 interior outliers, max 0.006mm** on the honest worst-gradU brute guard, watertight non-vacuous + manifold. PN is **provably unnecessary** (pn-fine: PN≈flat; contradicts E-CRESTRIBBON's "needs curved element" framing — the correctly-scoped honest re-measure shows the element order is NOT the lever, the DENSITY-DELIVERY mechanism + honest STOP test are). The wall was never representational — it was (a) a blind ruler and (b) a too-slow density-delivery loop.

**HONEST CAVEATS / SCOPE:** (1) CONFIRM is on a **1-BAY** patch (9885 tris); the 5-bay point-mode run left 2 residual outliers ONLY because it capped — edge-mode on 5-bay was started but the env-cost (3× nodes/pass) made pass-1 intractable in the run window (killed clean), so **5-bay+ edge-mode whole-mesh is UNMEASURED** (extrapolation, not a result). (2) The **sliver gate FAILS hard (81.4% <20°)** — a metric-aware / Delaunay-refinement apex spacing is REQUIRED before this is usable; the 0-outlier win is sliver-dirty. (3) Cost: edge-mode fat-tail at the apex is the risk — 1-bay reached 9885 tris; whole-Gothic tri-count under the 6M budget is UNMEASURED. (4) GeoStar (the other count-unstable cusp) UNTESTED.

**RECOMMENDATION:** productionize-path is NOT yet open (sliver gate + 5-bay+ scale + cost unmeasured), but the FRONTIER is DIFFERENT than the prior REFUTE claimed: Gothic ≤0.01 is reachable with FLAT P1 — no curved element needed — GIVEN (i) the honest brute as the loop STOP driver and (ii) geometric (edge-halving) apex density. Next experiments, ordered: (1) add metric-aware/Delaunay-quality apex spacing to kill the 81.4% sliver tail while HOLDING interiorOutliers=0 (the two gates together); (2) run EDGE-mode on 5-bay+ and measure whole-Gothic tri-count vs 6M; (3) the GeoStar arm. The banked reusable win stands: FGJ planarizeMM + no-bridge junction complex is watertight-proven; ADD: the honest-brute-driven + edge-refine loop (`refineInteriorBrute` mode='edge').
...
- **KERNEL BUILT END-TO-END + WATERTIGHT + MANIFOLD (all non-fidelity gates PASS):** Morse ridge graph extracted (familyCount=2, nSegU=1108, nSegT=177) → `planarizeMM` (new robust mm-space PSLG planarizer — the seam-aware `planarizeConstraintGraph` reported residual=0 but left **840 real mm crossings** that crash cdt2d `upperIds`; planarizeMM splits every crossing+T-junction) → **residualCrossings=0, 100% constraint recovery** → CDT with 1320 locked crest edges (no-bridge split) → INTERIOR-CRITERION arc-length-graded Steiner refine loop **CONVERGED in 4 passes, capped=false** (worstGN 0.031→0.0156→0.0149→**0.0085**, GN-outliers 6932→2941→319→**0**), 149,051 tris. **Watertight `auditNonManByIndex`=0 by index; NON-VACUOUS control CONFIRMED** (inject a 3rd triangle on an edge → count 0→1). Manifold across the junction network. (5-bay × 14mm single-arch patch, dims {H:120,Rb:40,Rt:50,expn:1}.)
- **FIDELITY GATE (the go/no-go) FAILS:** acceptance guard = ≥36-pt (45-pt) barycentric sampler on the **worst-gradU population** (top-400 facets, gradU 230–253 mm/rad = near-vertical flanks), **full-azimuth `bruteNearestOnRadialSurface` box-refined** (1024×120, refineIters=60, trusted-class per the SURFNATIVE calibration): **interiorMax = 0.13279mm, interiorOutliers = 113, ALL 113 ON-CREST (0 off-crest), guardP99 = 0.12964, guardP50 = 0.00542.** Reproduced identically across two independent runs.
- **Sliver gate (separate):** minAngle 0.10°, medianMinAngle 43°, pctBelow20 = 4.6% (a known regression tail).

**ROOT CAUSE (the decisive diagnosis):** the GN-driven refine loop terminated at worstGN=0.0085 (its driver said "0 outliers") but the honest full-azimuth brute reveals the near-vertical flank facets floor at **0.133mm** — the GN own-azimuth foot **UNDERSTATES** true-3D on steep flanks (the documented steep-lattice gotcha, LAB-CHEATSHEET). The mechanism did NOT actually reach 0 interior outliers under the honest ruler; it floors ~0.13mm on the 113 on-crest facets. Two engineering bugs found + fixed en route (both instructive): (1) the mm-crossing/`upperIds` cdt2d crash → planarizeMM; (2) a seam-normalization bug flipping u=−0.02→0.98 into a 248mm seam-spanning facet (worst 3.4mm) → keep native patch u.

**VERDICT: REFUTE (per the pre-registered kill-criterion: "any facet floors >0.02 after the interior loop terminates").** 0.133 ≫ 0.02, all on-crest, flank-pitch-refined-to-convergence. This is CONSISTENT with, and STRENGTHENS, the E-VERIFY-INTRINSIC-APEX refutation and the FGJ/INTRINSIC-APEX floors: a **flat P1 element cannot follow the Gothic zero-width `pow(sharp)` apex to ≤0.01 even with (a) a topologically-complete junction graph (residualCrossings=0, 100% recovery), (b) the crest as a protected no-bridge shared edge, AND (c) arc-length-graded facet-interior refinement driven to GN-convergence.** The E-RACE-SURFNATIVE proxy's 0.006mm was measured on ONE hand-split cusp with the GN-vs-brute gap not exposed on the whole junction network's worst population — it did NOT survive the honest whole-mesh brute guard. The ONLY non-fidelity claim that DID survive at whole-mesh scale: **the junction network splits non-manifold-free (residualCrossings=0, watertight nonMan=0, manifold) — FGJ's graph machinery is real and reusable.**

**RECOMMENDATION:** The whole-mesh, whole-network go/no-go says **flat-P1 surface-native + arc-length grading is NOT the Gothic 0-outlier closer** (REFUTE, not NO-OP: it beats the 0.058 embedded-crest floor's population but still floors 0.133 on the honest worst-gradU brute). Do NOT productionize as a Gothic ≤0.01 solution. The frontier remains: reaching ≤0.01 at the zero-width `pow(sharp)` apex needs a genuinely CURVED (P2/PN) element at the near-apex leaf (E-CRESTRIBBON's correctly-scoped fallback) OR an accept-tiny-residual policy at ~0.13mm — flat-Steiner-to-convergence provably does not. Also: (i) the refine loop MUST use the honest brute (not GN) as its termination driver, since GN understates the near-vertical flank; (ii) the FGJ planarizeMM + no-bridge junction complex is a banked, reusable, watertight-proven component. Next experiment: swap the near-apex leaf's flat element for a one-sided PN element (graft #2) on this exact end-to-end kernel and re-measure the same guard.
