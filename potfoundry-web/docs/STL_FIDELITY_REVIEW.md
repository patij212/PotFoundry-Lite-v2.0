...
### Why Jagged Features & Low Resolution?

Recent analysis identifies a **critical bottleneck** causing the "jagged feature" and "low resolution" artifacts observed in high-detail exports.

#### 1. The "Filtered Feature" Disconnect
*   **The Problem:** The GPU detects 100,000+ valid feature points. However, `useAdaptiveExport.ts` **filters this list to the top 5,000 points** (by strength) before passing it to the CPU.
*   **Why?** The CPU-based `cdt2d` triangulation is too slow to handle >10k constraints.
*   **The Artifact:** 
    *   **Missing Features:** Weak but visible feature lines are deleted.
    *   **Background Invasion:** The `ConstrainedTriangulator` sees these deleted areas as "empty" and fills them with a **coarse background grid**. This is why "features are low resolution" — they are being replaced by the background grid.
    *   **Jagged Snapping:** The GPU `snap_vertex` shader only receives the *sparse* 5k list. It snaps vertices to these distant, discrete points instead of a smooth continuous curve, creating a "staircase" or "Voronoi" effect along edges.

#### 2. Inefficient Mesh Density
*   **The Problem:** Smooth, non-featured areas are over-subdivided.
*   **Cause:** The `sag_circle` error term in `adaptive_mesh.wgsl` forces subdivision based on global curvature radius.
*   **Impact:** Without specific "Feature" guidance (because they were filtered), the shader assumes *everything* needs high subdivision to look round. This wastes triangle budget on smooth walls while starving the complex features (which are stuck with coarse background geometry).
