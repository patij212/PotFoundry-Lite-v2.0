...
**Priority 2 — dimension-space hardening (THE cutover blocker — DONE the harness, FOUND 2 gaps).** `__pfFidelity.setDimensions` + `e2e/_conforming_dimspace_probe.cjs` are built; run them. Results (`e2e/dimspace-findings-2026-06-08i.md`): the conforming path is clean at DEFAULT dims but BREAKS at extremes, and the breakage is BROADER than insertion:
  - **Gap 1 — SHORT-WIDE (flat wide dish, H40/OD300): ALL feature-bearing styles explode** (sliver up to 2.9M), INCLUDING the pre-existing warp styles GothicArches/LowPolyFacet (smooth styles stay clean). Cause: the `minUniformLevel` (warp columns) / `featureLevel` (insertion) floor forces a uniform 2^L×2^L (u,t) grid; on a short-t pot those square (u,t) cells map to extreme-aspect 3D (circumference/height ≈ 23:1) → 3D slivers, and the floor over-refines (3.4M tris vs 316k default). This is PRE-EXISTING (not from this session's work) and the HIGHEST-IMPACT fix. Likely fix: make the floor ANISOTROPIC (force fine-u columns without fine-t rows — the warp needs full-HEIGHT columns to EXIST, not to be t-subdivided) or aspect-cap the floor level. This is a quadtree/sizing change (PeriodicBalancedQuadtree.minUniformLevel + the warp column-lattice) — gate carefully (it touches the 8 warp styles).
